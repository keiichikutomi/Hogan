...
/*ultimateshearofpccolumn,ultimateshearofpcgirder Modified.*/
...
/*** 09.02.25 araki for obj ***************************************************/
struct estresspn{struct stress xp,xn,yp,yn,z;};
          /*xp,yp:HORIZONTAL POSI xn,yn:HORIZONTAL NEGA z:VERTICAL.*/
struct elementpn{
			   long int code;                        /*ELEMENT CODE*/
               struct section *sect;                      /*SECTION*/
			   struct structnode *node[2];             /*NODES[END]*/
			   struct estresspn head,tail;              /*STRESSES*/
			   int cmqcode;
               double Mo;                                     /*CMQ*/
              };
/******************************************************************************/
...
void inputfiletomemory2(FILE *ftext,
                       ...
                       struct element *elems);
/*** 09.02.25 araki for obj ***************************************************/
void inputfiletomemory3(FILE *ftext,
                       int *nnode,int *nelem,int *nsect,
                       struct section *sects,
                       struct structnode *nodes,
					   struct elementpn *elems);
/******************************************************************************/
...
double allowablebendingofrc(struct element elem,
                            struct materials m,
							int axis,
							/*double Nd,int period); */      //Default
							double Nd,int period,char sign); //130309 fukushima for RC
...
double ultimatebendingofsrc(struct element elem,
                            ...
                            double *rN,double *rM,
                            double *cN,double *cM,
                            char sign); // 150605 fukushima for ultimatebendingofsrc
...
double allowablecompressionofsteel(double E,double F,                  //LkSato
                                   double lkxx,double lkyy,
                                   struct section sect);
double allowablecompressionofsteel2(double E,double F,                  //TAKATA
                                   double lkxx,double lkyy,
                                   struct section sect,int axis);
...
double allowablebendingofpccolumn(struct element elem,
                                  ...
                                  double Nd,
                                  char sign); // 150416 fukushima for PC
double allowablebendingofpcgirder(struct element elem,
                                  struct materials m,int axis,
                                  double M); // 160302 fukushima for PC
...
double ultimatebendingofpcgirder(struct element elem,
                                 struct materials m,int axis,
								 int tensedside,
								 double Nd);       /*** 16.05.15 araki for PC ***/
...
int srcan001(char fname[])
{
  ...
  double uNpc,uQpc[2][2];  /*u:ULTIMATE*/ /*16.05.12 araki for PC route 3a*/
  /*double Ma,Mu,Muhead,Mutail,Qa,Qu;*/ /*a:ALLOABLE u:ULTIMATE*/  //Default
  double Mahead,Matail,Mu,Muhead,Mutail,Qa,Qu; /*a:ALLOABLE u:ULTIMATE*/ //130309 fukushima for RC
  ...
  /*** 2014.06.03 araki for safety long,short ********************/
  double rateml,ratems,rateql,rateqs;
  double mlrate[MAXSECT],msrate[MAXSECT];
  double qlrate[MAXSECT],qsrate[MAXSECT];
  /***************************************************************/
  /*int check;*/

  double sQrc,sMrc;

  double xarate1,xarate2,yarate1,yarate2,xarate,yarate;
  double xurate1,xurate2,yurate1,yurate2,xurate,yurate;
  int ielem1,ielem2,el=0,flag=0;
  ...
  else if(!strncmp(non,"Takaido",7)) strcpy(prj,"Takaido");
  else if(!strncmp(non,"Soleil", 6)) strcpy(prj,"Soleil");
  else if(!strncmp(non,"Ginza",  5)) strcpy(prj,"Ginza");
  else if(!strncmp(non,"RGinza", 6)) strcpy(prj,"RGinza");
  else if(!strncmp(non,"Kouenji",7)) strcpy(prj,"Kouenji");
  else if(!strncmp(non,"Table",  5)) strcpy(prj,"Table");
  else if(!strncmp(non,"Alumi",  5)) strcpy(prj,"Alumi");
  else if(!strncmp(non,"vene",   4)) strcpy(prj,"vene");
  else if(!strncmp(non,"ooiso",  5)) strcpy(prj,"ooiso");
  else if(!strncmp(non,"yoko",   4)) strcpy(prj,"yoko");
  else if(!strncmp(non,"gcdp",   4)) strcpy(prj,"gcdp");
  else if(!strncmp(non,"obj",    3)) strcpy(prj,"obj");
  else if(!strncmp(non,"timber", 6)) strcpy(prj,"timber");
  else if(!strncmp(non,"turu",   4)) strcpy(prj,"turu");
  else if(!strncmp(non,"chidori",7)) strcpy(prj,"chidori");
  else if(!strncmp(non,"chair",  5)) strcpy(prj,"chair");
  else if(!strncmp(non,"kato",   4)) strcpy(prj,"kato");
  else if(!strncmp(non,"vict",   4)) strcpy(prj,"vict");
  else if(!strncmp(non,"mita",   4)) strcpy(prj,"mita");
  else if(!strncmp(non,"rest",   4)) strcpy(prj,"rest");
  else if(!strncmp(non,"gate",   4)) strcpy(prj,"gate");
  else if(!strncmp(non,"boma",   4)) strcpy(prj,"boma");
  else if(!strncmp(non,"misato", 6)) strcpy(prj,"misato");
  else if(!strncmp(non,"yamaga", 6)) strcpy(prj,"yamaga");
  else if(!strncmp(non,"venizu", 6)) strcpy(prj,"venizu");
  else if(!strncmp(non,"lacie",  5)) strcpy(prj,"lacie");
  else if(!strncmp(non,"kyouai", 6)) strcpy(prj,"kyouai");
  else if(!strncmp(non,"ordos",  5)) strcpy(prj,"ordos");
  else if(!strncmp(non,"katsu",  5)) strcpy(prj,"katsu");
  else if(!strncmp(non,"hakushima",9)) strcpy(prj,"hakushima");
  else if(!strncmp(non,"aaxc",   4)) strcpy(prj,"aaxc");
  else if(!strncmp(non,"ureshiarena", 11)) strcpy(prj,"ureshiarena");
  else if(!strncmp(non,"hakutruss",9)) strcpy(prj,"hakutruss");
  else if(!strncmp(non,"sunny",  5)) strcpy(prj,"sunny");
  else if(!strncmp(non,"ureshitok", 9)) strcpy(prj,"ureshitok");
  else if(!strncmp(non,"ttbrest",    7)) strcpy(prj,"ttbrest");
  else if(!strncmp(non,"ttbpergola",10)) strcpy(prj,"ttbpergola");
  else if(!strncmp(non,"rikuzenwhole",12)) strcpy(prj,"rikuzenwhole");
  else if(!strncmp(non,"rikuzengym",10)) strcpy(prj,"rikuzengym");
  else if(!strncmp(non,"rikuzenbdj",10)) strcpy(prj,"rikuzenbdj");
  else if(!strncmp(non,"fukuda", 6)) strcpy(prj,"fukuda");
  else if(!strncmp(non,"mjdrg",  5)) strcpy(prj,"mjdrg");
  else if(!strncmp(non,"gialai", 6)) strcpy(prj,"gialai");
  else if(!strncmp(non,"west",4))    strcpy(prj,"west");
  else if(!strncmp(non,"east",4))    strcpy(prj,"east");
  else if(!strncmp(non,"mugi",4))    strcpy(prj,"mugi");
  else if(!strncmp(non,"tskr",4))    strcpy(prj,"tskr");
  else if(!strncmp(non,"obihiro",7)) strcpy(prj,"obihiro");
  else if(!strncmp(non,"housho",6))  strcpy(prj,"housho");
  else if(!strncmp(non,"aguri",5))   strcpy(prj,"aguri");
  else if(!strncmp(non,"fkym",4))    strcpy(prj,"fkym");
  else if(!strncmp(non,"kousa",5))   strcpy(prj,"kousa");
  else if(!strncmp(non,"rokunohe",8)) strcpy(prj,"rokunohe");
  else if(!strncmp(non,"osaka",  5)) strcpy(prj,"osaka");
  else if(!strncmp(non,"kuma",  4))  strcpy(prj,"kuma");
  else if(!strncmp(non,"toyono",6))  strcpy(prj,"toyono");
  else if(!strncmp(non,"kodaira",7)) strcpy(prj,"kodaira");
  else if(!strncmp(non,"osakiso",7)) strcpy(prj,"osakiso");
  else if(!strncmp(non,"tanno",5))   strcpy(prj,"tanno");
  else if(!strncmp(non,"Mnl",3))     strcpy(prj,"Mnl");
  else if(!strncmp(non,"okumura",7)) strcpy(prj,"okumura");
  else if(!strncmp(non,"funakoshi",9)) strcpy(prj,"funakoshi");
  else if(!strncmp(non,"suma",  4))  strcpy(prj,"suma");
  else if(!strncmp(non,"kamijujo",8)) strcpy(prj,"kamijujo");
  else if(!strncmp(non,"abiko",5))   strcpy(prj,"abiko");
  else if(!strncmp(non,"npuae",5))   strcpy(prj,"npuae");
  else if(!strncmp(non,"shirakawa",9)) strcpy(prj,"shirakawa");
  else if(!strncmp(non,"pkangh",6))  strcpy(prj,"pkangh");
  else if(!strncmp(non,"taichung",8)) strcpy(prj,"taichung");
  else if(!strncmp(non,"ishiro",6))  strcpy(prj,"ishiro");
  else if(!strncmp(non,"hashi",5))   strcpy(prj,"hashi");
  else if(!strncmp(non,"nishijin",8)) strcpy(prj,"nishijin");
  else if(!strncmp(non,"ichikawa",8)) strcpy(prj,"ichikawa");
  else if(!strncmp(non,"tondabayashi",12)) strcpy(prj,"tondabayashi");

  if(!strcmp(prj,"tondabayashi")) Ds=0.15; //araki for tondabayashi
  ...
  fprintf(fout0,"‰”’¼‰×dŽž‰ðÍŒ‹‰Ê       =%s\n",non);
  fprintf(fsafe,"%s\n",non);
  GetDlgItemText((wmenu.childs+2)->hwnd,ID_OUTPUTFILEX,non,80);
  fprintf(fout0,"…•½‰×dŽž‰ðÍŒ‹‰Ê X•ûŒü =%s\n",non);
  fprintf(fsafe,"%s\n",non);
  ...
  if(!strcmp(prj,"ken")  || !strcmp(prj,"tei") ||
     ...
     !strcmp(prj,"Takaido") || !strcmp(prj,"Soleil") ||
     !strcmp(prj,"Ginza") || !strcmp(prj,"Kouenji") ||
     !strcmp(prj,"Table") || !strcmp(prj,"Alumi") ||
	 !strcmp(prj,"RGinza") || !strcmp(prj,"vene") ||
     !strcmp(prj,"ooiso") || !strcmp(prj,"yoko") ||
	 !strcmp(prj,"gcdp") || !strcmp(prj,"obj") ||
	 !strcmp(prj,"timber") || !strcmp(prj,"turu") ||
     !strcmp(prj,"chidori") || !strcmp(prj,"chair") ||
     !strcmp(prj,"kato") || !strcmp(prj,"vict") ||
     !strcmp(prj,"mita") || !strcmp(prj,"rest") ||
	 !strcmp(prj,"misato") || !strcmp(prj,"yamaga")||
     !strcmp(prj,"venizu") || !strcmp(prj,"lacie")||
     !strcmp(prj,"kyouai") || !strcmp(prj,"ordos")||
     !strcmp(prj,"katsu")  || !strcmp(prj,"hakushima")||
     !strcmp(prj,"boma") || !strcmp(prj,"aaxc")||
	 !strcmp(prj,"ureshiarena") || !strcmp(prj,"hakutruss")||
	 !strcmp(prj,"gate") || !strcmp(prj,"sunny")||
	 !strcmp(prj,"ureshitok") || !strcmp(prj,"ttbrest")||
	 !strcmp(prj,"ttbpergola")|| !strcmp(prj,"rikuzenwhole")||
	 !strcmp(prj,"rikuzengym")|| !strcmp(prj,"rikuzenbdj")||
	 !strcmp(prj,"fukuda")|| !strcmp(prj,"mjdrg")||
	 !strcmp(prj,"gialai")|| !strcmp(prj,"west")||
	 !strcmp(prj,"east")  || !strcmp(prj,"mugi")||
	 !strcmp(prj,"tskr")  || !strcmp(prj,"obihiro")||
	 !strcmp(prj,"housho")  || !strcmp(prj,"aguri")||
	 !strcmp(prj,"fkym")  || !strcmp(prj,"kousa")||
	 !strcmp(prj,"rokunohe")  || !strcmp(prj,"osaka")||
	 !strcmp(prj,"kuma")  || !strcmp(prj,"toyono")||
	 !strcmp(prj,"kodaira")  || !strcmp(prj,"osakiso")||
	 !strcmp(prj,"tanno")    || !strcmp(prj,"Mnl")||
	 !strcmp(prj,"okumura")  || !strcmp(prj,"funakoshi")||
	 !strcmp(prj,"suma")     || !strcmp(prj,"kamijujo")||
	 !strcmp(prj,"abiko")    || !strcmp(prj,"npuae")||
	 !strcmp(prj,"shirakawa")|| !strcmp(prj,"pkangh")||
	 !strcmp(prj,"taichung") || !strcmp(prj,"ishiro")||
	 !strcmp(prj,"hashi")    || !strcmp(prj,"nishijin")||
	 !strcmp(prj,"ichikawa") || !strcmp(prj,"tondabayashi"))
  {
  ...
  for(is=0;is<nsect;is++)
  {
	...
	qlrate[is]=0.0; /***araki for safety long,short ***/
	qsrate[is]=0.0;
	mlrate[is]=0.0;
	msrate[is]=0.0;
  }
...
/*else if(!strcmp(prj,"kato"))
{
}*/
/*else if(!strcmp(prj,"mita"))
{
}*/
/*else if(!strcmp(prj,"misato"))
{
}*/
/*else if(!strcmp(prj,"yamaga"))
{
}*/
if(!strcmp(prj,"venizu"))
{
}
/*if(!strcmp(prj,"ureshi"))
{
}*/
/*if(!strcmp(prj,"ttbpergola"))
{
  if(elem.sect->code==203 || elem.sect->code==213)
  {
  }
}*/
/*if(!strcmp(prj,"fukuda"))
{
}*/
/*if(!strcmp(prj,"mugi"))
{
...
////////////////////////////////////////////////////////////////TAKATA FOR VENE/
          ...
          /*LONG...................................................*/
          /*MATERIAL FOR LONG*/
          /*** 2011.07.25 Program ‹–—e’l‚ÌŽg—p—L–³ƒ`ƒFƒbƒN ***/
		  ...
		  /*** araki for tsurukawa ********************************************/
          if(!strcmp(prj,"turu")) gmaterial.pcF=240.0;   /*PC CONCRETE Fc240*/
          /********************************************************************/
		  gmaterial.pcfc=gmaterial.pcF/3.0;         /*+:COMPRESSION*/
		  if(gmaterial.pcfc>210.0) gmaterial.pcfc=210.0;
		  gmaterial.pcft=0.0;
          /*** araki for tsurukawa ********************************************/
		  if(!strcmp(prj,"turu")) gmaterial.pcft=0.07*gmaterial.pcF;   /*PRC*/
          /********************************************************************/
...
if(!strcmp(prj,"izu") && (sect1.code==901 ||
			...
			if(rate>rateml) rateml=rate;/*araki for safety long */
...
if(!strcmp(prj,"izu") && (sect1.code==901 ||
			...
			/*** 2014.06.03 araki for safety long ********************/
			if(rate>rateml) rateml=rate;
			if(rate>mlrate[soffset-1]) mlrate[soffset-1]=rate;
			rateql=rateml;
          ...
          else
          {
            ...
            for(ia=0;ia<=na;ia++) /*FOR AXIS*/
            {
			  ...
			  /*Ma=0.0;*/  //Default
			  Mahead=0.0;  //130309 fukushima for RC
			  Matail=0.0;
			  ...
			  if(sect1.stype==STYPE_S)                            /*S*/
              {
				/*Ma=allowablebendingofsteel(PLONG,sect1,gmaterial.sE,
										   h,ia,(-lN)); */ //Default
                /*Ma=allowablebendingofsteel(PLONG,sect1,gmaterial.sE,
										   h,h,ia,(-lN));*/       //LkSatobymihara
				Mahead=allowablebendingofsteel(PLONG,sect1,gmaterial.sE,    //LkSatobymihara
											   h,ia,(-lN));          /*!!!cf) 130309_Mapatch*/
				Matail=Mahead;

              }
              else if(sect1.stype==STYPE_RC)                     /*RC*/
			  {
				/*Ma=allowablebendingofrc(elem,gmaterial,ia,(-lN),PLONG);*/  //Default
				...
				if(face[1][ia]*pstress[1]->M[ia]<=0.0) // 160301 fix sign judgement in case Mj==0.0 by fukushima
				{
              ...
              else if(sect1.stype==STYPE_SRC)                   /*SRC*/
			  {
                ...
                {
				  /*Ma=allowablebendingofsrc(elem,gmaterial,ia,(-lN),PLONG); */  //Default
				  Mahead=allowablebendingofsrc(elem,gmaterial,ia,(-lN),PLONG);
				  Matail=Mahead;
                }
			  }
			  else if(sect1.stype==STYPE_PC)                     /*PC*/
              {
                if(sect1.etype==COLUMN)
                {
				  /*Ma=allowablebendingofpccolumn(elem,gmaterial,ia,lN);*/   //Default
				  /* Mahead=allowablebendingofpccolumn(elem,gmaterial,ia,lN); */
				  /* Matail=Mahead; */
				  /* 150416 fukushima for PC */
				  if(face[0][ia]*pstress[0]->M[ia]>=0.0)
				  {
				    Mahead=allowablebendingofpccolumn(elem,gmaterial,ia,lN,0);
				  } else {
				    Mahead=allowablebendingofpccolumn(elem,gmaterial,ia,lN,1);
				  }
				  if(face[1][ia]*pstress[1]->M[ia]<=0.0) // 160301 fix sign judgement in case Mj==0.0 by fukushima
				  {
				    Matail=allowablebendingofpccolumn(elem,gmaterial,ia,lN,0);
				  } else {
				    Matail=allowablebendingofpccolumn(elem,gmaterial,ia,lN,1);
				  }
				  /***/
				}
                else if(sect1.etype==GIRDER || sect1.etype==BEAM)
                {
				  /*Ma=allowablebendingofpcgirder(elem,gmaterial,ia);*/   //Default
				  /* Mahead=allowablebendingofpcgirder(elem,gmaterial,ia); */
				  /* Matail=Mahead; */
#if 0
				  /* 150416 fukushima for PC */
				  if(face[0][ia]*pstress[0]->M[ia]>=0.0)
				  {
				    Mahead=allowablebendingofpcgirder(elem,gmaterial,ia,0);
				  } else {
				    Mahead=allowablebendingofpcgirder(elem,gmaterial,ia,1);
				  }
				  if(face[1][ia]*pstress[1]->M[ia]<=0.0) // 160301 fix sign judgement in case Mj==0.0 by fukushima
				  {
				    Matail=allowablebendingofpcgirder(elem,gmaterial,ia,0);
				  } else {
				    Matail=allowablebendingofpcgirder(elem,gmaterial,ia,1);
				  }
				  /***/
#endif
                  /* 160302 fukushima for PC */
				  Mahead=allowablebendingofpcgirder(elem,gmaterial,ia,face[0][ia]*pstress[0]->M[ia]*100000.0);
                  Matail=allowablebendingofpcgirder(elem,gmaterial,ia,-1.0*face[0][ia]*pstress[1]->M[ia]*100000.0);
                  /***/
                }
			  }
			  else if(sect1.stype==STYPE_WOOD)                 /*WOOD*/
              {
				/*Ma=allowablebendingofwood(PLONG,sect1,h,h,ia,(-lN));*/ //LkSato  //Default
				Mahead=allowablebendingofwood(PLONG,sect1,h,h,ia,(-lN)); //LkSato
				Matail=Mahead;
			  }
			  ...
			  /*sprintf(str," %8.3f(%8.2f) %8.3f(%8.2f)",
					  ...
					  Ma/100000.0,si*Ma/100000.0);*/  //Default
			  sprintf(str," %8.3f(%8.2f) %8.3f(%8.2f)",
              ...
              */
			  ...
			  if(rate1>rateml) rateml=rate1;/*araki for safety long */
			  if(rate1>mlrate[soffset-1]) mlrate[soffset-1]=rate1;
			  if(rate2>rateml) rateml=rate2;/*araki for safety long */
              ...
              for(in=HEAD;in<=TAIL;in++) /*FOR END*/
              {
                ...
                if(sect1.stype==STYPE_S)                          /*S*/
                {
                  ...
                  if(!strcmp(prj,"katsu"))
                  {
                    if((elem.sect->code==204 && ia==SY) ||
                       (elem.sect->code==205 && ia==SX))
                       Qa=1621;                                      //A.Bolt 1-M16[kgf] for out-of-plane shear of I
                    else if((elem.sect->code==204 && ia==SX) ||
                            (elem.sect->code==205 && ia==SY))
                       Qa=4502;                                      //A.Bolt 1-M16[kgf] for inplane shear of I
                    else if((elem.sect->code==206))
                       Qa=6123;                                      //A.Bolt 1-M16[kgf] for L or T
                    else if((elem.sect->code==207))
                       Qa=11818;                                      //A.Bolt 2-M16[kgf] for L-B or T-B
                    else
                ...
                {
                  /*** araki for sleeve ***/
                  /*if(!strcmp(prj,"gcdp")) Qa=19925;
                  else*/

                  /*if(!strcmp(prj,"turu"))
                  {
                  if(elem.sect->code==301)      Qa=226487;     //D=800:QaL=226487[kgf],D=500:QaL=280241[kgf]
                  else if(elem.sect->code==302) Qa= 83141;     //D=500:QaL= 83141[kgf]
                  else if(elem.sect->code==304) Qa=190650;     //D=800:QaL=190650[kgf],D=500:QaL=244405[kgf]
                  else if(elem.sect->code==306) Qa=156174;     //D=800:QaL=156174[kgf],D=500:QaL=189075[kgf]
                  else if(elem.sect->code==307) Qa= 45676;     //D=500:QaL= 45676[kgf]
                  else if(elem.sect->code==401
                        ||elem.sect->code==402
                        ||elem.sect->code==403) Qa= 24169;     //D=250:QaL= 24169[kgf]
                  else if(elem.sect->code==404) Qa= 54192;     //D=250:QaL= 54192[kgf]
                  else if(elem.sect->code==406) Qa= 87594;     //D=250:QaL= 87594[kgf]
                  else if(elem.sect->code==407) Qa= 16663;     //D=200:QaL= 16663[kgf]
                  else if(elem.sect->code==408) Qa=  9529;     //D=200:QaL=  9529[kgf]
                  else if(elem.sect->code==414) Qa=158670;     //D=250:QaL=158670[kgf]
                  else if(elem.sect->code==415) Qa= 32425;     //D=250:QaL= 32425[kgf]
                  else
                  ...
                  /*if(!strcmp(prj,"misato"))
                  {
                  if(elem.sect->code==301)      Qa= 33473;     //D=600:QaL=33473[kgf]
                  else if(elem.sect->code==302) Qa= 24426;     //D=600:QaL=24426[kgf]
                  else if(elem.sect->code==303) Qa= 21250;     //D=600:QaL=21250[kgf]
                  else
                  Qa=allowableshearofrclong(elem,ia,
                                            pstress[in]->Q[1-ia]
                                            *1000.0,
                  ...
                  /*if(!strcmp(prj,"yamaga"))
                  {
                  if(elem.sect->code==301)      Qa= 4745;     //D=200:QaL=4745[kgf]
                  else if(elem.sect->code==302
                       || elem.sect->code==312) Qa= 8690;     //D=200:QaL=8690[kgf]
                  else if(elem.sect->code==303
                       || elem.sect->code==313) Qa= 8690;     //D=200:QaL=8690[kgf]
                  else if(elem.sect->code==304
                       || elem.sect->code==314) Qa= 5234;     //D=200:QaL=8690[kgf]
                  else if(elem.sect->code==305) Qa= 7511;     //D=200:QaL= 7511[kgf]
                  else if(elem.sect->code==401
                       || elem.sect->code==412) Qa= 3157;     //D=250:QaL= 24169[kgf]
                  else if(elem.sect->code==406
                       || elem.sect->code==407) Qa= 959;     //D=250:QaL= 54192[kgf]
                  else if(elem.sect->code==408) Qa= 5206;     //D=250:QaL= 24169[kgf]
                  else if(elem.sect->code==409
                       || elem.sect->code==413) Qa= 4745;     //D=250:QaL= 24169[kgf]
                  else if(elem.sect->code==410) Qa=11081;     //D=200:QaL= 7511[kgf]
                  else
                  Qa=allowableshearofrclong(elem,ia,
                                            pstress[in]->Q[1-ia]
                                            *1000.0,
               ...
               elem.cmqcode!=0)
            {
              ...
              else
              {
				/*Ma=allowablebendingofpcgirderonmid(elem,gmaterial,
												   h0[SX],0);*/  //Default
				Mahead=allowablebendingofpcgirderonmid(elem,gmaterial,
														 h0[SX],0);
				Matail=Mahead;
				/*if(Ma<0.1) Ma=0.1;*/

				/*if(Ma>0.0) rate=fabs(Mm*100000.0/Ma);*/    //Default
				if(Mahead>0.0) rate=fabs(Mm*100000.0/Mahead);
                else       rate=10.0;
...
////////////////////////////////////////////////////////////////TAKATA FOR VENE/
          ...
          for(hoko=0;hoko<=1;hoko++) /*LOAD DIRECTION X,Y.*/
          {
			...
			if(!strcmp(prj,"timber")) break;  /*gcdp_timber : SKIP SHORT.*/
			if(!strcmp(prj,"chair"))  break;  /*chair : SKIP SHORT.*/
			if(!strcmp(prj,"vict"))   break;  /*vict  : SKIP SHORT.*/
            /*if(!strcmp(prj,"misato")) break;*/  /*misato  : SKIP SHORT.*/
            /*if(!strcmp(prj,"yamaga")) break;*/  /*yamaga  : SKIP SHORT.*/
            /*if(!strcmp(prj,"lacie"))  break;*/  /*lacie : SKIP SHORT.*/
            /*if(!strcmp(prj,"hakushima")) break;*/  /*hakushima  : SKIP SHORT.*/
            /*if(!strcmp(prj,"ureshi")) break;*/  /*ureshi: SKIP SHORT.*/
            if(!strcmp(prj,"hakutruss")) break;  /*hakutruss  : SKIP SHORT.*/
            /*if(!strcmp(prj,"gate")) break;*/       /*gate  : SKIP SHORT.*/
            /*if(!strcmp(prj,"sunny")) break;*/  /*sunny  : SKIP SHORT.*/
            /*if(!strcmp(prj,"ttb")) break;*/  /*ttblattice  : SKIP SHORT.*/
			/*if(!strcmp(prj,"rikuzen")) break;*/  /*rikuzen  : SKIP SHORT.*/
            if(!strcmp(prj,"gialai"))   break;  /*gialai  : SKIP SHORT.*/
...
if(!strcmp(prj,"yamaga"))
{
  if(/*sect1.code==511 || sect1.code==512 || sect1.code==513 || sect1.code==516 || */sect1.code==611)
  {
    nfact=1.0;
    mfact=1.0;
    qfact=1.0;
...
if(!strcmp(prj,"katsu") && (sect1.code==204 ||
                            sect1.code==205 ||
                            sect1.code==206 ||
                            sect1.code==207))
{
...
/*if(!strcmp(prj,"mejirodai"))
{
  nfact=1.0;
  mfact=1.0;
  qfact=1.5;
}*/
if(!strcmp(prj,"west") || !strcmp(prj,"east"))
{
  if((200<sect1.code && sect1.code<300) || (500<sect1.code && sect1.code<600))
  {
    nfact=1.0;
    mfact=1.0;
    qfact=1.0;
...
if(!strcmp(prj,"taichung"))
{
  nfact=1.0;
  mfact=1.0;
  qfact=1.5;
}
if(!strcmp(prj,"ishiro"))
{
  if((200<sect1.code && sect1.code<300) || (500<sect1.code && sect1.code<600))
  {
	nfact=1.0;
	...
	qfact=1.3;
            ...
            for(sign=0;sign<=1;sign++) /*LOAD POSITIVE,NEGATIVE.*/
            {
                 ...
                 && sect1.sform.type==STEEL_PLATE) /*FB*/
              {
				...
				if(rate>ratems) ratems=rate;/*araki for safety short */
				if(rate>msrate[soffset-1]) msrate[soffset-1]=rate;
				...
				/*** 2014.06.03 araki for safety short ********************/
				if(rate>ratems) ratems=rate;
				if(rate>msrate[soffset-1]) msrate[soffset-1]=rate;
				rateqs=ratems;
				qsrate[soffset-1]=msrate[soffset-1];
				/***/
              ...
              else
              {
                ...
                for(ia=0;ia<=na;ia++) /*FOR AXIS*/
                {
                  ...
                  sM[TAIL][ia]=(elem.tail.z.M[ia]
                               ...
                               *100000.0;
                  /*** araki for misato ***/
                  /*if(!strcmp(prj,"misato") && sect1.code==204 && ia==SX)
                    sM[HEAD][ia]=1.615*100000.0;
                  else if(!strcmp(prj,"misato") && sect1.code==214 && ia==SY)
                    sM[HEAD][ia]=1.615*100000.0;*/
                  /***/
                  /*if(Ma<0.1) Ma=0.1;*/
                  ...
                  if(sect1.stype==STYPE_S)                        /*S*/
                  {
					/*Ma=allowablebendingofsteel(PSHORT,sect1,gmaterial.sE,
											   h,ia,(-sN));*/           //Default
					/*Ma=allowablebendingofsteel(PSHORT,sect1,gmaterial.sE,
											   h,h,ia,(-sN));*/   //LkSatobymihara
					Mahead=allowablebendingofsteel(PSHORT,sect1,gmaterial.sE,
											   h,ia,(-sN));      /*!!!cf) 130309_Mapatch*/
					Matail=Mahead;
                  }
                  else if(sect1.stype==STYPE_RC)                 /*RC*/
                  {
					/*Ma=allowablebendingofrc(elem,gmaterial,ia,(-sN),PSHORT);*/ //Default
					if(sM[HEAD][ia]>=0.0)
					{
					...
					if(sM[TAIL][ia]<=0.0) // 160301 fix sign judgement in case Mj==0.0 by fukushima
					{
                  ...
                  else if(sect1.stype==STYPE_SRC)               /*SRC*/
                  {
                    if(sect1.sform.type==STEEL_RECTS)
                    {
					  /*Ma=allowablebendingofsrc(elem,gmaterial,ia,(-sN),PSHORT);*/ //Default
					  Mahead=allowablebendingofsrc(elem,gmaterial,ia,(-sN),PSHORT);
					  Matail=Mahead;
					}
                  }
                  else if(sect1.stype==STYPE_WOOD)             /*WOOD*/
                  {
					/*Ma=allowablebendingofwood(PSHORT,sect1,h,h,ia,(-sN));*/  //LkSato  //Default
					Mahead=allowablebendingofwood(PSHORT,sect1,h,h,ia,(-sN));  //LkSato
					Matail=Mahead;
				  }
                  else if(sect1.stype==STYPE_ALUMI)           /*ALUMI*/
                  {
                  ...
                  */
				  ...
				  if(rate1>ratems) ratems=rate1;/*araki for safety short */
				  if(rate1>msrate[soffset-1]) msrate[soffset-1]=rate1;
				  if(rate2>ratems) ratems=rate2;/*araki for safety short */
				  if(rate2>msrate[soffset-1]) msrate[soffset-1]=rate2;
                  ...
                  for(in=HEAD;in<=TAIL;in++) /*FOR END*/
                  {
                    ...
                    if(sect1.stype==STYPE_S)                      /*S*/
                    {
                      ...
                      if(!strcmp(prj,"katsu"))
                      {
                        if((elem.sect->code==204 && ia==SY) ||
                           (elem.sect->code==205 && ia==SX))
                           Qa=3242;                                  //A.Bolt 1-M16[kgf] for out-of-plane shear of I
                        else if((elem.sect->code==204 && ia==SX) ||
                                (elem.sect->code==205 && ia==SY))
						   Qa=6484;                                  //A.Bolt 1-M16[kgf] for inplane shear of I
                        else if((elem.sect->code==206))
                           Qa=9994;                                  //A.Bolt 1-M16[kgf] for L,T
                        else if((elem.sect->code==207))
                           Qa=19134;                                  //A.Bolt 2-M16[kgf] for L-B,T-B
                        else
                           Qa=allowultimshearofsteel(PSHORT,gmaterial.sF,sect1,ia);
                      }
                      else
                    ...
                    {
                    //////////////////////////////////////////////////////////// 2007.11.3 TAKATA
                      if(sQ[HEAD][1-ia]>sQ[HEAD][1-ia]) sQrc=sQ[HEAD][1-ia];
                      else                              sQrc=sQ[TAIL][1-ia];

                      if(sM[HEAD][ia]>sM[HEAD][ia]) sMrc=sM[HEAD][ia];
                      else                          sMrc=sM[TAIL][ia];
                    ////////////////////////////////////////////////////////////
                      /*** araki for gcdp ***/
                      /*if(!strcmp(prj,"gcdp")) Qa=29887;
                      else*/
                      /***/
					...
					/*** 2014.06.03 araki for safety short ********************/
					if(rate>rateqs) rateqs=rate;
					if(rate>qsrate[soffset-1])
					{
					  qsrate[soffset-1]=rate;
					}
					/***/
...
if(calc[PULTIMATE]==1 || sect1.stype==STYPE_PC)                                    //Default, 2015.05.28 araki for mjdrg
//if(calc[PULTIMATE]==1 && sect1.stype==STYPE_PC)                                  //2007.8.31 TAKATA
{
            ...
            for(sign=0;sign<=1;sign++) /*LOAD POSITIVE,NEGATIVE.*/
            {
              ...
              for(ia=0;ia<=na;ia++) /*FOR AXIS*/
              {
				...
				uM[TAIL][ia]=(elem.tail.z.M[ia]
							 ...
							 *100000.0;
				/*** 16.05.12 araki for PC route 3a ***/
				if(elem.sect->etype==COLUMN && sect1.stype==STYPE_PC)
				{
				  uQpc[HEAD][1-ia]=(elem.head.z.Q[1-ia]
									+dsign
									*(Fes*2.5)*(pstress[HEAD]->Q[1-ia]))
								   *1000.0;
				  uQpc[TAIL][1-ia]=(elem.tail.z.Q[1-ia]
									+dsign
									*(Fes*2.5)*(pstress[TAIL]->Q[1-ia]))
								   *1000.0;
				  /*uN=1000.0*(elem.head.z.N
						   +dsign*(Fes*2.5)*(pstress[HEAD]->N)); */
				}
				else if(elem.sect->etype==GIRDER && sect1.stype==STYPE_PC)
				{
				  uQpc[HEAD][1-ia]=(elem.head.z.Q[1-ia]
									+dsign
									*(Fes*2.25)*(pstress[HEAD]->Q[1-ia]))
								   *1000.0;
				  uQpc[TAIL][1-ia]=(elem.tail.z.Q[1-ia]
									+dsign
									*(Fes*2.25)*(pstress[TAIL]->Q[1-ia]))
								   *1000.0;
				  /*uN=1000.0*(elem.head.z.N
						   +dsign*(Fes*2.5)*(pstress[HEAD]->N));*/
				}
				/*else if(elem.sect->etype==WALL && sect1.stype==STYPE_PC)
				{
				  uNpc=1000.0*(elem.head.z.N
							   +dsign*(Fes*2.5)*(pstress[HEAD]->N));
				}*/
				/******************************************************/
                if(ia==0)
				...
				{
				  sprintf(str," %8.3f(%8.2f) %8.3f(%8.2f)",
						  uQpc[HEAD][1-ia]/1000.0,si*uQpc[HEAD][1-ia]/1000.0,
						  uQpc[TAIL][1-ia]/1000.0,si*uQpc[TAIL][1-ia]/1000.0);
				  strcat(strQ[0][1-ia],str);
				}
				/***/
				else
...
*/
                  ...
                  Mu=ultimatebendingofsrc(elem,ia,(-uN),
                                          &Ns,&Ms,&Nr,&Mr,&Nc,&Mc);
                  */
				  /* 150605 fukushima for ultimatebendingofsrc*/
                  if(uM[HEAD][ia]>=0.0)
                  {
                    Muhead=ultimatebendingofsrc(elem,ia,(-uN),
                                                &Ns,&Ms,&Nr,&Mr,&Nc,&Mc,0);
                  }
                  else
                  {
                  ...
                  if(uM[TAIL][ia]>=0.0)
                  {
                    Mutail=ultimatebendingofsrc(elem,ia,(-uN),
                                                &Ns,&Ms,&Nr,&Mr,&Nc,&Mc,1);
                  }
                  else
                  {
...
/*
  fprintf(fout0,"Surface %d %15.3f %15.3f\n",ii,uN,Mu);
                ...
                else if(sect1.stype==STYPE_PC)                 /*PC*/
                {
                  ...
                  if(sect1.etype==GIRDER)
				  {
					...
					Muhead=ultimatebendingofpcgirder(elem,gmaterial,ia,
													 tensedside,uN);   /*** 16.05.15 araki for PC ***/
                ...
                if(sect1.stype==STYPE_RC || sect1.stype==STYPE_SRC || // 150605 fukushima for ultimatebendingofsrc
                   (sect1.stype==STYPE_PC && sect1.etype==GIRDER))
                {
                ...
                for(in=HEAD;in<=TAIL;in++) /*FOR END*/
                {
				  ...
				  /*** 16.05.12 araki for PC route 3a ***/
				  if(sect1.stype==STYPE_PC) rate=fabs(uQpc[in][1-ia]/Qu);
				  else
				  /***/
...
if(in==TAIL)
{
  uQe[TAIL][1-ia]=(elem.tail.z.Q[1-ia]
		  ...
		  /*** 2014.06.03 araki for safety long,short ********************/
		  fprintf(fout0," Ql/Qal=%.5f Qs/Qas=%.5f",rateql,rateqs);
		  /***/
		  ...
		  /*** 2014.06.03 araki for safety long,short ********************/
		  fprintf(fout0," Ml/Mal=%.5f Ms/Mas=%.5f",rateml,ratems);
		  /***/
          if(calc[PULTIMATE]==1 || sect1.stype==STYPE_PC)
          {
		  ...
		  if(frate!=NULL)
		  {
			fprintf(frate,"ELEM: %5d SECT: %5d  %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f\n",
					ielem,isect,rateqa,ratequ,ratema,ratemu,rateql,rateqs,rateml,ratems);
		  }
          /***/
          /*if(check){gets(non); if(non[0]!='\0') return 0;}*/
        }
...
if(!strcmp(prj,"Ginza"))
{
          fprintf(fout0,"“à–@:Lo=%.1f[cm](’Œ’†SŠÔ‹——£:L=%.1f[cm]) H=%.1f[cm] ",l0,l,h0[1]);
}
else
{
          ...
          else
          {
            ...
            wrate2=sqrt((elem.sect->wlength)*(elem.sect->wheight)
                        /(l0*h0[1]));
            wrate3=(elem.sect->wheight)/h0[1];        // 2007.6.12 TAKATA
            #endif
          ...
          {
            if(wrate2>=wrate3) elem.sect->windowrate=wrate2;
            else elem.sect->windowrate=wrate3;
          }
          ...
          if(sect1.stype==STYPE_RC)
          {
            Qa=allowultimshearofrcwall(elem,l/2,l0,PLONG);  //130611 mihara
          }
...
{
//  l0=l;
  Qu=allowultimshearofrcwall(elem,l/2,l0,PULTIMATE);    //130611 mihara
}
else
{
  Qa=allowultimshearofrcwall(elem,l/2,l0,PSHORT);       //130611 mihara
  Qu=allowultimshearofrcwall(elem,l/2,l0,PULTIMATE);    //130611 mihara
}
          }
          if(sect1.stype==STYPE_WOOD)
...
/*if(!strcmp(prj,"yoko")) wafact=1.0;*/

/*if(!strcmp(prj,"gcdp")) wafact=1.0;   */
...
if(!strcmp(prj,"mita")&& sect1.etype==WALL)
{
  wafact=2.0;
}
...
if(!strcmp(prj,"boma")&& sect1.stype==STYPE_WOOD)
{
  wafact=1.0;
}
if(!strcmp(prj,"ureshitok"))
{
  wafact=1.0;
}
if(!strcmp(prj,"rikuzenwhole"))
...
if(!strncmp(prj,"west",4) || !strncmp(prj,"east",4))
{
  if(sect1.code>=901)
  {
  wafact=1.0;
...
if(!strcmp(prj,"tskr")    || !strcmp(prj,"obihiro")  ||
   !strcmp(prj,"housho")  || !strcmp(prj,"aguri")    ||
   !strcmp(prj,"fkym")    || !strcmp(prj,"kousa")    ||
   !strcmp(prj,"rokunohe")|| !strcmp(prj,"toyono")   ||
   !strcmp(prj,"kodaira") || !strcmp(prj,"tanno")    ||
   !strcmp(prj,"okumura") || !strcmp(prj,"funakoshi")||
   !strcmp(prj,"suma")    || !strcmp(prj,"kamijujo") ||
   !strcmp(prj,"abiko")   || !strcmp(prj,"shirakawa")||
   !strcmp(prj,"pkangh")  || !strcmp(prj,"ishiro")||
   !strcmp(prj,"hashi")   || !strcmp(prj,"ichikawa"))
{
  if(sect1.stype==STYPE_WOOD)  wafact=1.0;
  else                         wafact=2.0;
}
...
if(!strcmp(prj,"nishijin"))
{
  if(hoko==0) wafact=1.0;
  else if(hoko==1) wafact=1.3;
}
...
//  fprintf(fout0," Qs/Qa=%7.5f",rate);
  ...
  if(el==1)
  {
    xarate=(xarate1+xarate2)/2.0;
    yarate=(yarate1+yarate2)/2.0;
  }
}
else
{
*/
  ...
  /*** 2014.06.03 araki for safety short ********************/
  if(rate>rateqs) rateqs=rate;
  if(rate>qsrate[soffset-1]) qsrate[soffset-1]=rate;
  /***/
  fprintf(fout0," Qs/Qa=%7.5f",rate);
...
if(!strcmp(prj,"Ginza"))
{
  ...
  if(el==1)
  {
    xurate=(xurate1+xurate2)/2.0;
    yurate=(yurate1+yurate2)/2.0;
  }
}
*/
            }
...
if(!strcmp(prj,"Ginza"))
{
  if(el==1)
  {
    if(xurate>1.0 || yurate>1.0)
    {
      fprintf(fsafe,"NG ELEM:%5d,%5d SECT:%4d ",ielem1,ielem2,isect);
      fprintf(fsafe,"MAX:Qx/Qu=%.5f Qy/Qu=%.5f\n",xurate,yurate);
    }
  }
...
//}
		  ...
		  if(frate!=NULL)
          {
			fprintf(frate,"ELEM: %5d SECT: %5d  %.5f %.5f 0.00000 0.00000 %.5f %.5f 0.00000 0.00000\n",
					ielem,isect,rateqa,ratequ,rateql,rateqs);
		  }
		  /***/
		}
		...
		if(soffset && sect1.etype==SLAB)/*...........SHEAR OF SLAB.*/
        {
          ...
          fprintf(fout0,"°Œú:%.2f[mm] ",sect1.thick*10.0);                 //araki for thick
...
if(!strcmp(prj,"mita")&&sect1.etype==SLAB)
{
  wafact=1.0;
}
if(!strcmp(prj,"yamaga"))
            ...
            fprintf(fout0,"Qs=%7.3f[tf](%7.2f[kN])",
                   /*    (lQ+wafact*sQ[0][0])/1000.0,
                    si*(lQ+wafact*sQ[0][0])/1000.0);*/
					   (fabs(lQ)+fabs(wafact*sQ[0][0]))/1000.0,                 /*** araki for RC Slab ***/
                    si*(fabs(lQ)+fabs(wafact*sQ[0][0]))/1000.0);
            ...
            fprintf(fout0,"Qu=%7.3f[tf](%7.2f[kN])",
                   /*    (lQ+wufact*sQ[0][0])/1000.0,
                    si*(lQ+wufact*sQ[0][0])/1000.0);  */
                       (fabs(lQ)+fabs(wufact*sQ[0][0]))/1000.0,                 /*** araki for RC Slab ***/
                    si*(fabs(lQ)+fabs(wufact*sQ[0][0]))/1000.0);
}
...
if(calc[PULTIMATE]==1)
{
  ...
  fprintf(fout0,"=====================================");
  fprintf(fout0,"=================\n");
  fprintf(fout0,"Še’f–ÊŽí•Ê‚Ì‹–—e,I‹Ç‹È‚°ˆÀ‘S—¦‚ÌÅ‘å’l\n\n");

  rateqa=0.0; ratequ=0.0;
  ratema=0.0; ratemu=0.0;

  rateql=0.0; rateqs=0.0;/*araki for safety long,short */
  rateml=0.0; ratems=0.0;
  ...
  for(is=1;is<=nsect;is++)
  {
    ...
    if(soffset)
    {
         ...
         sect1.etype!=BRACE)
      {
		...
		/*** araki for safe ***/
		fprintf(fout0," Ql/Qal=%7.5f Qs/Qas=%7.5f",qlrate[soffset-1],qsrate[soffset-1]);
		/***/
        if(calc[PULTIMATE]==1 || sect1.stype==STYPE_PC)
		{
		...
		/*** araki for safe ***/
		fprintf(fout0," Ml/Mal=%7.5f Ms/Mas=%7.5f",mlrate[soffset-1],msrate[soffset-1]);
		/***/
        if(calc[PULTIMATE]==1 || sect1.stype==STYPE_PC)
		{
      ...
      if(sect1.etype==BRACE)
      {
		...
		/*** araki for safe ***/
		fprintf(fout0," Nl/Nal=%7.5f Ns/Nas=%7.5f",qlrate[soffset-1],qsrate[soffset-1]);
		/***/
      ...
      if(sect1.etype==SLAB)
      {
        fprintf(fout0," t=%4.2f[cm]",sect1.thick);                              //araki for thick
	  ...
	  /*araki for safety long,short */
	  if(qlrate[soffset-1]>rateql) rateql=qlrate[soffset-1];
	  if(qsrate[soffset-1]>rateqs) rateqs=qsrate[soffset-1];
	  if(mlrate[soffset-1]>rateml) rateml=mlrate[soffset-1];
	  if(msrate[soffset-1]>ratems) ratems=msrate[soffset-1];
	  /***/
	}
  }

  sprintf(txt,"ˆÀ‘S—¦‚ÌÅ‘å’l");
  sprintf(non," Q/Qa=%7.5f Q/Qu=%7.5f",rateqa,ratequ);
  /*** araki for safe ***/
  sprintf(non," Ql/Qal=%7.5f Qs/Qas=%7.5f",rateql,rateqs);
  /***/
  strcat(txt,non);
  sprintf(non," M/Ma=%7.5f M/Mu=%7.5f",ratema,ratemu);
  /*** araki for safe ***/
  sprintf(non," Ml/Mal=%7.5f Ms/Mas=%7.5f",rateml,ratems);
  /***/
  strcat(txt,non);
  fprintf(fout0,"\n%s\n",txt);
  ...
  if(globalmessageflag==1) MessageBox(NULL,txt,"SRCan",MB_OK);
  return 1;
}/*srcan001*/

int srcan002(char fname[])
{
/*SRCan for Steel in 5 Directions*/
  int ii,jj,ie,is,ic,ia,in,na,n1,n2,n3,n4,n5,ielem,isect,ni,nj;                 //araki for posi nega
  int hoko;                 /*DIRECTION OF HORIZONTAL LOAD 0:X 1:Y.*/
  int sign=0;      /*SIGN OF HORIZONTAL LOAD 0:POSITIVE 1:NEGATIVE.*/           //araki for posi nega
  int calc[3];              /*CALCULATION FLAG LONG,SHORT,ULTIMATE.*/
  double dsign;
  double lN,lQ,eN; /*LONG,EARTHQUAKE STRESS.*/
  double sN,sQ[2][2],sMt,sM[2][2];  /*s:SHORT [END][AXIS]*/
  ...
  FILE *fin,*fcmq=NULL,*flist,*fsafe,*frate;
  FILE *fz=NULL,*fxp=NULL,*fyp=NULL,*fxn=NULL,*fyn=NULL;                        //araki for posi nega
  char txt[400],non[80],str[256];
  char strhoko[2][10]={"X","Y"};
  /*char straxis[2][10]={"x","y"};*/
  /*char strend[2][10]={"i","j"};*/
  ...
  double si=9.80665;                                    /*SI UNITS.*/
  struct element elem,pair;
  struct elementpn elempn;                                                    //araki for posi nega
  struct stress *pstress[2],estress; /*HEAD,TAIL*/

  struct structnode *nodes;
  struct element *elems;
  struct elementpn *elemspn;                                                        //araki for posi nega
  struct section *sects;
  ...
  GetDlgItemText((wmenu.childs+2)->hwnd,ID_SECTIONFILE,non,80);
  if(!strncmp(non,"mjdrg",5)) strcpy(prj,"mjdrg");
  else if(!strncmp(non,"zoo",    3)) strcpy(prj,"zoo");
  ...
  flist=fgetstofopen(dir,"r",ID_SECTIONFILE);
  fz   =fgetstofopen("\0","r",ID_OUTPUTFILEZ);
  fxp  =fgetstofopen("\0","r",ID_INPUTFILEX);
  fyp  =fgetstofopen("\0","r",ID_INPUTFILEY);
  fxn  =fgetstofopen("\0","r",ID_OUTPUTFILEX);
  fyn  =fgetstofopen("\0","r",ID_OUTPUTFILEY);
  if(fin==NULL)   return 0;
  if(flist==NULL) return 0;
  if(fz==NULL)    return 0;
  if(fxp==NULL)   return 0;                                                     //araki for posi nega
  if(fyp==NULL)   return 0;
  if(fxn==NULL)   return 0;
  if(fyn==NULL)   return 0;
  ...
  if(!strcmp(prj,"mjdrg")|| !strcmp(prj,"zoo"))
  {
	fgetinitial2(fin,&nnode,&nelem,&nsect);   /*INPUT ARCLM INITIAL.*/
  }
  else
  ...
  elems=(struct element *)malloc(nelem*sizeof(struct element));
  if(elems==NULL) return 0;
  elemspn=(struct elementpn *)malloc(nelem*sizeof(struct elementpn));        //araki for posi nega
  if(elemspn==NULL) return 0;

  if(!strcmp(prj,"mjdrg")|| !strcmp(prj,"zoo"))
  {
	inputfiletomemory2(fin,&nnode,&nelem,&nsect,sects,nodes,elems);
	inputfiletomemory3(fin,&nnode,&nelem,&nsect,sects,nodes,elemspn);          //araki for posi nega
  }
  else
  {
    inputfiletomemory(fin,&nnode,&nelem,&nsect,sects,nodes,elems);
...
*/
  ...
  while(1) /*CALCULATION*/
  {
    ...
    elem=*(elems+ie);            /*ALL ELEMENTS POINTING EACH SECT.*/
	elempn=*(elemspn+ie);                                                        //araki for posi nega

	if(slabcount==0 && ie<nelem-1) pair=*(elems+ie+1);
	if(slabcount==1)               pair=*(elems+ie-1);

	initializestress(&(elempn.head.xp));          /*INITIALIZE STRESS*/           //araki for posi nega
	initializestress(&(elempn.tail.xp));
	initializestress(&(elempn.head.yp));
	initializestress(&(elempn.tail.yp));
	initializestress(&(elempn.head.xn));
	initializestress(&(elempn.tail.xn));
	initializestress(&(elempn.head.yn));
	initializestress(&(elempn.tail.yn));
	initializestress(&(elempn.head.z));
	initializestress(&(elempn.tail.z));

	n1=fgetstress(fz,&ielem,&isect,&ni,&(elempn.head.z));
	n2=fgetstress(fxp,&ielem,&isect,&ni,&(elempn.head.xp));                       //araki for posi nega
	n3=fgetstress(fyp,&ielem,&isect,&ni,&(elempn.head.yp));
    n4=fgetstress(fxn,&ielem,&isect,&ni,&(elempn.head.xn));
	n5=fgetstress(fyn,&ielem,&isect,&ni,&(elempn.head.yn));

	if(n1==0 || n2==0 || n3==0 || n4==0 || n5==0) break; /*END OF FILE.*/       //araki for posi nega

	if(n1==9 && n2==9 && n3==9 && n4==9 && n5==9)                               //araki for posi nega
    {
      if(ielem)
      {
		n1=fgetstress(fz,&ielem,&isect,&nj,&(elempn.tail.z));
		n2=fgetstress(fxp,&ielem,&isect,&nj,&(elempn.tail.xp));                   //araki for posi nega
        n3=fgetstress(fyp,&ielem,&isect,&nj,&(elempn.tail.yp));
        n4=fgetstress(fxn,&ielem,&isect,&nj,&(elempn.tail.xn));
        n5=fgetstress(fyn,&ielem,&isect,&nj,&(elempn.tail.yn));

		if(n1<7 || n2<7 || n3<7 || n4<7 || n5<7) break;                         //araki for posi nega
		else ie++; /*COUNT ELEMENTS.*/
...
if(!strcmp(prj,"mjdrg"))
{
		isect=elem.sect->code;        /*** 2016.12.31 araki for posi nega in mjdrg ***/
}
else
{
		/* elem.sect->code=isect; */ // 150429 fukushima for original section
                   ...
                   && sect1.etype!=BRACE)
        {
		  ...
		  fprintf(fout0,"’f–Ê:%3d",isect);                                      //araki check isect 161202
          if(sect1.stype     ==STYPE_S)    fprintf(fout0,"=‚r");
          else if(sect1.stype==STYPE_RC)   fprintf(fout0,"=‚q‚b");
          else if(sect1.stype==STYPE_SRC)  fprintf(fout0,"=‚r‚q‚b");
          else if(sect1.stype==STYPE_PC)   fprintf(fout0,"=‚o‚b");
     ...
     !strcmp(prj,"kyok"))
  {
    gmaterial.sF=2400.0*jis; /*STEEL SN400A*/
		  ...
		  fprintf(fout0,"‰”’¼ŽžZ    : %8.3f(%8.2f)",                            //araki for posi nega
                  elempn.head.z.N,si*elempn.head.z.N);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.z.Q[0],si*elempn.head.z.Q[0],
                  elempn.tail.z.Q[0],si*elempn.tail.z.Q[0]);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.z.Q[1],si*elempn.head.z.Q[1],
                  elempn.tail.z.Q[1],si*elempn.tail.z.Q[1]);
          fprintf(fout0," %8.3f(%8.2f)",
                  elempn.head.z.Mt,si*elempn.head.z.Mt);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.z.M[0],si*elempn.head.z.M[0],
                  elempn.tail.z.M[0],si*elempn.tail.z.M[0]);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)\n",
                  elempn.head.z.M[1],si*elempn.head.z.M[1],
				  elempn.tail.z.M[1],si*elempn.tail.z.M[1]);

		  fprintf(fout0,"…•½ŽžX³  : %8.3f(%8.2f)",                            //araki for posi nega
                  elempn.head.xp.N,si*elempn.head.xp.N);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.xp.Q[0],si*elempn.head.xp.Q[0],
                  elempn.tail.xp.Q[0],si*elempn.tail.xp.Q[0]);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.xp.Q[1],si*elempn.head.xp.Q[1],
                  elempn.tail.xp.Q[1],si*elempn.tail.xp.Q[1]);
          fprintf(fout0," %8.3f(%8.2f)",
                  elempn.head.xp.Mt,si*elempn.head.xp.Mt);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.xp.M[0],si*elempn.head.xp.M[0],
                  elempn.tail.xp.M[0],si*elempn.tail.xp.M[0]);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)\n",
                  elempn.head.xp.M[1],si*elempn.head.xp.M[1],
                  elempn.tail.xp.M[1],si*elempn.tail.xp.M[1]);

		  fprintf(fout0,"…•½ŽžY³  : %8.3f(%8.2f)",                            //araki for posi nega
                  elempn.head.yp.N,si*elempn.head.yp.N);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.yp.Q[0],si*elempn.head.yp.Q[0],
                  elempn.tail.yp.Q[0],si*elempn.tail.yp.Q[0]);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.yp.Q[1],si*elempn.head.yp.Q[1],
                  elempn.tail.yp.Q[1],si*elempn.tail.yp.Q[1]);
          fprintf(fout0," %8.3f(%8.2f)",
                  elempn.head.yp.Mt,si*elempn.head.yp.Mt);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.yp.M[0],si*elempn.head.yp.M[0],
                  elempn.tail.yp.M[0],si*elempn.tail.yp.M[0]);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)\n",
                  elempn.head.yp.M[1],si*elempn.head.yp.M[1],
                  elempn.tail.yp.M[1],si*elempn.tail.yp.M[1]);

		  fprintf(fout0,"…•½ŽžX•‰  : %8.3f(%8.2f)",                            //araki for posi nega
                  elempn.head.xn.N,si*elempn.head.xn.N);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.xn.Q[0],si*elempn.head.xn.Q[0],
                  elempn.tail.xn.Q[0],si*elempn.tail.xn.Q[0]);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.xn.Q[1],si*elempn.head.xn.Q[1],
                  elempn.tail.xn.Q[1],si*elempn.tail.xn.Q[1]);
          fprintf(fout0," %8.3f(%8.2f)",
                  elempn.head.xn.Mt,si*elempn.head.xn.Mt);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.xn.M[0],si*elempn.head.xn.M[0],
                  elempn.tail.xn.M[0],si*elempn.tail.xn.M[0]);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)\n",
                  elempn.head.xn.M[1],si*elempn.head.xn.M[1],
                  elempn.tail.xn.M[1],si*elempn.tail.xn.M[1]);

		  fprintf(fout0,"…•½ŽžY•‰  : %8.3f(%8.2f)",                            //araki for posi nega
                  elempn.head.yn.N,si*elempn.head.yn.N);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.yn.Q[0],si*elempn.head.yn.Q[0],
                  elempn.tail.yn.Q[0],si*elempn.tail.yn.Q[0]);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.yn.Q[1],si*elempn.head.yn.Q[1],
                  elempn.tail.yn.Q[1],si*elempn.tail.yn.Q[1]);
          fprintf(fout0," %8.3f(%8.2f)",
                  elempn.head.yn.Mt,si*elempn.head.yn.Mt);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.yn.M[0],si*elempn.head.yn.M[0],
                  elempn.tail.yn.M[0],si*elempn.tail.yn.M[0]);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)\n",
                  elempn.head.yn.M[1],si*elempn.head.yn.M[1],
                  elempn.tail.yn.M[1],si*elempn.tail.yn.M[1]);
    ...
    for(ia=0;ia<=na;ia++) /*FOR AXIS*/
    {
      ...
      else
	  {
        face[1][ia]=(fabs(elem.tail.z.M[ia]/elem.tail.z.Q[1-ia])
		  ...
		  fprintf(fout0,"’·Šú       : %8.3f(%8.2f)",                            //araki for posi nega
                  elempn.head.z.N,si*elempn.head.z.N);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                  elempn.head.z.Q[0],si*elempn.head.z.Q[0],
                  elempn.tail.z.Q[0],si*elempn.tail.z.Q[0]);
          ...
          fprintf(fout0," %8.3f(%8.2f)",
                  elempn.head.z.Mt,si*elempn.head.z.Mt);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)",
                     face[0][0]*elempn.head.z.M[0],
                  si*face[0][0]*elempn.head.z.M[0],
                     face[1][0]*elempn.tail.z.M[0],
                  si*face[1][0]*elempn.tail.z.M[0]);
          fprintf(fout0," %8.3f(%8.2f) %8.3f(%8.2f)\n",
                     face[0][1]*elempn.head.z.M[1],
                  si*face[0][1]*elempn.head.z.M[1],
                     face[1][1]*elempn.tail.z.M[1],
				  si*face[1][1]*elempn.tail.z.M[1]);
          ...
          if((sect1.stype==STYPE_S ||
             ...
             && sect1.sform.type==STEEL_PLATE) /*STEEL FB*/
          {
			pstress[0]=&(elempn.head.z); pstress[1]=&(elempn.tail.z);           //araki for posi nega
...
if(!strcmp(prj,"izu") && (sect1.code==901 ||
          ...
          else
          {
            ...
            for(ia=0;ia<=na;ia++) /*FOR AXIS*/
			{
			  ...
			  lN=1000.0*elempn.head.z.N; /*LONG N[kgf].*/                       //araki for posi nega
			  ...
			  else if(sect1.stype==STYPE_PC)                     /*PC*/
              {
                ...
                else if(sect1.etype==GIRDER || sect1.etype==BEAM)
                {
				  ...
				  /***/
#endif
				  /* 160302 fukushima for PC */                                 //araki check 161202
                  Mahead=allowablebendingofpcgirder(elem,gmaterial,ia,face[0][ia]*pstress[0]->M[ia]*100000.0);
                  Matail=allowablebendingofpcgirder(elem,gmaterial,ia,-1.0*face[0][ia]*pstress[1]->M[ia]*100000.0);
                  /***/
                }
            ...
            if(sect1.stype==STYPE_PC &&
               ...
               elem.cmqcode!=0)
			{                                                                   //araki for posi nega
			  Mm=elem.Mo-0.5*(elempn.tail.z.M[0]-elempn.head.z.M[0]);               //2011.05.04 araki
              fprintf(fout0,"        ’†‰›:Mo=%8.3f Mm=%8.3f",
                      elem.Mo,Mm);
              ...
              else
              {
				/*Ma=allowablebendingofpcgirderonmid(elem,gmaterial,
												   h0[SX],0);*/  //Default
				Mahead=allowablebendingofpcgirderonmid(elem,gmaterial,          //araki check 161202
														 h0[SX],0);
				Matail=Mahead;
				/*if(Ma<0.1) Ma=0.1;*/
...
////////////////////////////////////////////////////////////////TAKATA FOR VENE/
          ...
          /*for(hoko=0;hoko<=1;hoko++)*/ /*LOAD DIRECTION X,Y.*/
          for(hoko=0;hoko<=3;hoko++) /*LOAD DIRECTION X POSI,YPOSI,XNEGA,YNEGA.*/ //araki
          {
            if(sect1.stype==STYPE_PC) break;  /*PC ROUTE 3a : SKIP SHORT.*/
if(!strcmp(prj,"mjdrg"))
{
  ...
  qfact=1.5;
            ...
            if(hoko==0)
            {
              pstress[0]=&(elempn.head.xp); pstress[1]=&(elempn.tail.xp);
            }
            if(hoko==1)
            {
              pstress[0]=&(elempn.head.yp); pstress[1]=&(elempn.tail.yp);
            }
            if(hoko==2)
            {
              pstress[0]=&(elempn.head.xn); pstress[1]=&(elempn.tail.xn);
            }
            if(hoko==3)
            {
              pstress[0]=&(elempn.head.yn); pstress[1]=&(elempn.tail.yn);
            }

            /*for(sign=0;sign<=1;sign++)*/ /*LOAD POSITIVE,NEGATIVE.*/          //default
            if(sign==0)                                                         //araki
            {
              for(ii=0;ii<=4;ii++)
              {
              ...
              sprintf(strQ[0][0],"’ZŠú%s%s•ûŒü:",
                      strhoko[hoko%2],strhugo[(hoko-hoko%2)/2]);                           //araki
                 ...
                 && sect1.sform.type==STEEL_PLATE) /*FB*/
              {
                ...
                /*HEAD*/
                estress.N   =elempn.head.z.N
                             +dsign*nfact*(pstress[HEAD]->N);
                estress.Q[0]=elempn.head.z.Q[0]
							 +dsign*qfact*(pstress[HEAD]->Q[0]);
                estress.Q[1]=elempn.head.z.Q[1]
                             +dsign*qfact*(pstress[HEAD]->Q[1]);
                estress.Mt  =elempn.head.z.Mt
                             +dsign*mfact*(pstress[HEAD]->Mt);
                estress.M[0]=elempn.head.z.M[0]
                             +dsign*mfact*(pstress[HEAD]->M[0]);
                estress.M[1]=elempn.head.z.M[1]
							 +dsign*mfact*(pstress[HEAD]->M[1]);
				...
				/*TAIL*/
                estress.N   =elempn.tail.z.N
                             +dsign*nfact*(pstress[TAIL]->N);
                estress.Q[0]=elempn.tail.z.Q[0]
                             +dsign*qfact*(pstress[TAIL]->Q[0]);
                estress.Q[1]=elempn.tail.z.Q[1]
                             +dsign*qfact*(pstress[TAIL]->Q[1]);
                estress.Mt  =elempn.tail.z.Mt
                             +dsign*mfact*(pstress[TAIL]->Mt);
                estress.M[0]=elempn.tail.z.M[0]
                             +dsign*mfact*(pstress[TAIL]->M[0]);
                estress.M[1]=elempn.tail.z.M[1]
                             +dsign*mfact*(pstress[TAIL]->M[1]);
              ...
              else
              {
                ...
                for(ia=0;ia<=na;ia++) /*FOR AXIS*/
                {
                  ...
                  sN=1000.0*(elempn.head.z.N                                   //araki
                             +dsign*nfact*(pstress[HEAD]->N));
                  sQ[HEAD][1-ia]=(elempn.head.z.Q[1-ia]
                                  +dsign
                                  *qfact*(pstress[HEAD]->Q[1-ia]))
                                 *1000.0;
                  sQ[TAIL][1-ia]=(elempn.tail.z.Q[1-ia]
                                  +dsign
								 *qfact*(pstress[TAIL]->Q[1-ia]))
                                 *1000.0;
                  sMt=(elempn.head.z.Mt+dsign*mfact*(pstress[HEAD]->Mt))
                      *100000.0;

                  sM[HEAD][ia]=(elempn.head.z.M[ia]
                                +dsign*facei
                                *mfact*(pstress[HEAD]->M[ia]))
                               *100000.0;
                  sM[TAIL][ia]=(elempn.tail.z.M[ia]
                                +dsign*facej
                                *mfact*(pstress[TAIL]->M[ia]))
							   *100000.0;
                  /*** araki for misato ***/
...
//if(calc[PULTIMATE]==1 || sect1.stype==STYPE_PC)                                    //Default
if(calc[PULTIMATE]==1 && (sect1.stype==STYPE_PC || sect1.stype==STYPE_RC))           /*2016.12.25 araki for mjdrg*/
//if(calc[PULTIMATE]==1 && sect1.stype==STYPE_PC)                                  //2007.8.31 TAKATA
{
          /*MATERIAL FOR ULTIMATE*/
		  gmaterial.sftu=2400.0*jis;                  /*STEEL SN400*/
          ...
          for(hoko=0;hoko<=3;hoko++) /*LOAD DIRECTION X POSI,YPOSI,XNEGA,YNEGA.*/ //araki
          {
            ...
            {
			  Fes=1.0+(HENSHINX-0.15)/0.15*0.5;
              pstress[0]=&(elempn.head.xp); pstress[1]=&(elempn.tail.xp);        //araki
            }
            if(hoko==1)
            {
              Fes=1.0+(HENSHINY-0.15)/0.15*0.5;
              pstress[0]=&(elempn.head.yp); pstress[1]=&(elempn.tail.yp);        //araki
            }
            if(hoko==2)
            {
              Fes=1.0+(HENSHINX-0.15)/0.15*0.5;
              pstress[0]=&(elempn.head.xn); pstress[1]=&(elempn.tail.xn);        //araki
            }
            if(hoko==3)
            {
              Fes=1.0+(HENSHINY-0.15)/0.15*0.5;
              pstress[0]=&(elempn.head.yn); pstress[1]=&(elempn.tail.yn);        //araki
			}
			...
			if(sign==0)                                                         //araki
			{
              ...
              for(ia=0;ia<=na;ia++) /*FOR AXIS*/
              {
				...
				uN=1000.0*(elempn.head.z.N                                     //araki
                           +dsign*(Fes*Ds/Co)*(pstress[HEAD]->N));
                uQ[HEAD][1-ia]=(elempn.head.z.Q[1-ia]                          //araki
                                +dsign
                                *(Fes*Ds/Co)*(pstress[HEAD]->Q[1-ia]))
                               *1000.0;
                uQ[TAIL][1-ia]=(elempn.tail.z.Q[1-ia]                          //araki
                                +dsign
                                *(Fes*Ds/Co)*(pstress[TAIL]->Q[1-ia]))
                               *1000.0;
				uMt=100000.0*(elempn.head.z.Mt                                 //araki
                              +dsign*(Fes*Ds/Co)*(pstress[HEAD]->Mt));
                uM[HEAD][ia]=(elempn.head.z.M[ia]                              //araki
                              +dsign*facei
                              *(Fes*Ds/Co)*(pstress[HEAD]->M[ia]))
                             *100000.0;
                uM[TAIL][ia]=(elempn.tail.z.M[ia]                              //araki
                              +dsign*facej
                              *(Fes*Ds/Co)*(pstress[TAIL]->M[ia]))
							 *100000.0;
				/*** 16.05.12 araki for PC route 3a ***/
				if(elem.sect->etype==COLUMN && sect1.stype==STYPE_PC)
				{
				  uQpc[HEAD][1-ia]=(elempn.head.z.Q[1-ia]
									+dsign
									*(Fes*2.5)*(pstress[HEAD]->Q[1-ia]))
								   *1000.0;
				  uQpc[TAIL][1-ia]=(elempn.tail.z.Q[1-ia]
									+dsign
									*(Fes*2.5)*(pstress[TAIL]->Q[1-ia]))
								   *1000.0;
				  /*uN=1000.0*(elempn.head.z.N
						   +dsign*(Fes*2.5)*(pstress[HEAD]->N)); */
				}
				else if(elem.sect->etype==GIRDER && sect1.stype==STYPE_PC)
				{
...
*/
                  ...
                  */
                  ...
                  if(uM[HEAD][ia]>=0.0)
                  {
					Muhead=ultimatebendingofsrc(elem,ia,(-uN),                  //araki check 161202
                                                &Ns,&Ms,&Nr,&Mr,&Nc,&Mc,0);
                  }
                  else
                  {
  ...
  fprintf(fout0,"Surface %d %15.3f %15.3f\n",ii,uN,Mu);
                ...
                for(in=HEAD;in<=TAIL;in++) /*FOR END*/
                {
                  ...
                  */
                  ...
                  if(in==HEAD)
				  {
					Qp[in]+=1000.0*fabs(elempn.head.z.Q[1-ia]);                //araki
				  }
                  if(in==TAIL)
                  {
					Qp[in]+=1000.0*fabs(elempn.tail.z.Q[1-ia]);                //araki
                  }
...
if(in==HEAD)
{
  uQe[HEAD][1-ia]=(elempn.head.z.Q[1-ia]                                       //araki
                   +dsign
                   *(Fes*1.5)*(pstress[HEAD]->Q[1-ia]))
                  *1000.0;
}
if(in==TAIL)
{
  uQe[TAIL][1-ia]=(elempn.tail.z.Q[1-ia]                                       //araki
                   +dsign
				   *(Fes*1.5)*(pstress[TAIL]->Q[1-ia]))
                  *1000.0;
}
...
if(!strcmp(prj,"mjdrg"))
{
		  if(calc[PULTIMATE]==1)
		  {
			if(sect1.stype==STYPE_PC)
			{
			  fprintf(fout0,"MAX:Ql/Qal=%.5f Q/Qu=%.5f",rateql,ratequ);
			  fprintf(fout0," M/Mal=%.5f M/Mu=%.5f",rateml,ratemu);
			}
			else if(sect1.stype==STYPE_RC)
			{
			  fprintf(fout0,"MAX:Ql/Qal=%.5f Qs/Qas=%.5f Q/Qu=%.5f",rateql,rateqs,ratequ);
			  fprintf(fout0," M/Mal=%.5f Ms/Mas=%.5f M/Mu=%.5f",rateml,ratems,ratemu);
			}
			else
			{
			  fprintf(fout0,"MAX:Ql/Qal=%.5f Qs/Qas=%.5f",rateql,rateqs);
			  fprintf(fout0," Ml/Mal=%.5f Ms/Mas=%.5f",rateml,ratems);
			}
		  }
		  else
		  {
			...
			{
			  fprintf(fout0,"MAX:Ql/Qal=%.5f Q/Qu=%.5f",rateql,ratequ);
			  fprintf(fout0," Ml/Mal=%.5f M/Mu=%.5f",rateml,ratemu);
			}
			else
			{
			  fprintf(fout0,"MAX:Ql/Qal=%.5f Qs/Qas=%.5f",rateql,rateqs);
...
else
{
		  ...
		  /*if(frate!=NULL)
		  {
			fprintf(frate,"%5d %5d %5d %5d %.5f %.5f %.5f %.5f %.5f %.5f %.5f %.5f\n",
					ielem,isect,ni,nj,rateqa,ratequ,ratema,ratemu,rateql,rateqs,rateml,ratems);
		  }*/
		  /***/
          /*if(check){gets(non); if(non[0]!='\0') return 0;}*/
        }
...
else
{
		  ...
		  lN=elempn.head.z.N;                                    //2011.04.30 araki
          lQ=lN*l/sqrt(l*l+h*h)*1000.0;
...
if(!strcmp(prj,"taichung"))
{
  wafact=1.0;
          ...
          for(hoko=0;hoko<=3;hoko++) /*LOAD DIRECTION X POSI,YPOSI,XNEGA,YNEGA.*/ //araki
          {
            if(hoko==0) eN=elempn.head.xp.N;                                    //araki
            if(hoko==1) eN=elempn.head.yp.N;
            if(hoko==2) eN=elempn.head.xn.N;
            if(hoko==3) eN=elempn.head.yn.N;

if(!strcmp(prj,"nishijin"))
{
  if(hoko==0 || hoko==2) wafact=1.0;
  else if(hoko==1 || hoko==3) wafact=1.3;
}

            sQ[0][0]=eN*l/sqrt(l*l+h*h)*1000.0;
            fprintf(fout0,"…•½Žž%s%s:",strhoko[hoko%2],strhugo[(hoko-hoko%2)/2]);   //araki
            fprintf(fout0,"N=%7.3f[tf](%7.2f[kN])",eN,si*eN);
            fprintf(fout0," …•½¬•ª:Qh=%7.3f[tf](%7.2f[kN]) ",
					sQ[0][0]/1000.0,si*sQ[0][0]/1000.0);
/*
...
//}
		  ...
		  /*if(frate!=NULL)
		  {
			fprintf(frate,"%5d %5d %5d %5d  %.5f %.5f 0.00000 0.00000 %.5f %.5f 0.00000 0.00000\n",
					ielem,isect,ni,nj,rateqa,ratequ,rateql,rateqs);
		  }*/
		  /***/
		}
...
if(!strcmp(prj,"mjdrg") && sect1.stype==STYPE_PC)
{
		  fprintf(stderr,"ELEM:%d SECT:%d\n",ielem,isect);
		  ...
		  fprintf(fout0,"’f–Ê:%3d",isect);
		  if(sect1.stype==STYPE_PC)        fprintf(fout0,"=‚o‚b•ÇƒuƒŒ[ƒX ");
		  else                             fprintf(fout0,"=•s–¾•ÇƒuƒŒ[ƒX ");

		  fprintf(fout0,"•ÇŒú:%.1f[cm] —LŒø•:%.1f[cm] \n",sect1.sform.H,sect1.sform.B);
          ...
          for(hoko=0;hoko<=3;hoko++)                                            //araki
          {
            if(hoko==0) eN=elempn.head.xp.N;
            if(hoko==1) eN=elempn.head.yp.N;
            if(hoko==2) eN=elempn.head.xn.N;
            if(hoko==3) eN=elempn.head.yn.N;
...
else
{
		  ...
		  lN=elempn.head.z.N;                                                  //araki

		  fprintf(fout0,"‰”’¼Žž   :");                                          //araki
		  fprintf(fout0,"Nl =%7.3f[tf](%7.2f[kN]) ",lN,si*lN);
...
if(!strcmp(prj,"taichung"))
{
  wafact=1.0;
		 ...
		 /*for(hoko=0;hoko<=1;hoko++)*/
		  for(hoko=0;hoko<=3;hoko++) /*LOAD DIRECTION X POSI,YPOSI,XNEGA,YNEGA.*/ //araki
          {
            if(hoko==0) eN=elempn.head.xp.N;                                   //arak
            if(hoko==1) eN=elempn.head.yp.N;
            if(hoko==2) eN=elempn.head.xn.N;
			if(hoko==3) eN=elempn.head.yn.N;
...
if(!strcmp(prj,"mjdrg"))
{
		  if(calc[PULTIMATE]==1)
		  {
			if(sect1.stype==STYPE_PC)
			{
			  fprintf(fout0," MAX:Ql/Qal=%7.5f                Q /Qu =%7.5f",qlrate[soffset-1],qurate[soffset-1]);
			  fprintf(fout0," Ml/Mal=%7.5f                M /Mu =%7.5f",mlrate[soffset-1],murate[soffset-1]);
			}
			else if(sect1.stype==STYPE_RC)
			{
			  fprintf(fout0," MAX:Ql/Qal=%7.5f Qs/Qas=%7.5f Q /Qu =%7.5f",qlrate[soffset-1],qsrate[soffset-1],qurate[soffset-1]);
			  fprintf(fout0," Ml/Mal=%7.5f Ms/Mas=%7.5f M /Mu =%7.5f",mlrate[soffset-1],msrate[soffset-1],murate[soffset-1]);
			}
			else
			{
			  fprintf(fout0," MAX:Ql/Qal=%7.5f Qs/Qas=%7.5f               ",qlrate[soffset-1],qsrate[soffset-1]);
			  fprintf(fout0," Ml/Mal=%7.5f Ms/Mas=%7.5f",mlrate[soffset-1],msrate[soffset-1]);
			}
		  }
		  else
...
{
		fprintf(fout0,"                                ");
		fprintf(fout0," Ac=%8.2f[cm2]",sect1.sform.H*sect1.sform.B);
}
else
{
		fprintf(fout0," As=%7.2f[cm2]",sect1.thick);
...
if(!strcmp(prj,"mjdrg"))
{
		fprintf(fout0," MAX:               Ns/Nas=%7.5f",qsrate[soffset-1]);
}
else
{
		fprintf(fout0," MAX:N/Na=%7.5f",qarate[soffset-1]);
      ...
      if(sect1.etype==SLAB)
      {
		fprintf(fout0," t=%4.1f[cm]",sect1.thick);                              //araki for thick
...
if(!strcmp(prj,"mjdrg"))
{
  sprintf(non," Ql/Qal=%7.5f Qs/Qas=%7.5f Q/Qu=%7.5f",rateql,rateqs,ratequ);
  strcat(txt,non);
  sprintf(non," Ml/Mal=%7.5f Ms/Mas=%7.5f M/Mu=%7.5f",rateml,ratems,ratemu);
  strcat(txt,non);
}
else
{
  ...
  for(ie=0;ie<nelem;ie++) /*CALCULATION*/
  {
    ...
    if(soffset && sect1.etype!=WALL && sect1.etype!=SLAB)
    {
      ...
      for(ia=0;ia<=na;ia++) /*FOR AXIS*/
      {
        ...
        {
		  Mu=ultimatebendingofsrc(elem,ia,(-uN),
                                  &Ns,&Ms,&Nr,&Mr,&Nc,&Mc,0); /*[kgfcm]*/ // 150605 fukushima for ultimatebendingofsrc: sign==0
        }
        else if(sect1.stype==STYPE_SRC)                         /*SRC*/
        {
          Mu=ultimatebendingofsrc(elem,ia,(-uN),
...
//      MessageBox(NULL,txt,"Surface",MB_OK);
 ...
 //   fprintf(fout1,"Sect=%d\n",sect1.code);
 //   fprintf(fout1,"         NXMAX %8.1f NZMIN %8.1f\n",ys.fmax[0],ys.fmin[0]);
  }
      ...
      if(scode==code)
      {
           ...
           sect->etype==BEAM)
        {
          while(1)
          {
            ...
            if(!strncmp(*(data+0),"SRECT",5))
            {
              ...
              /*** 2011.07.25 Program Þ—¿•s–¾ ***/
              else if(n>=6 && !strncmp(*(data+5),"H1300",5))
              {
                sect->srect[nsteel].F=13000.0*jis;
              }
              else if(n>=6 && !strncmp(*(data+5),"G3600",5))
              {
                sect->srect[nsteel].F=36000.0*jis;
              }
              /***********************************/
            ...
            if(!strncmp(*(data+0),"HKYOU",5))
            {
              ...
              {
                sect->sform.F=13000.0*jis;
              }
              else if(n>=6 && !strncmp(*(data+5),"G3600",5))
              {
                sect->sform.F=36000.0*jis;
              }
              /***********************************/
              else if(n>=6 && !strncmp(*(data+5),"AS175",5))
              {
            ...
            if(!strncmp(*(data+0),"CPIPE",5))
            {
              ...
              else if(n>=4 && !strncmp(*(data+3),"SN490",5))
              {
                sect->sform.F=3300.0*jis;
                            ...
                            || !strncmp(*(data+3),"t>40_SN400",10)))
              {
                sect->sform.F=2200.0*jis;
              }
              else if(n>=4 && (!strncmp(*(data+3),"SN490T40",8)
                            || !strncmp(*(data+3),"t>40_SN490",10)))
              {
                sect->sform.F=3000.0*jis;
              }
              ...
              /*** 2011.07.25 Program Þ—¿•s–¾ ***/
              else if(n>=4 && !strncmp(*(data+3),"H1300",5))
              {
                sect->sform.F=13000.0*jis;
              }
              else if(n>=4 && !strncmp(*(data+3),"G3600",5))
              {
                sect->sform.F=36000.0*jis;
              }
              /***********************************/
			...
			if(!strncmp(*(data+0),"PLATE",5))
			{
			  ...
			  else if(n>=4 && !strncmp(*(data+3),"2ndSUGI",7))
			  {
				sect->wform.Fc=208.0; /*[kgf/cm2]*/
				sect->wform.Ft=159.0;
				sect->wform.Fb=263.0;
                sect->wform.Fs= 18.3;
			  }
			  else if(n>=4 && !strncmp(*(data+3),"BEIMATSU",8))
			  {
				sect->wform.Fc=183.5; /*[kgf/cm2]*/
				sect->wform.Ft=140.7;
				sect->wform.Fb=232.4;
                sect->wform.Fs= 24.4;
			  }
              ...
              /*** 2011.07.25 Program Žg—p‰Â”ÛŒŸ“¢ ***/
              else if(n>=4 && !strncmp(*(data+3),"KATAGI",6))
              {
                sect->wform.Fc=330.3; /*[kgf/cm2]*/
                sect->wform.Ft=247.6;
                sect->wform.Fb=412.9;
                sect->wform.Fs= 24.4;
              }
              ...
              /*** 2011.07.25 Program Žg—p‰Â”ÛŒŸ“¢ ***/
              else if(n>=4 && !strncmp(*(data+3),"E70HINOKI",9))
              {
                sect->wform.Fc=183.5; /*[kgf/cm2]*/
                sect->wform.Ft=134.6;
                sect->wform.Fb=226.3;
			  ...
			  /***********************************/
			  else if(n>=4 && !strncmp(*(data+3),"2ndBEIMATSU",11))
			  {
				sect->wform.Fc=183.5; /*[kgf/cm2]*/
				sect->wform.Ft=140.7;
				sect->wform.Fb=232.4;
			  ...
			  else if(n>=4 && !strncmp(*(data+3),"E110BEIMATSU",12))
			  {
				sect->wform.Fc=24.6/9.80665*100; /*[kgf/cm2]*/
				sect->wform.Ft=18.6/9.80665*100;
				sect->wform.Fb=30.6/9.80665*100;
				sect->wform.Fs= 2.4/9.80665*100;
			  }
			  ...
			  else if(n>=4 && !strncmp(*(data+3),"E105-F300",9))   /*‰ü³Ž¦1024†*/
			  {
			  ...
			  else if(n>=4 && !strncmp(*(data+3),"E105-F345",9))
			  {
				sect->wform.Fc=286.5; /*[kgf/cm2]*/
				sect->wform.Ft=249.8;
				sect->wform.Fb=351.8;
				sect->wform.Fs= 36.7;
			  }
			  else if(n>=4 && !strncmp(*(data+3),"E95-F270",8))
			  {
				sect->wform.Fc=220.2; /*[kgf/cm2]*/
				sect->wform.Ft=189.6;
				sect->wform.Fb=275.3;
			  ...
			  else if(n>=4 && !strncmp(*(data+3),"E95-F310",8))
			  {
				sect->wform.Fc=263.0; /*[kgf/cm2]*/
				sect->wform.Ft=189.6;
				sect->wform.Fb=275.3;
				sect->wform.Fs= 18.3;
			  }
			  else if(n>=4 && !strncmp(*(data+3),"E55-F200",8))
			  {
				sect->wform.Fc=172.3; /*[kgf/cm2]*/
				sect->wform.Ft=137.6;
				sect->wform.Fb=205.9;
				sect->wform.Fs= 27.5;
			  }
			  else if(n>=4 && !strncmp(*(data+3),"E65-F255",8))
			  {
				sect->wform.Fc=208.0; /*[kgf/cm2]*/
				sect->wform.Ft=183.5;
				sect->wform.Fb=256.9;
			  ...
			  /*** 2016.06.01 Ž¦—vŠm”F ***/
			  #if 0
			  else if(n>=4 && !strncmp(*(data+3),"M30CLT",6))
			  {
				sect->wform.Fc=119.3; /*[kgf/cm2]*/
				sect->wform.Ft= 87.9;
				sect->wform.Fb=149.1;
				sect->wform.Fs=  9.1;
			  }
			  ...
			  /***/
			  /*** 2011.07.25 Program Žg—p‰Â”ÛŒŸ“¢ ***/
              else if(n>=4 && !strncmp(*(data+3),"KURI",4))
              {
			  ...
			  /***********************************/
			  /*** 2014.10.23 G30 Pavilion ***/
			  else if(n>=4 && !strncmp(*(data+3),"STICK",5))
              {
				sect->wform.Fc=0.163; /*[kgf/cm2]*/
				sect->wform.Ft=0.163;
				sect->wform.Fb=0.964;
				sect->wform.Fs=0.163;
			  }
			  ...
			  else if(n>=4 && !strncmp(*(data+3),"NPUAE",5))
			  {
				sect->wform.Fc=240.0; /*[kgf/cm2]*/
				sect->wform.Ft=  8.0;
				sect->wform.Fb=  8.0;
				sect->wform.Fs=  8.0;
			  }
            ...
            if(!strncmp(*(data+0),"CKYOU",5))
            {
              sect->sform.type=STEEL_CKYOU;
            ...
            if(!strncmp(*(data+0),"CRECT",4))
            {
			  ...
			  else if(n>=6 && !strncmp(*(data+5),"FC60",4))
              {
				...
				/*sect->crect[nconc].F=611.8;*/   /*Mejirodai*/
			  }
			  ...
			  /*else if(n>=6 && !strncmp(*(data+5),"FC86",4))
			  {
				sect->crect[nconc].F=884.1;
			  }*/                              /*Mejirodai*/
            ...
            if(!strncmp(*(data+0),"REINS",5))
            {
			  ...
			  else if(n>=5 && !strncmp(*(data+4),"SD490",5))
              {
				sect->rein[nrein].F=4996.0/**jis*/;
			  }
            ...
            if(!strncmp(*(data+0),"HOOPS",5))
            {
			  ...
			  else if(n>=4 && !strncmp(*(data+3),"SD490",5))
			  {
				sect->wF=4996.0/**jis*/;
			  }
		...
		if(sect->etype==BRACE)
        {
          while(1)
          {
			...
			if(!strncmp(*(data+0),"SAREA",5))
			{
              ...
              else if(n>=3 && !strncmp(*(data+2),"SN490",5))
              {
                sect->cF=3300.0*jis;
                            ...
                            || !strncmp(*(data+2),"t>40_SN400",10)))
              {
                sect->cF=2200.0*jis;
              }
              else if(n>=3 && (!strncmp(*(data+2),"SN490T40",8)
                            || !strncmp(*(data+2),"t>40_SN490",10)))
              {
                sect->cF=3000.0*jis;
              }
			...
			/*** 16.12.31 araki for mjdrg PCa wall***/
			if(!strncmp(*(data+0),"PAREA",5))
			{
			  sect->sform.H =strtod(*(data+1),NULL);     /*PCa Wall Thickness*/
			  sect->sform.B =strtod(*(data+2),NULL);     /*PCa Wall Width*/
			}
			if(!strncmp(*(data+0),"NSHRT",5))
			{
			  sect->thick=strtod(*(data+1),NULL);        /*PCa Wall Allowable Axial Force Short*/
			}
			/***/
        ...
        if(sect->etype==WALL || sect->etype==SLAB)
        {
          while(1)
          {
            ...
            if(!strncmp(*(data+0),"SREIN",5))
            {
			  ...
			  else if(n>=3 && !strncmp(*(data+2),"SD490",5))
			  {
				sect->wF=4996.0/**jis*/;
			  }
			  else sect->wF=3000.0*jis;
                     ...
                     struct viewparam vp,int mode)
{
  ...
  {
	setfontformat(hdc,96,36,"‚l‚r –¾’©",0,0,0); /*Default*/
	/*setfontformat(hdc,96,36,"‚l‚r –¾’©",0,0,0);*/ /*17.01.02 araki for pdf section list*/
    ...
    TextOut(hdc,ox,top,str,strlen(str));
	setfontformat(hdc,80,30,"‚l‚r –¾’©",0,0,0); /*Default*/
	/*setfontformat(hdc,160,60,"‚l‚r –¾’©",0,0,0);*/ /*17.01.02 araki for pdf section list*/
    top+=100;
  }
...
{
  fprintf(f,"\n");
  fprintf(f,"As:“Sœ Ar:Žå‹Ø Ac:ƒRƒ“ƒNƒŠ[ƒg Ap:‚o‚bƒXƒgƒ‰ƒ“ƒh\n");
  fprintf(f,"N:Ž²—Í Q:‚¹‚ñ’f—Í ");
  fprintf(f,"Mt:‚Ë‚¶‚èƒ‚[ƒƒ“ƒg M:‹È‚°ƒ‚[ƒƒ“ƒg\n");
...
double allowablebendingofrc(struct element elem,
                            ...
                            double Nd /*[kgf]*/,
							int period,
							char sign)          /*0:M>=0.0, 1:M<0.0*/ //130309 fukushima for RC
...
/*N=+:TENSION -:COMPRESSION [kgf]*/
{
  ...
  for(i=0;i<(elem.sect->nconc);i++)
  {
    if(axis==0)
    {
	  ...
	  cYj[i]=elem.sect->crect[i].bottom;*/  //Default
	  if(sign==0)
	  {
		cYi[i]=elem.sect->crect[i].top;
    ...
    if(axis==1)
    {
	  ...
	  cYj[i]=elem.sect->crect[i].left;*/  //Default
	  if(sign==0)
	  {
  ...
  for(i=0;i<(elem.sect->nrein);i++)
  {
	...
	if(axis==1) rYi[i]=elem.sect->rein[i].x;*/  //Default
	if(sign==0)
	{
	  if(axis==0) rYi[i]=elem.sect->rein[i].y;
  ...
  else if(elem.sect->rein[0].F==3500.0 && elem.sect->rein[0].area<6.424)
  {
  ...
  else if(elem.sect->rein[0].F==3976.0 && elem.sect->rein[0].area<6.424)
  {
	if(period==PLONG) {rft=2200.0; rfc=-rft;}
  ...
  else if(elem.sect->rein[0].F>=4996.0 && elem.sect->rein[0].area<6.424)
  {
	if(period==PLONG) {rft=2200.0; rfc=-rft;}
	if(period==PSHORT){rft=4996.0; rfc=-rft;}
  }
  else if(elem.sect->rein[0].F>=4996.0 && elem.sect->rein[0].area>=6.424)
  {
	if(period==PLONG) {rft=2000.0; rfc=-rft;}
...
if(!strcmp(prj,"Odawara") || !strcmp(prj,"Flytower"))
{
  ...
  */
  ...
  {
  /*
    fprintf(fout0,"Žå‹Ø‹–—e‰ž—Í“x ft=%.1f\n",rft);
    fprintf(fout0,"RCŽ²—Í rcN=%11.3f[tf]",-Nd/1000.0);
    fprintf(fout0," dS:%11.3f",Yg);
...
#if 0
if(!strcmp(prj,"kiryu") && sect.code==202 && axis==SX) lk=290.0;
if(!strcmp(prj,"kiryu") && sect.code==203 && axis==SX) lk=290.0;
if(!strcmp(prj,"kiryu") && sect.code==204 && axis==SX) lk=290.0;
if(!strcmp(prj,"kiryu") && sect.code==205 && axis==SX) lk=290.0;
if(!strcmp(prj,"kiryu") && sect.code==206 && axis==SX) lk=290.0;
if(!strcmp(prj,"kiryu") && sect.code==207 && axis==SX) lk=290.0;
if(!strcmp(prj,"kiryu") && sect.code==208 && axis==SX) lk=290.0;
if(!strcmp(prj,"kiryu") && sect.code==209 && axis==SX) lk=290.0;
#endif
...
/*Odawara110.inp-*/
if(!strcmp(prj,"Odawara") && sect.code==501) lk/=4;
if(!strcmp(prj,"Odawara") && sect.code==502) lk/=4;
if(!strcmp(prj,"Odawara") && sect.code==503) lk/=2;
if(!strcmp(prj,"Odawara") && sect.code==504) lk/=2;
if(!strcmp(prj,"Odawara") && sect.code==505) lk/=2;
if(!strcmp(prj,"Odawara") && sect.code==506) lk/=2;
...
if(!strcmp(prj,"Odawara") && sect.code==510) lk/=2;
if(!strcmp(prj,"Odawara") && sect.code==511) lk/=2;
if(!strcmp(prj,"Odawara") && sect.code==512) lk/=3;
if(!strcmp(prj,"Odawara") && sect.code==513) lk/=2;
if(!strcmp(prj,"Odawara") && sect.code==514) lk/=2;
if(!strcmp(prj,"Odawara") && sect.code==515) lk/=2;
...
if(!strcmp(prj,"Flytower") && sect.code==221 && axis==SX) lk/=sqrt(2.0);
...
if(!strcmp(prj,"Parking") && sect.code==203) lk/=2;
if(!strcmp(prj,"Parking") && sect.code==501) lk/=2;
if(!strcmp(prj,"Parking") && sect.code==502) lk/=2;


if(!strcmp(prj,"Soleil") && sect.code==501) lk/=2;
if(!strcmp(prj,"Soleil") && sect.code==502) lk/=2;
if(!strcmp(prj,"Soleil") && sect.code==505) lk/=2;
if(!strcmp(prj,"Soleil") && sect.code==506) lk/=2;
/*
if(!strcmp(prj,"Soleil") && sect.code==501) lk/=sqrt(2.0);
if(!strcmp(prj,"Soleil") && sect.code==502) lk/=sqrt(2.0);
if(!strcmp(prj,"Soleil") && sect.code==505) lk/=sqrt(2.0);
if(!strcmp(prj,"Soleil") && sect.code==506) lk/=sqrt(2.0);
*/
...
if(!strcmp(prj,"RGinza") && sect.code==202 && axis==SY) lk*=3.0;
if(!strcmp(prj,"RGinza") && sect.code==501 && axis==SX) lk=584.0;
if(!strcmp(prj,"RGinza") && sect.code==502) lk=584.0;

//if(!strcmp(prj,"ooiso") && sect.code==201) lk-=12.5;

  if(Nd<0.0)  Na=-allowablecompressionofsteel2(E,sF,lk,lk,sect,axis);  //Lksato_TAKATA
  if(Nd>=0.0) Na=-allowabletensionofsteel(sF,sect);

  if(period==PSHORT) Na*=1.5;

  if(!strcmp(prj,"katsu") && sect.code==204)
  {
    if(Nd<0.0)  Na=-1000000;
    if(Nd>=0.0) Na=2478;              //A.Bolt 1-M16[kgf] for I
  }
  if(!strcmp(prj,"katsu") && sect.code==205)
  {
    if(Nd<0.0)  Na=-1000000;
    if(Nd>=0.0) Na=2478;              //A.Bolt 1-M16[kgf] for I
  }
  if(!strcmp(prj,"katsu") && sect.code==206)
  {
    if(Nd<0.0)  Na=-1000000;
    if(Nd>=0.0) Na=5948;              //A.Bolt 3-M16[kgf] for L,T
  }
  if(!strcmp(prj,"katsu") && sect.code==207)
  {
    if(Nd<0.0)  Na=-1000000;
    if(Nd>=0.0) Na=5948;              //A.Bolt 3-M16[kgf] for L-B,T-B
  }

  /*** araki for eigen value ***/
  if(!strcmp(prj,"hakushima") && (sect.code>400 && sect.code<500))
  //if(!strcmp(prj,"hakushima") && (sect.code>400 && sect.code<510))        //for hakushima63(truss)
  {
    double eigen=1.0;
    ...
    if(Nd<0.0)
    {
      if(Nd/Na>1/eigen) Na=Nd*eigen;
    }
  }

  if(!strcmp(prj,"rest") && (sect.code>500/* && sect.code<600*/))
  {
    double eigen=1.0;
    ...
    {
      //eigen=3.59450;
      eigen=3.08527;      /*rest52bcl_12_X2X5Y1pin.otp*/
      eigen/=2.167;
    }
    else if(period=PSHORT)
    {
    ...
    if(Nd<0.0)
    {
      if(Nd/Na<1/eigen) Na=Nd*eigen;      //Lk‚ð’Z‚­“ü—Í‚µ‚ÄŽZ’è,ˆ³‰ó‚É‚æ‚éˆÀ‘S—¦‚ÆÀ‹ü‚ÌˆÀ‘S—¦‚Ì‚‚¢•û‚ðÌ—p
    }
  }

  if(!strcmp(prj,"ureshiarena") && (sect.code==504))
  {
    double eigen=1.0;
  ...
  if(Na==0.0 || (Nd/Na)>=1.0)
  {
	fprintf(fout0,"ERROR.N > Na=%.3f[tf]\n",Na/1000);       /*Keisansho*/
    return 0.0;
  }

  fprintf(fout0,"Na=%.3f[tf]\n",Na/1000);   /*Keisansho*/
...
/*** 2009.01.15 araki for btlength ********************************************/
  if(axis==SY)
  {
    if(sect.btlength[0]!=0.0) lk=sect.btlength[0];
    else if(sect.btfact[0]!=0.0) lk*=sect.btfact[0];
  }

  else if(axis==SX)
  {
    if(sect.btlength[1]!=0.0) lk=sect.btlength[1];
    else if(sect.btfact[1]!=0.0) lk*=sect.btfact[1];
  }
/******************************************************************************/
...
if(!strcmp(prj,"Flytower") && sect.code==215) Lk=300;
if(!strcmp(prj,"Flytower") && sect.code==216) Lk-=15;
if(!strcmp(prj,"Flytower") && sect.code==218) Lk=400;
...
if(!strcmp(prj,"Takaido") && sect.code==201) Lk=350.0;

//if(!strcmp(prj,"Kouenji") && sect.code==202) Lk*=0.7;
  ...
  if((sect.etype==COLUMN && lamda>200)||(lamda>250))
  {
    if(fout0!=NULL)
    {
	  fprintf(fout0,"ELEMENT TOO LONG.Lk/i=%.5f\n",lamda);                /*Keisansho*/
  ...
  //if(!strcmp(prj,"hakushima") && (sect.code>400 && sect.code<510))      //for hakushima63(truss)
  {
    ...
    if(N>=0.0)
    {
      if(N/Na>1/eigen) Na=N*eigen;
    }
  }


  fprintf(fout0,"Lk=%.1f\n",Lk);                     /* araki for fout0 */
  fprintf(fout0,"Na=%8.3f,Nay=%8.3f,A=%8.3f,fc=%8.3f\n",Na,Nay,A,fc);      /* araki for fout0 */
...
//if(!strcmp(prj,"Kouenji") && sect.code==202) Lk*=0.7;
  ...
  fprintf(fout0,"Lk=%.1f\n",Lk);                 /* araki for fout0 */
  fprintf(fout0,"Na=%8.3f,Nax=%8.3f\n",Na,Nax);  /* araki for fout0 */
...
//if(!strcmp(prj,"Kouenji") && sect.code==202) Lk*=0.7;
    ...
    fprintf(fout0,"cx=%.3f, Lk=%.3f, Kx=%.3f\n",cx,Lk,Kx);         /* araki for fout0 */
...
//if(!strcmp(prj,"Kouenji") && sect.code==202) Lk*=0.7;
    ...
    fprintf(fout0,"cy=%.3f, Lk=%.3f, Ky=%.3f\n",cy,Lk,Ky);         /* araki for fout0 */
  }
  else /*TENSION*/
  {
  ...
  /*** araki for buckling safety level ****************************************/
  #if 1
  /*BUCKLING SAFETY LEVEL*/
  if(!strcmp(prj,"obj") && sect.code<=499)
  {
    if(period!=PLONG)
    {
      if(fabs(N)/(*Ncr)<0.133)
...
/*WOOD TYPE:PLATE*/
{
  ...
  {
    /*** for class ***/
    if(sect.code==501 || sect.code==502 || sect.code==503 || sect.code==504 || sect.code==515
    || sect.code==521 || sect.code==522 || sect.code==523 || sect.code==524)           //2012.02.14 add Sect 524
    {
      A  =90.0;
	  Ixx=848.826;
      Iyy=997.734;
   ...
   /* else if(sect.code==517)
    ...
    /*** for hall ***/
    /*if(sect.code==501 || sect.code==502 || sect.code==503 || sect.code==504 || sect.code==515
    || sect.code==521 || sect.code==522 || sect.code==523)
    {
      A  = 126.0;
      Ixx=1488.000;
      Iyy=1714.000;
    ...
    else if(sect.code==601 || sect.code==602 || sect.code==603 || sect.code==604 || sect.code==605 || sect.code==606)
    {
      A  =207.0;
      Ixx=3768.554;
      Iyy=4205.250;
...
if(!strcmp(prj,"hiro") && sect.code==501) iyy=ixx;
  ...
  if(!strcmp(prj,"west") || (!strcmp(prj,"east")))
  {
  if(sect.code==221 || sect.code==403 || sect.code==413 || sect.code==602 ||
	 (sect.code>230 && sect.code<241) || (sect.code>630 && sect.code<641))

  {
	...
	if(!strcmp(prj,"west"))
	{
	  eigen=16.7812;      /* west05_r01b */
	}
	else if (!strcmp(prj,"east"))
    {
	  eigen=16.7812;      /* west05_r01b */
	}
    ...
    if(period==PLONG)
    {
      eigen*=1.1/3.0*3000.0*sect.wform.Fc/1000.0/(3.14159265*3.14159265*70.0);
    }
    else if(period=PSHORT)
    {
      eigen*=2.0/3.0*3000.0*sect.wform.Fc/1000.0/(3.14159265*3.14159265*70.0);
    }
  ...
  if(!strcmp(prj,"gcdp"))
  {
  if(sect.code==203||sect.code==505||sect.code==506)
  {
  if(period==PLONG)
  {
    if(Nd< 0.0) Na=-sect.bblength[0]; /*COMPRESSION [kgf]*/
    if(Nd>=0.0) Na= sect.bblength[0]; /*TENSION     [kgf]*/
  }
  if(period==PSHORT)
  {
    if(Nd< 0.0) Na=-sect.bblength[1]; /*COMPRESSION [kgf]*/
    if(Nd>=0.0) Na= sect.bblength[1]; /*TENSION     [kgf]*/
  }
  }
  }
  #endif
  ...
  fprintf(fout0,"A=%.3f[cm],Ixx=%.3f[cm],Iyy=%.3f[cm],Zxx=%.3f[cm],Zyy=%.3f[cm]\n",A,Ixx,Iyy,Zxx,Zyy);
  fprintf(fout0,"Fc=%.1f[kgf/cm2],Ft=%.1f[kgf/cm2],Fb=%.1f[kgf/cm2],Fs=%.1f[kgf/cm2]\n",
                 sect.wform.Fc,sect.wform.Ft,sect.wform.Fb,sect.wform.Fs);
...
/*N=+:TENSION -:COMPRESSION*/
{
  ...
  for(i=0;i<(elem.sect->nsteel);i++)
  {
    if(axis==0)
    {
      ...
      {
        sYi[i]=-elem.sect->srect[i].bottom;
        sYj[i]=-elem.sect->srect[i].top;
      }
      /***/
    }
    if(axis==1)
    {
      ...
      if(sign==0)
      {
        sYi[i]=-elem.sect->srect[i].left;
        sYj[i]=-elem.sect->srect[i].right;
      }
      else if(sign==1)
      {
        sYi[i]=elem.sect->srect[i].right;
  ...
  for(i=0;i<(elem.sect->nrein);i++)
  {
    ...
    /* if(rYi[i]<0.0) */
    if((sign==0 && rYi[i]<0.0) || sign==1 && rYi[i]>0.0) // 150605 fukushima for ultimatebendingofsrc
    {
      rAt +=rAi[i];
      rYtg+=rAi[i]*rYi[i];
    }
  ...
  */
  ...
  /*if(fout0!=NULL)
  {
	fprintf(fout0,"yn=%.5f[cm],Yn=%.5f[cm] ",Yn,yn);
	fprintf(fout0,"gN= S%6.3f + R%6.3f + C%6.3f = %6.3f[tf]\n",
			*sN/1000,*rN/1000,*cN/1000,Ng/1000);
	fprintf(fout0,"gM= S%6.3f + R%6.3f + C%6.3f = %6.3f[tfm]\n",
			*sM/100000,*rM/100000,*cM/100000,Mg/100000);
  }

  if(fout0!=NULL)
  {
	fprintf(fout0," Ž²—Í srcN=%11.3f[tf]",-Ng/1000.0);
    fprintf(fout0," dS:%11.3f",Yg);
...
/*RETURN:ALLOWABLE SHEAR OF RC COLUMN,GIRDER FOR LONG.*/
{
  ...
  if(elem.sect->etype==COLUMN) /*"RC STANDARD 2010" P.151*/
  {
	if(Qd==0.0 || dc==0.0) ac=1.0;     // 2016.5.25 Tsuzuki
    else
    {
  ...
  else if(elem.sect->etype==GIRDER || elem.sect->etype==BEAM) /*"RC STANDARD 2010" P.151*/
  {
	if(Qd==0.0 || dc==0.0) ac=1.0;    // 2016.5.25 Tsuzuki
    else
    {
  ...
  if(elem.sect->srein[1-axis].n>=1)
  {
	...
	else if(elem.sect->srein[1-axis].F==3976.0) wft=2000.0;   // Center P.238 TAKATA
	else if(elem.sect->srein[1-axis].F==4996.0) wft=2000.0;
  ...
  else
  {
	...
	else if(elem.sect->wF==3976.0) wft=2000.0;  // Center P.238 TAKATA
	else if(elem.sect->wF==4996.0) wft=2000.0;

    pw=elem.sect->shearrein[1-axis];
  }

  /*if(pw>0.012) pw=0.012;*/
  if(pw>0.006) pw=0.006;                        // "RC Standard 2010" P.151 araki
  ...
  if(!strcmp(prj,"mita"))
  {
    if(elem.sect->code==401 || elem.sect->code==402)
    Qa=7.0/8.0*eAc*(ac*cfs+0.5*wft*(pw-0.002)); /*16.(22)*/
  }
  if(!strcmp(prj,"yamaga"))
  {
    if(elem.sect->code==401 || elem.sect->code==404 || elem.sect->code==405)
    Qa=7.0/8.0*eAc*(ac*cfs+0.5*wft*(pw-0.002)); /*16.(22)*/
  }
  if(!strcmp(prj,"aaxc"))
  {
    if(elem.sect->code>400)
    Qa=7.0/8.0*eAc*(ac*cfs+0.5*wft*(pw-0.002)); /*16.(22)*/
  }
  if(!strcmp(prj,"rest"))
  {
...
/*RETURN:ALLOWABLE SHEAR OF RC COLUMN,GIRDER FOR SHORT.*/
{
  ...
  */
  ...
  if(elem.sect->etype==GIRDER || elem.sect->etype==BEAM)
  {
	if(Qd==0.0 || dc==0.0) ac=1.0 ;                     //2016.5.25 Tsuzuki
	else
	{
  ...
  if(elem.sect->srein[1-axis].n>=1)
  {
	...
	else if(elem.sect->srein[1-axis].F==4996.0) wft=4996.0;
  ...
  else
  {
	...
	else if(elem.sect->wF==4996.0) wft=4996.0;
...
if(!strcmp(prj,"Odawara") || !strcmp(prj,"Flytower"))
{
  ...
  if(elem.sect->etype==COLUMN)               /*"RC STANDARD"16.(25)*/
  {
	/*Qa=7.0/8.0*eAc*(cfs+0.5*wft*(pw-0.002));*/  /*Default*/
	Qa=7.0/8.0*eAc*(2.0/3.0*ac*cfs+0.5*wft*(pw-0.002)); /*15.3 2016.5.25 Tsuzuki*/
  }
  else if(elem.sect->etype==GIRDER || elem.sect->etype==BEAM)
  {
	if(!strcmp(prj,"osakiso"))
	Qa=7.0/8.0*eAc*(ac*cfs+0.5*wft*(pw-0.002)); /*16.(22)*/  /*Default*/
	else
	Qa=7.0/8.0*eAc*(2.0/3.0*ac*cfs+0.5*wft*(pw-0.002)); /*15.3 2016.5.25 Tsuzuki*/
  }
  else Qa=0.0;
...
/*WITH NO N,Q,M.*/
{
  ...
  else if(sect.srein[1].F==4996.0) wft2=4996.0;
...
/*ALLOWABLE,ULTIMATE SHEAR OF RC WALL.*/
{
  ...
  else if(period==PLONG || period==PSHORT)
  {
    ...
    if(period==PLONG)
    {
	  ...
	  else if(elem.sect->wF==3976.0) wft=/*fabs(m.wft)*/2000.0;   // Center P.238 TAKATA
	  else if(elem.sect->wF==4996.0) wft=/*fabs(m.wft)*/2000.0;

      if(!strcmp(prj,"Odawara"))
      {
        wft*=1.1;
      }
      /*if(wft>2200.0) wft=2200.0; */
    ...
    else if(period==PSHORT)
    {
	  ...
	  else if(elem.sect->wF==4996.0) wft=/*fabs(m.wft)*/4996.0;
  ...
  if(fout0!=NULL)
  {
    fprintf(fout0,"THICK=%.3f Ps=%.3f WINDOW=%.3f",t,ps,r);                     //araki for wall
  ...
  if(period==PLONG)
  {
    ...
    if(!strcmp(prj,"ureshitok")) Qw=r*ps*t*l*wft;       /*"RC STANDARD" 18.(32)*/
    else

    Qw=0.0;                                   /*"RC STANDARD" 18.(32)*/
  }
  else
  {
    Qc=r*t*l*cfs;                             /*"RC STANDARD" 18.(30)*/
    Qw=r*ps*t*l0*wft;                          /*"RC STANDARD" 18.(32)*/
  }
...
if(period==PULTIMATE)
{
  ...
  {
    if(!strcmp(prj,"rest"))
    fprintf(fout0,"CONCRETE:Qc=%.2fx%.2fx%.2fx%.2f=%7.3f[tf]",r,t,l,cfs,Qc/1000.0);
    else
...
double allowablecompressionofsteel2(double E,double F,
                                    double lkxx,double lkyy, /*BUCKLING LENGTH[cm]*/
                                    struct section sect,int axis)
/*ALLOWABLE COMPRESSION OF STEEL LONG. =SHORT/1.5*/
{
  double A,Ixx,Iyy,Zxx,Zyy,ixx,iyy,i;
  double lk,lam,lamxx=0.0,lamyy=0.0,LAM,nyu;
  double sfca,sNca;             /*s:STEEL c:COMPRESSION a:ALLOWABLE*/
...
*/
    ...
    lamxx=lk/ixx;    //Default

/*    if(sect.bblength[0]!=0.0) lkxx=sect.bblength[0];
    else if(sect.bbfact[0]!=0.0) lkxx*=sect.bbfact[0];

    lamxx=lkxx/ixx; */     //09.01.13 araki
...
*/
	...
	/*fprintf(fout0,"Lkxx=%.2f, ixx=%4f, lamxx=%.2f\n",lk,ixx,lamxx); */ /*araki memo*/
...
*/
    ...
    lamyy=lk/iyy;   //Default

/*    if(sect.bblength[1]!=0.0) lkyy=sect.bblength[1];
    else if(sect.bbfact[1]!=0.0) lkyy*=sect.bbfact[1];

    lamyy=lkyy/iyy; */     //09.01.13 araki

/*
  }
*/

	/*fprintf(fout0,"Lkyy=%.2f, iyy=%4f, lamyy=%.2f\n",lk,iyy,lamyy);   *//*araki memo*/

/*  if(lkxx>lkyy) lk=lkxx;
  else          lk=lkyy;   */   //09.01.13 araki


  if(lamxx>lamyy) lam=lamxx;
  else            lam=lamyy;

//fprintf(fout0,"lamxx=%.1f, lamyy=%.1f, lam=%.1f\n",lamxx,lamyy,lam);
  ...
  /*if(fout0!=NULL)
  {
	fprintf(fout0,"Zxx=%.3f[cm],Zyy=%.3f[cm]\n",Zxx,Zyy);
  }*/ /*araki memo*/

  /*if(fout0!=NULL)
  {
    fprintf(fout0,"A=%.3f[cm2] Ixx=%.3f[cm4] Iyy=%.3f[cm4] ixx=%.3f[cm] iyy=%.3f[cm]\n",
            A,Ixx,Iyy,ixx,iyy);
    fprintf(fout0,"sfc=%.3f[kgf/cm2] As=%.3f[cm2]\n",sfca,A);
  } */

  /*fprintf(fout0,"Na=%.3f[tf] (Lk=%.1f[cm])\n",sNca/1000,lk); */ /*araki memo*/

  return sNca;
}/*allowablecompressionofsteel2*/
...
/*RETURN:ALLOWABLE BENDING [kgfcm] OF S WEAK,CLOSED.*/
{
  ...
  if(fout0!=NULL)
  {
    ...
    fprintf(fout0," Z=%.3f[cm3]\n",Ze);
    fprintf(fout0,"“SœŽ²—Í sN=%.3f[tf] ‹–—e’l sMa=%.3f[tfm]",
...
/*AXIS SX=Qy,SY=Qx.*/
{
  ...
  */
  ...
  /*if(!strcmp(prj,"katsu"))
  {
    if((sect.code==204 && axis==SX) || (sect.code==205 && axis==SY))
    {
      if(period==PLONG) sQa==1.425;
      else              sQa==2.850;
    }
    else if((sect.code==204 && axis==SY) || (sect.code==205 && axis==SX))
    {
      if(period==PLONG) sQa==2.470;
      else              sQa==4.957;
    }
  }*/
...
/*AXIS SX=Qy,SY=Qx.*/
{
  ...
  if(!strcmp(prj,"yamaga"))
  {
    ...
    else if(sect.code==516)     //for yamagahall
    {
      As=B*H;
    }
   /* else if(sect.code==517)
...
/*  if(!strcmp(prj,"sunny"))
  {
    if(sect.code==501 || sect.code==502 || sect.code==503 || sect.code==511 || sect.code==512)
    {
      As=B*H;
    }
    else
...
/*COEFFICIENTS OF STEEL.*/
{
  ...
  /*  *Ixx=2.0*(1.0/12.0*tw*B*B*B+tw*B*(B/2.0-yg)*(B/2.0-yg))
        +1.0/12.0*(H-2.0*tf)*tw*tw*tw+(H-2.0*tf)*tw*(yg-tw/2.0)*(yg-tw/2.0);*/  //default

    *Ixx=2.0*(1.0/12.0*tf*B*B*B+tf*B*(B/2.0-yg)*(B/2.0-yg))
        +1.0/12.0*(H-2.0*tf)*tw*tw*tw+(H-2.0*tf)*tw*(yg-tw/2.0)*(yg-tw/2.0);    //araki for CWEAK
  }
 ...
 /*   *Iyy=2.0*(1.0/12.0*tw*B*B*B+tw*B*(B/2.0-xg)*(B/2.0-xg))
        +1.0/12.0*(H-2.0*tf)*tw*tw*tw+(H-2.0*tf)*tw*(xg-tw/2.0)*(xg-tw/2.0);*/    //default

    *Iyy=2.0*(1.0/12.0*tf*B*B*B+tf*B*(B/2.0-xg)*(B/2.0-xg))
        +1.0/12.0*(H-2.0*tf)*tw*tw*tw+(H-2.0*tf)*tw*(xg-tw/2.0)*(xg-tw/2.0);      //araki for CKYOU
  }
  else if(sect.sform.type==STEEL_CWEAK)
  {
  /*  *Iyy=(H*B*B*B-(H-tw)*(B-2.0*tf)*(B-2.0*tf)*(B-2.0*tf))/12.0;*/            //default

    *Iyy=(B*H*H*H-(B-tw)*(H-2.0*tf)*(H-2.0*tf)*(H-2.0*tf))/12.0;                //araki for CWEAK
    xg=0.0;
    yc= B/2.0;
    yt=-B/2.0;
  }
...
double allowablebendingofpccolumn(struct element elem,
                                  ...
                                  int axis, /*0:Mx 1:My*/
                                  double Nd, /*[kgf]*/
                                  char sign)          /*0:M>=0.0, 1:M<0.0*/ //150416 fukushima for PC
...
/*FOR ONLY 1 CRECT.*/
{
  ...
  if(elem.sect->stype==STYPE_PC)
  {
	...
	cft=m.pcft; /*-:TENSION*/
	cfc=elem.sect->crect[0].F/3.0; /*+:COMPRESSION*/  /*180125 araki for Mejirodai*/
	/*cft=-elem.sect->crect[0].F/30;*/  /*-:TENSION*/      /*180125 araki for Mejirodai*/
  }
  else return 0.0;
...
/*
  if(fout0!=NULL) fprintf(fout0,"Nd=%.3f[tf], Ninit=%.3f[tf], cfc=%.3f[kgf/cm2]\n",Nd/1000,Ninit/1000,cfc);
  if(fout0!=NULL) fprintf(fout0,"Z=%.3f[cm3], cft=%.3f[tf]\n",cZ1,cft);
  if(fout0!=NULL) fprintf(fout0,"Ma1=%.3f[tfm], Ma2=%.3f[tfm]\n",Ma1/100000,Ma2/100000);
  if(fout0!=NULL) fprintf(fout0,"pcMa=%.3f[tfm]\n",Ma/100000);
*/
  return Ma;
...
/*FOR ONLY CENTER OF PRESTRESS ABOVE CENTER OF CONCRETE.*/
{
  ...
  factor=1.0; /* factor=m.stfact; */
  Mmin=-cZ2*(-cft+factor*Ninit/Ac
                 +factor*Ninit*(pYg-cYg)/cZ2);
  if (Mmin>0.0 &&  fabs(M)<Mmin) {
	Ma=0.0;
  }

  /*if(fout0!=NULL) fprintf(fout0,"# pYg=%.3f[cm],cYg=%.3f[cm]\n",pYg,cYg);
  if(fout0!=NULL) fprintf(fout0,"# cfc=%.3f[kgf/cm2],cft=%.3f[kgf/cm2]\n",cfc,cft);
  if(fout0!=NULL) fprintf(fout0,"# Ninit=%.3f[tf],Ac=%.3f[cm2]\n",Ninit,Ac);
  if(fout0!=NULL) fprintf(fout0,"# cZ1=%.3f[cm3],cZ2=%.3f[cm3]\n",cZ1,cZ2);
  if(fout0!=NULL) fprintf(fout0,"# Ma1=%.3f[tfm],Ma2=%.3f[tfm]\n",Ma1/100000,Ma2/100000);
  if(fout0!=NULL) fprintf(fout0,"# Mmin=%.3f[tfm],M=%.3f[tfm]\n",Mmin/100000,M/100000);*/
   /*araki memo*/
  return Ma;
...
/*
  Ma1=cZ1*( cfc-(m.stfact)*Ninit/Ac                         // TAKATA 2007.10.25
               -(m.stfact)*Ninit*(pYg-cYg)/cZ1);
  Ma2=cZ2*(-cft+(m.stfact)*Ninit/Ac
               -(m.stfact)*Ninit*(pYg-cYg)/cZ2);
*/
...
/*
  Ma1=     cZ1*( cfc-(m.stfact)*Ninit/Ac                         // araki for Tsurukawa 2009.08.07
                    +(m.stfact)*Ninit*fabs(pYg-cYg)/cZ1);
  Ma2=     cZ2*(-cft+(m.stfact)*Ninit/Ac
                    +(m.stfact)*Ninit*fabs(pYg-cYg)/cZ2);
*/
/*
  Ma1=     cZ1*( cfc-(m.stfact)*Ninit/Ac                         // araki for PRC 2010.03.13
					+(m.stfact)*Ninit*fabs(pYg-cYg)/cZ1);
  Ma2=     cZ2*( cft+(m.stfact)*Ninit/Ac
                    +(m.stfact)*Ninit*fabs(pYg-cYg)/cZ2);
*/
...
/*FOR ONLY 1 CRECT.*/
{
  ...
  double rNy;                      /*YEILD STRESS OF TENSILE REINS.*/ /*16.06.07 araki for PC*/
  double pNy;                    /*YEILD STRESS OF TENSILE STRANDS.*/ /*16.06.07 araki for PC*/
  int count=0;           /*16.06.07 araki for PC*/
  double rTi[MAXREINS],pTi[MAXSTRND];  /*COMPRESSION=0.TENSION=1.*/ /*16.06.13 araki for PC*/

  double Ac,Ar,Ap;
  double rAY,pAY;
  double Muc,Mup;
  double Mun;            /*16.06.07 araki for PC*/
  ...
  dn=cDi[0]/2; /*16.06.07 araki for PC*/

while(1)     /*16.06.07 araki for PC*/
{
  Ar=0.0; /*TOTAL AREA OF REINS.[cm2]*/
  rAY=0.0;
  rYc=-1000.0; rYt=1000.0; /*[cm]*/
  rpNy=0.0;
  rNy=0.0; /*16.06.07 araki for PC*/
  for(i=0;i<(elem.sect->nrein);i++)
  {
	rAi[i]=elem.sect->rein[i].area;
	if(axis==0) rYi[i]=elem.sect->rein[i].y;
	if(axis==1) rYi[i]=elem.sect->rein[i].x;
	rTi[i]=0;      /*COMPRESSION.*/  /*16.06.13 araki for PC*/
	...
	if(rYi[i]<cYc-dn)
	{
	  rNy+=m.rftu*rAi[i]; /*[kgf]*/  /*16.06.07 araki for PC*/
	  rTi[i]=1;        /*TENSION.*/  /*16.06.13 araki for PC*/
	}
  ...
  pYc=-1000.0; pYt=1000.0; /*[cm]*/
  Ninit=0.0;
  pNy=0.0; /*16.06.07 araki for PC*/
  for(i=0;i<(elem.sect->nstrnd);i++)
  {
    pAi[i]=elem.sect->strnd[i].area[PEND];
	if(axis==0) pYi[i]=elem.sect->strnd[i].y[PEND];
	if(axis==1) pYi[i]=elem.sect->strnd[i].x[PEND];
	pTi[i]=0;      /*COMPRESSION.*/  /*16.06.13 araki for PC*/

	Ninit+=elem.sect->strnd[i].Ni[PEND]*1000.0; /*[kgf]*/
	rpNy+=m.stftu[elem.sect->strnd[i].type]*pAi[i]; /*[kgf]*/
	if(pYi[i]<cYc-dn)
	{
	  pNy+=m.stftu[elem.sect->strnd[i].type]*pAi[i]; /*[kgf]*/  /*16.06.07 araki for PC*/
	  pTi[i]=1;                                      /*TENSION.*/  /*16.06.13 araki for PC*/
	}
  ...
  /*if(elem.sect->stype==STYPE_PC) cfc=m.pcF;*/ /*+:COMPRESSION*/   //Default
  /*** 16.05.15 araki for PC ***/
  if(elem.sect->stype==STYPE_PC) cfc=elem.sect->crect[0].F; /*+:COMPRESSION*/
  /***/
  else return 0.0;
  ...
  if(Nd<0)
  {
	if((fabs(Nd))>=rpNy)
	{
	  if(fout0!=NULL)
	  {
		fprintf(fout0,"TOO MUCH AXIAL FORCE.\n");
	  }
	  return 0.0;
	}
  }
  ...
  /*dn=(Nd+(rpNy/2))/(k1*cfc)/cBi[0];*/  //Default
  /*** 16.06.07 araki for PC ***/
  dn=(Nd+rNy+pNy)/(k1*cfc)/cBi[0];
  count++;
  if(count==4)
  {
	/*if(dn>(cDi[0]/2))
	{
	  if(fout0!=NULL)
	  {
		...
		fprintf(fout0,"Nd=%.3f[tf],rpNy=%.3f[tf],x=%.3f[cm]\n",Nd/1000.0,rpNy/1000.0,dn);
	  }
  ...
  /*if(fout0!=NULL)
  {
	fprintf(fout0,"Nd=%.3f[tf],rpNy=%.3f[tf],rNy=%.3f[tf],pNy=%.3f[tf],x=%.3f[cm]\n",Nd/1000.0,rpNy/1000.0,rNy/1000.0,pNy/1000.0,dn);
  }*/  /*araki memo*/

  /*Muc=(Nd+(rpNy/2))*((1-k2)*dn);*/ /*Default*/
  Mun=Nd*(cYc-k2*dn);                /*** 16.05.15 araki for PC ***/
  Mup=0.0;
  /*** Default ***/
  #if 0
  for(i=0;i<(elem.sect->nstrnd);i++)
  ...
  for(i=0;i<(elem.sect->nstrnd);i++)
  {
	if(pYi[i]<cYc-dn)   /*16.06.07 araki for PC*/
	{
	  pft=m.stftu[elem.sect->strnd[i].type]; /*+:TENSION*/
	  pN=pft*pAi[i];
	  Mup+=pN*(cYc-pYi[i]-k2*dn);         /*** 16.05.15 araki for PC ***/
	}
  }
  for(i=0;i<(elem.sect->nrein);i++)
  {
	if(rYi[i]<cYc-dn)   /*16.06.07 araki for PC*/
	{
	  pN=m.rftu*rAi[i];
	  Mup+=pN*(cYc-rYi[i]-k2*dn);        /*** 16.05.15 araki for PC ***/
	}
  }
  #endif
  /***/

  /***16.06.13 araki for PC***/
  for(i=0;i<(elem.sect->nstrnd);i++)
  {
	pft=m.stftu[elem.sect->strnd[i].type]; /*+:TENSION*/
	pN=pft*pAi[i];
	Mup+=pN*(cYc-pYi[i]-k2*dn)*pTi[i];         /*** 16.06.13 araki for PC ***/
  }
  for(i=0;i<(elem.sect->nrein);i++)
  {
	pN=m.rftu*rAi[i];
	Mup+=pN*(cYc-rYi[i]-k2*dn)*rTi[i];        /*** 16.06.13 araki for PC ***/
  }
  /***/

  /*Mu=Muc+Mup;*/  /*Default*/
  Mu=Mun+Mup;      /*** 16.05.15 araki for PC ***/

  /*if(fout0!=NULL)
  {
	fprintf(fout0,"Nd=%.3f rpNy=%.3f[tf]\n",Nd/1000.0,rpNy/1000.0);
	fprintf(fout0,"dn=%.3f[cm]\n",dn);
	fprintf(fout0,"Mun=(%.3f)*((%.3f)-(%.3f)*(%.3f))=%.3f\n",Nd/1000.0,cYc,k2,dn,Mun/100000.0);
	fprintf(fout0,"Mup=%.3f\n",Mup/100000.0);
...
/*FOR ONLY 1 CRECT.*/
{
  ...
  double Mu;                                           /*u:ULTIMATE*/
  double Ninit;               /*TOTAL INITIAL PRESTRESS IN STRANDS.*/
  double rpNy,rpNyi;       /*TOTAL YEILD STRESS OF TENSILE STRANDS.*/
  double rrNy,rrNyi;         /*TOTAL YEILD STRESS OF TENSILE REINS.*/
  double Nytotal;    /*TOTAL YEILD STRESS OF ALL STRANDS AND REINS.*/  /*16.06.07 araki for PC*/
...
while(1)     /*16.06.07 araki for PC*/
{
  Ar=0.0; /*TOTAL AREA OF REINFORCEMENTS.[cm2]*/
  rYg=0.0;
  rYc=-1000.0; rYt=1000.0; /*[cm]*/
  rrNy=0.0;
  Nytotal=0.0; /*16.06.07 araki for PC*/
  for(i=0;i<(elem.sect->nrein);i++)
  {
    rAi[i]=elem.sect->rein[i].area;
	...
	/*if(tensedside==LOWER && rYi[i]<cYg)*/ //Default
	if(tensedside==LOWER && rYi[i]<cYc-dn)
    {
	  rrNyi=m.rftu*rAi[i];
      rrNy+=rrNyi;
	...
	/*if(tensedside==UPPER && rYi[i]>cYg)*/ //Default
	if(tensedside==UPPER && rYi[i]>cYt+dn)  /*16.06.07 araki for PC*/
    {
	  rrNyi=m.rftu*rAi[i];
	  rrNy+=rrNyi;
	...
	Nytotal+=m.rftu*rAi[i];  /*16.06.07 araki for PC*/

    if(rYc<rYi[i]) rYc=rYi[i];
    if(rYt>rYi[i]) rYt=rYi[i];
  }

  if(rrNy!=0.0) // 150430 fukushima fix ultimatebendingofpcgirder
  {
    rYg/=rrNy; /*CENTER OF YIELD STRESS OF REINFORCEMENTS.*/
  }
  ...
  for(i=0;i<(elem.sect->nstrnd);i++)
  {
	...
	/*if((tensedside==UPPER && pYi[i]>cYg) ||
	   (tensedside==LOWER && pYi[i]<cYg))  */  //Default
	if((tensedside==UPPER && pYi[i]>cYt+dn) ||
	   (tensedside==LOWER && pYi[i]<cYc-dn))   /*16.06.07 araki for PC*/
	{
	...
	Nytotal+=m.stftu[elem.sect->strnd[i].type]*pAi[i];  /*16.06.07 araki for PC*/
  ...
  if(rpNy!=0.0) // 150430 fukushima fix ultimatebendingofpcgirder
  {
  ...
  /*COMPRESSED CONCRETE DEPTH = NEUTRAL AXIS.*/
  /*dn=(rrNy+rpNy)/(k1*cfc)/cBi[0];*/   /*Default*/
  dn=(rrNy+rpNy+Nd)/(k1*cfc)/cBi[0];    /*** 16.05.15 araki for PC ***/
  count++;
  if(count==4)
  {
	...
	/*if((tensedside==UPPER && (dn>(rYt-cYt)||dn>(rYc-cYt)||dn>(pYt-cYt)||dn>(pYc-cYt))) ||
	   (tensedside==LOWER && (dn>(cYc-rYc)||dn>(cYc-rYt)||dn>(cYc-pYt)||dn>(cYc-pYt))))
	{
	  if(fout0!=NULL)
	  {
		fprintf(fout0,"COMPRESSED CONCRETE OVERLAPPED.\n");
		if(tensedside==UPPER)      fprintf(fout0,"UPPER ");
		else if(tensedside==LOWER) fprintf(fout0,"LOWER ");
		fprintf(fout0,"cYc=%.1f cYt=%.1f rYc=%.1f rYt=%.1f pYc=%.1f pYt=%.1f dn=%.1f[cm]\n",cYc,cYt,rYc,rYt,pYc,pYt,dn);
	  }
	}*/  /*Error Message Under Construction*/
	break;
  }
  ...
  if(tensedside==LOWER)
  {
	Muc=rrNy*(cYc-rYg-k2*dn)+rpNy*(cYc-pYg-k2*dn);
  }
  else if(tensedside==UPPER)
  {
	Muc=rrNy*(rYg-cYt-k2*dn)+rpNy*(pYg-cYt-k2*dn);
  }*/ //Default
  ...
  for(i=0;i<(elem.sect->nrein);i++)
  {
	...
	if(tensedside==LOWER && rYi[i]<cYg)
	{
	  Mur+=rN*(cYc-rYi[i]-k2*dn);
	  /*fprintf(fout0,"rN=%.3f[tf] dc=%.3f[cm] Mur1=%.3f[tfm]\n",
			  pN/1000.0,(cYc-rYi[i]),pN*(cYc-dn-rYi[i])/100000.0);*/
	}
	else if(tensedside==UPPER && rYi[i]>cYg)
	{
	  Mur+=rN*(rYi[i]-cYt-k2*dn);
	  /*fprintf(fout0,"rN=%.3f[tf] dc=%.3f[cm] Mur1=%.3f[tfm]\n",
			  pN/1000.0,(rYi[i]-cYt),pN*(pYi[i]-rYt-dn)/100000.0);*/
	}
  }
  #endif
  /***/
  ...
  for(i=0;i<(elem.sect->nrein);i++)
  {
	...
	if(tensedside==LOWER && rYi[i]<cYc-dn)    /*16.06.07 araki for PC*/
	{
	  Mur+=rN*(cYc-rYi[i]-k2*dn);      /*** 16.05.15 araki for PC ***/
	}
	else if(tensedside==UPPER && rYi[i]>cYt+dn)   /*16.06.07 araki for PC*/
	{
	  Mur+=rN*(rYi[i]-cYt-k2*dn);    /*** 16.05.15 araki for PC ***/
	}
  }
  #endif
  /***/
  ...
  for(i=0;i<(elem.sect->nrein);i++)
  {
	...
	if(tensedside==LOWER)
	{
	  Mur+=rN*(cYc-rYi[i]-k2*dn)*rTi[i];      /*** 16.06.13 araki for PC ***/
	}
	else if(tensedside==UPPER)
	{
	  Mur+=rN*(rYi[i]-cYt-k2*dn)*rTi[i];      /*** 16.06.13 araki for PC ***/
	}
  }
  /***/
  ...
  for(i=0;i<(elem.sect->nstrnd);i++)
  {
	...
	if(tensedside==LOWER && pYi[i]<cYc-dn)    /*16.06.07 araki for PC*/
	{
	  Mup+=pN*(cYc-pYi[i]-k2*dn);   /*** 16.05.15 araki for PC ***/
	  if(fout0!=NULL)
	  {
	...
	else if(tensedside==UPPER && pYi[i]>cYt+dn)   /*16.06.07 araki for PC*/
	{
	  Mup+=pN*(pYi[i]-cYt-k2*dn);   /*** 16.05.15 araki for PC ***/
	  if(fout0!=NULL)
	  {
		fprintf(fout0,"UPPER pYi=%.3f cYt=%.3f dn=%.3f[cm]\n",pYi[i],cYt,dn);
	  }
  ...
  for(i=0;i<(elem.sect->nstrnd);i++)
  {
	...
	{
	  Mup+=pN*(pYi[i]-cYt-k2*dn)*pTi[i];   /*** 16.06.13 araki for PC ***/
	  /*if(fout0!=NULL && pTi[i]==1)
	  {
		fprintf(fout0,"UPPER pYi=%.3f cYt=%.3f dn=%.3f[cm]\n",pYi[i],cYt,dn);
	  }*/ /*araki memo*/
  ...
  Mu=Mur+Mup+Mun;  /*16.06.07 araki for PC*/

  /*if(fout0!=NULL)
  {
	fprintf(fout0,"Ac=%.3f Ap=%.3f[cm2]\n",Ac,Ap);
	fprintf(fout0,"cfc=%.3f [kgf/cm2]\n",cfc);
	fprintf(fout0,"Ninit=%.3f rrNy=%.3f rpNy=%.3f[tf] Nytotal=%.3f[tf]\n",
			Ninit/1000.0,rrNy/1000.0,rpNy/1000.0,Nytotal/1000.0);

	fprintf(fout0,"dn=%.3f[cm]\n",dn);
	fprintf(fout0,"Mun=%.3f Mur=%.3f Mup=%.3f[tfm]\n",Mun/100000.0,Mur/100000.0,Mup/100000.0);
...
/*FOR FULL PRESTRESSING.*/
{
  ...
  if(pw>0.012) pw=0.012;
  if(wft>3000.0) wft=3000.0;

////////////////////////////////////////////////////////////////////////////////
  de=1000.0; /*[cm]*/                                       // TAKATA 2007.10.25
...
*/
  ...
  if(Qd==0.0)      alpha=1.0;
  else if(fabs(Md/Qd)/(cYc-fabs(de))+1.0==0.0)      alpha=2.0;
  else
  {
  alpha=4.0/(fabs(Md/Qd)/(cYc-fabs(de))+1.0);
  }
  /* alpha=4.0/(Md/Qd/(cYc-pYgt)+1.0);*/
  if(alpha<1.0)      alpha=1.0;
  else if(2.0<alpha) alpha=2.0;

  cfs=7.5+1.5/100.0*(elem.sect->crect[0].F);

  sg=(Nd+Ninit)/Ac;

//  cJ=7.0/8.0*(cYc-pYgt);
  cJ=7.0/8.0*(cYc+(cYc-fabs(de)));

  Qu=(alpha*(cfs+0.1*sg)+0.5*wft*(pw-0.002))*cBi[0]*cJ;

  if(fout0!=NULL)
  {
	fprintf(fout0,"alpha=%.3f, cfs=%.3f, sg=%.3f, cJ=%.3f, \n",alpha,cfs,sg,cJ);
  }
...
/*** 16.05.15 araki for PC Standard P233(71.2a) ***/
double ultimateshearofpccolumn(struct element elem,
							   struct materials m,
							   int axis, /*0:Qy 1:Qx*/
							   double Nd,double Qd,double Md)
...
/*FOR FULL PRESTRESSING.*/
{
  ...
  double cYi[MAXCRECT],cYj[MAXCRECT];
  double rYi[MAXREINS];
  double cYt,cYc,pYt,pYgt,pYc,cYg,rYt,rYc,rYgt,rYgc;
  double pw;
  double jo;

  double cfc,pft,wft;                          /*ALLOWABLE STRESSES*/
  double sg;
  double Qu;                                /*a:ALLOWABLE*/
  double Ninit;               /*TOTAL INITIAL PRESTRESS IN STRANDS.*/

  double Ac,Ap,pAt,rAt,rAc,rAY;
  double pAY;
  double alpha,Lr,nyu,tantheta;
  ...
  //Ar=0.0;  /*TOTAL AREA OF REINS.[cm2]*/
  rAt=0.0; /*AREA OF TENSION REINS.*/
  rAc=0.0; /*AREA OF COMPRESSION REINS.*/
  rAY=0.0;
  rYgt=0.0; /*CENTER OF TENSION REINS.*/
  rYgc=0.0; /*CENTER OF COMPRESSION REINS.*/
  rYc=-1000.0; rYt=1000.0; /*[cm]*/
  for(i=0;i<(elem.sect->nrein);i++)
  {
	rAi[i]=elem.sect->rein[i].area;
  ...
  if(elem.sect->stype==STYPE_PC) cfc=elem.sect->crect[0].F;
  else return 0.0;
...
*/
  ...
  if(Qd==0.0) tantheta=0.0;
  else        tantheta=sqrt(pow(2.0*(fabs(Md))/(fabs(Qd))/cDi[0],2)+1)-2.0*(fabs(Md))/(fabs(Qd))/cDi[0];

  if(Qd==0.0) Lr=1.0;
  else        Lr=(fabs(Md))/2/(fabs(Qd))/cDi[0];
  if(1.0<Lr)  Lr=1.0;

  alpha=sqrt(60/9.80665*100/cfc);
  if(1.0<alpha) alpha=1.0;

  sg=(Nd+Ninit)/Ac;

  nyu=alpha*Lr*(1+sg/cfc);
  if(nyu<0.65) nyu=0.65;
  if(1.0<nyu)  nyu=1.0;

  Qu=cBi[0]*jo*pw*wft+cBi[0]*cDi[0]/2*(nyu*cfc-2*pw*wft)*tantheta;

  /*if(fout0!=NULL)
  {
	fprintf(fout0,"rYgt=%.3f, rYgc=%.3f, alpha=%.3f, Lr=%.3f, sg=%.3f, Fc=%.3f, tantheta=%.3f \n",rYgt,rYgc,alpha,Lr,sg,cfc,tantheta);
  }*/ /*araki memo*/
...
/*FOR FULL PRESTRESSING.*/
{
  ...
  double pYi[MAXSTRND];
  double cYi[MAXCRECT],cYj[MAXCRECT];
  double rYi[MAXREINS],rYj[MAXREINS];
  double cYt,cYc,pYt,pYgt,pYc,cYg;
  double Yg,Ar,dA;
  double pw;
...
////////////////////////////////////////////////////////////////////////////////
  Yg=0.0;                                                   // TAKATA 2007.10.25
  Ar=0.0;
  for(i=0;i<(elem.sect->nrein);i++)
  {
    dA=(elem.sect->rein[i].area);
    if(axis==SX)
    {
      rYi[i]=elem.sect->rein[i].y;
      if(tensedside==UPPER)
      {
        if(elem.sect->rein[i].y>0.0)
        {
          Ar+=(elem.sect->rein[i].area);
          Yg+=dA*rYi[i];
        }
      }
      if(tensedside==LOWER)
    ...
    if(axis==SY)
    {
      rYi[i]=elem.sect->rein[i].x;
      if(tensedside==UPPER)
      {
        if(elem.sect->rein[i].x>0.0)
        {
          Yg+=dA*rYi[i];
          Ar+=(elem.sect->rein[i].area);
        }
  ...
  if(tensedside==UPPER) de=Yg/Ar-cYt;
  if(tensedside==LOWER) de=cYc-Yg/Ar;
////////////////////////////////////////////////////////////////////////////////
...
if(pAt!=0.0)  pYgt/=pAt;
  ...
  if(pw>0.012) pw=0.012;
  if(pw<0.002) pw=0.002;
  if(wft>3000.0) wft=3000.0;

  if(Qd==0.0) alpha=1.0;
  else if((fabs(Md/Qd/de)+1.0)==0.0) alpha=2.0;
  else
  {
...
{
  if((elem.sect->code)==401 || (elem.sect->code)==402 ||
     (elem.sect->code)==403 || (elem.sect->code)==415 ||
     (elem.sect->code)==416 || (elem.sect->code)==417 ||
     (elem.sect->code)==418 || (elem.sect->code)==419 ||
     (elem.sect->code)==420 || (elem.sect->code)==421 ||
     (elem.sect->code)==422 || (elem.sect->code)==434)
  {
    cfs=0.9*(7.5+1.5/100.0*(elem.sect->crect[0].F));    // Light Concrete
  }
  else
  {
    cfs=7.5+1.5/100.0*(elem.sect->crect[0].F);
...
else cfs=7.5+1.5/100.0*(elem.sect->crect[0].F);

  sg=(Nd+Ninit)/Ac;

  cJ=7.0/8.0*de;

  Qu=(alpha*(cfs+0.1*sg)+0.5*wft*(pw-0.002))*cBi[0]*cJ;     // TAKATA 2007.10.25

/*
  fprintf(fout0,"cfs=%.3f\n",cfs);
  fprintf(fout0,"a=%.3f, cfs=%.1f[kgf/cm2], sg=%.1f[kgf/cm2]\n",alpha,cfs,sg);
  fprintf(fout0,"        wft=%.1f[kgf/cm2], pw=%.4f, B=%.1f[cm],d=%.1f[cm]\n",wft,pw,cBi[0],de);
*/
  return Qu;
...
/*FOR FULL PRESTRESSING.*/
{
  ...
  double cfc,pft,wft;                          /*ALLOWABLE STRESSES*/
  double sg;
  double Qu,Qu1;                                /*a:ALLOWABLE*/  /*araki Qu1*/
  double Ninit;               /*TOTAL INITIAL PRESTRESS IN STRANDS.*/
...
*/
  ...
  Qu=cBi[0]*jo*pw*wft+cBi[0]*cDi[0]/2*(nyu*cfc-2*pw*wft)*tantheta;
  /*Qu1=0.5*(Nd+(m.stfact)*Ninit);
  if(Qu1<Qu) Qu=Qu1; */   /*araki Qu1*/
