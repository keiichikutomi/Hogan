...
#include "archg120-3ujok.c"      /*ANALYSIS NONLINEAR,MATH,VIEWING.*/
#include "archg015.c"                     /*ANALYSIS STATIC LINEAR.*/
#include "bclng020ujok.c"                       /*ELASTIC BUCKLING.*/
#include "qadhg001.c"               /*ANALYSIS BIQUADRATIC ELEMENT.*/
#include "srcal007.c"                         /*DRAW SECTIONS LIST.*/
#include "gnshn102.c"                           /*DYNAMIC ANALYSIS.*/
#include "optimization.c"                 			 /*OPTIMIZATION*/
#include "model001.c"                      /*INP-TO-DXF CONVERSION.*/
/*#include "gnshn102-araki.c"*/                 /*DYNAMIC ANALYSIS.*/


#define MENUDIALOGS 4 /*MENU DIALOG BOXES.*/
#define INPUTWIDTH 200      /*DIALOGUE FONT:"SYSTEM" 18pts.*/
...
#define FILENAME           "HP_model11_edited"
...
/*
#define DEFAULTFILETYPE F_ARCLM
#define FILENAME           "t5plane_bend2"
#define OUTPUTLAP          "250"
#define DEFAULTINPUTFILE   FILENAME".inl"
#define DEFAULTINPUTFILEZ  FILENAME".inl"
#define DEFAULTINPUTFILEX  FILENAME".ihx"
#define DEFAULTINPUTFILEY  FILENAME".ihy"
#define DEFAULTOUTPUTFILE  FILENAME"_LAP"OUTPUTLAP".otl2"
#define DEFAULTOUTPUTFILEZ FILENAME"_LAP"OUTPUTLAP".otl2"
#define DEFAULTOUTPUTFILEX FILENAME"_LAP"OUTPUTLAP".otl2"
#define DEFAULTOUTPUTFILEY FILENAME"_LAP"OUTPUTLAP".otl2"
#define DEFAULTSECTIONFILE "dimple.fig"
*/
...
struct arclmframe arci={0,0,
                        ...
                        NULL,NULL,NULL,NULL,
                        NULL,NULL,
                        NULL,NULL,NULL};       /*NULL ARCLM FRAME.*/
  ...
  switch(message)
  {
    ...
    case WM_COMMAND:
	  switch(LOWORD(wParam))
      {
		...
		case IDM_DRAWINGS:                      /*Drawing Window*/
             ...
             wmenu.hwnd!=NULL) /*CREATION.*/
          {
			...
			windowdefinition(wdraw.childs+1,
                             ...
                             WS_CHILD |
							 WS_VISIBLE,
							 0,0,maxX-BARWIDTH-5,maxY-BARWIDTH-5+200,   //size
							 //0,0,1200,900,
							 //0,0,900,1200,
							 (wdraw.childs+0)->hwnd);
        ...
        case IDM_ERROR:
             ...
             wmenu.hwnd!=NULL) /*CREATION.*/
          {
			windowdefinition(&wmesg,
                             ...
                             WS_THICKFRAME |
                             WS_VISIBLE,
							 950,100,300,500,	//700,100,300,500,
                             wmain.hwnd);
		...
		case IDM_EXTRACTARCLMQUIET:
		  if(wdraw.nchilds>=2 &&
             (wdraw.childs+1)->hwnd!=NULL &&
			 (wmenu.childs+2)->vparam.vflag.mv.ftype==F_ORGAN &&
			 (wdraw.childs+1)->org.nodes!=NULL)
		  {
			if(extractarclmfromorganquiet(&((wdraw.childs+1)->org),
									 &arc,&arcx,&arcy)==0)
			{
			  break;
			}
		...
		case IDM_BCLNGCONDENSATION:           //ujioka
		  if((wmenu.childs+2)->vparam.vflag.mv.ftype==F_ARCLM)
		  {
			getviewparam((wmenu.childs+2)->hwnd,
						 &((wdraw.childs+1)->vparam));
...
/*                         H SIZE*/
{
  ...
  ftype=2; /*FTYPE:1=STRESS BY AXIS, 2=STRESS BY FLOOR, 3=DISP BY FLOOR*/
  ...
  for(i=0;i<3;i++) /*FOR PERIOD Z,X,Y*/
  {
    ...
    for(j=np1;j<=np2;j++) /*FOR FRAME X,Y*/
    {
      ...
      for(k=0;k<nframe;k++)
      {
        ...
        {
          /*Mx*/
          sprintf(str,"‹È‚°ƒ‚[ƒƒ“ƒg Mx %s",fname);
          SetDlgItemText((wmenu.childs+3)->hwnd,IDV_TITLEBOX,str);
          SendMessage((wmenu.childs+3)->hwnd,WM_COMMAND,
                      MAKEWPARAM(IDV_MX,0),0);
          SendMessage((wmenu.childs+3)->hwnd,WM_COMMAND,
          ...
          /*My*/
          sprintf(str,"‹È‚°ƒ‚[ƒƒ“ƒg My %s",fname);
          SetDlgItemText((wmenu.childs+3)->hwnd,IDV_TITLEBOX,str);
          SendMessage((wmenu.childs+3)->hwnd,WM_COMMAND,
                      MAKEWPARAM(IDV_MX,0),0);
          SendMessage((wmenu.childs+3)->hwnd,WM_COMMAND,
