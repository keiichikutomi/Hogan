...
#define BCLNGCONDENSATION 0 //ujioka
/*
  Yield Surface 
...
int gnshn101(struct arclmframe *af)
	...
	//fxacc = fopen("TA95000NS_fft.txt", "r");
	//fxacc = fopen("takatori_fft2.txt", "r");
	//fxacc = fopen("takatori_fft9.txt", "r");
	fxacc = fopen("TAT95000NS.txt", "r");
	//fxacc = fopen("TA95000NS_fft.txt", "r");
	//fxacc = fopen("takatori_newwave.txt", "r");
	...
	afact = 50/ 149.843; /* TAKATORI NS */
	//afact = 50.0 / 149.843; /* TAKATORI NS FFT*/
	...
	mtx23 = (struct gcomponent*)
		malloc(msize*sizeof(struct gcomponent));  /* DIAGONALS OF MATRIX. */

	for (i = 0; i < msize; i++) {
		(mtx23 + i)->down = NULL;
	}                             /* GLOBAL MATRIX INITIALIZATION. */

	mtx24 = (struct gcomponent*)
		malloc(msize*sizeof(struct gcomponent));  /* DIAGONALS OF MATRIX. */
	...
	if (dtype == 1) {
		...
		T1 = 2.48184398 ;
        //T1 = 0.78363682; /* FUNDAMENTAL NATURAL PERIOD 1éüå≈óLé¸ä˙ */
		/*CHANGE T1*/
	...
	*/
	...
	for (nlap = 1; nlap <= n101 ; nlap++) { /*Å@n11Ç≈ã≠êßèIóπ or lapsÇ‹Ç≈Å@*/
		...
		for (i = 0; i < nelem; i++) /* ASSEMBLAGE GLOBAL MATRIX. */ {
            ...
            {
              estiff = assempmtxbc(elem,estiff,ncr[i-1]);
                                                   /* ADD PLASTIC MATRIX. */
            }
		...
		if (nlap == 1) {
			if (MessageBox(NULL,"Calculate Eigen Period.","GNSHN",MB_YESNO) == IDYES){
				...
				for (i = 0; i < neig; i++) {
					...
					if (fout != NULL)
						fprintf(fout, "\nBISECSYLVESTER EIGEN VECTOR %ld\n",
						(i + 1));
...
//            errormessage(string);/*for check*/
              elemstressIIbc(estress, &elem, du, melem, fout, drccos, tmatrix,
				estiff, gdisp, edisp, func,ftxt,ncr[i-1]);
            }
            else                     //tension
            {
              elemstressIIbc(estress, &elem, du, melem, fout, drccos, tmatrix,
	...
	/*edit*/
		if (ftxt0 != NULL) {
			...
			*/
			...
			// íeê´ÉGÉlÉãÉMÅ[Ç∆ãzé˚ÉGÉlÉãÉMÅ[ÇÃçáåvÇèoóÕÅiñÿâ∫Åj
			for (ii = 0; ii < nelem; ii++) {
...
int gnshn102(struct arclmframe *af)
	...
	*/
	...
	evct = (double **)malloc(neig*sizeof(double *));
	eigen = (double *)malloc(neig*sizeof(double));
