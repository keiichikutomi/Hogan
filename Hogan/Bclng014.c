...
#define MSIZE   5 /*MAX MATRIX SIZE BY ARRAY.*/
#define NEIGEN  2 /*NUMBERS OF EIGEN VALUES TO BE OBTAINED.*/
...
/*AFTER "ARCLM001".*/
{
  ...
  for(i=1;i<=nelem;i++)                 /*ASSEMBLAGE GLOBAL MATRIX.*/
  {
    ...
    inputelem(elems,af->felem,i-1,&elem);
                   ...
                   double *E,double **V)
{
  ...
  if(ndim==1)
  {
    ...
    if(T<=0.0)
    {
      errormessage("DEIGAB:MATRIX [B] IS NOT POSITIVE DEFINITE.");
    }
    else
    {
