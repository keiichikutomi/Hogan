...
//#define SIUNIT 980665.0                             /*SI UNIT FACTOR for M[Ncm]*/
...
#define RADIUS 0.95                       /*RADIUS OF YIELD SURFACE*/
...
#define DIRECTORY "c:\\cdocs\\data\\" /*DATA DIRECTORY*/
...
struct osect{long int code,loff,ocode/*miharasect*/;
             char *name;
...
///////////////////////////////////////////by MIHARA for Dimension Line
struct dnode{long int loff;
			 double d[3];};

struct delem{char *name;
			 long int loff;
			 int type;
			 double length;
			 struct knode **nods;};

struct dimension{int ndx,ndy,ndz;
				 struct dnode *xnodes;
				 struct dnode *ynodes;
				 struct dnode *znodes;
				 struct delem *xelems;
...
struct dimensionvisible{char dimensionline;
				  };                /*VISIBLE FLAG FOR DIMENSION LINE.*/
/////////////////////////////////////////////////////////////////////////
...
struct globvisible{char axis;
				   ...
				   struct dimensionvisible dv;  //by MIHARA for Dimension Line
...
extern int nmultinode,nmultielem;
extern struct oelem **multielem; /*POINTING CURRENT ELEMENT.*/
extern struct onode **multinode;                     /***araki for multinode***/
...
void drawglobalconfigofaf(HDC hdc,struct viewparam vp,struct arclmframe go,
                                struct onode gn,int mode,long int loff);
///////////////////////////////////////////////////////////////////////////////
void drawdimensionline(FILE *ffig,HDC hdc,struct viewparam vp,int mode,struct organ *go); //mihara for dimension line
void readdimensionfigures(FILE *fin,struct dimension *dim,struct organ *go);
...
int selectlinenode(struct viewparam vp,struct oelem *oe,POINT point); /*mihara for reverse bond*/
...
int gwrite(struct gcomponent *gmtx,
           long int i,long int j,double data);
int gwritesparse(struct gcomponent *gmtx,
                 long int i,long int j,double data);
int gadd(struct gcomponent *gmtx,           //150219 fukushima for gcomponentadd
         long int i,long int j,double data);
...
void weightdistribution(HDC hdc,FILE *fout,struct viewparam *vp,
                        struct organ *org);
void weightdistribution2(HDC hdc,FILE *fout,struct viewparam *vp,
                         struct organ *org);
...
/*MessageBox(NULL,"Begin","Sweep",MB_OK);*/
  ...
  for(i=0;i<msize;i++) /*PIVOT LINE.*/
  {
    ...
    for(j=0;j<msize;j++) /*FOR EACH LINES.*/
    {
      if(j!=i)
      {
        for(k=0;k<msize;k++) /*FOR EACH LOWS.*/
        {
          if(k!=i)
          {
...
/*RETURN:INVERSE MATRIX SYMMETRIC BY CROUT'S LU.*/
{
  ...
  if(det<=0.0) errormessage("Completed as Matrix Instable.");
...
/*RETURN:FLAG OF SUCCESS,PARAMETER OF NEAREST DOTS.*/
{
  ...
  if(lengthvector(vcthh)<eps) /*CASE SAME HEAD.*/
  {
    *nearest1=0.0;
    *nearest2=0.0;
    *distance=0.0;
  ...
  if(fabs(innerproduct(vcthh,outer))<eps)
  /*CASE L1 AND L2 ON SAME PLANE.*/
  {
    if(fabs(outer.dc[GZ])>eps)
    {
  ...
  else /*CASE L1 AND L2 NOT ON SAME PLANE.*/
  {
    ...
    if(otlength<eps || htlength1<eps || htlength2<eps)
    {
      *nearest1=0.0;
      *nearest2=0.0;
      *distance=0.0;
...
/*GET VIEW PARAMETERS FROM DIALOG.*/
{
  ...
  GetDlgItemText(hdwnd,IDV_CONFSIZE,data,80);
  vp->dparam.csize=strtod(data,NULL);
...
/*SET VIEW PARAMETERS INTO DIALOG.*/
{
  ...
  sprintf(str,"%.0f",vp.dparam.csize);
  SetDlgItemText(hdwnd,IDV_CONFSIZE,str);
...
//      vp.dparam.hsize*=5.0;
    ...
    ptLine[0].x = Ox-(vp.dparam.hsize)*sqrt(3);  ptLine[0].y = Oy;
    ptLine[1].x = Ox+(vp.dparam.hsize)*sqrt(3);  ptLine[1].y = Oy;
    ptLine[2].x = Ox+(vp.dparam.hsize)*sqrt(3);  ptLine[2].y = Oy+2.0*(vp.dparam.hsize)*sqrt(3);
    ptLine[3].x = Ox-(vp.dparam.hsize)*sqrt(3);  ptLine[3].y = Oy+2.0*(vp.dparam.hsize)*sqrt(3);
...
//sprintf(str,"%.3f",gn.d[GZ]);                  //mihara
//TextOut(hdc,Ox+2,Oy,str,strlen(str));
//sprintf(str,"%ld",gn.code);                  //mihara
//TextOut(hdc,Ox+20,Oy,str,strlen(str));
  }
...
//mihara for dimension line
void drawdimensionline(FILE *ffig,HDC hdc,struct viewparam vp,int mode,struct organ go)
/*DRAW DIMENSION LINE ON SCREEN BY AXONOMETRICS,PERSPECTIVE.*/
{
  HPEN hpen,ppen;
  ...
  //# if 0
  //for(i=0;i<ge.nnod;i++)   //yocheck
  //{
  //	if(!insiderange(**(ge.nods+i),vp.range)) return;
  //}

  readdimensionfigures(ffig,&dim,&go);

  # if 0
  createdimensionelem(dim,vp,r);

  /*COLOR OF LINE ELEMENT*/
  if(mode==ONPRINTER)      hpen=CreatePen(PS_SOLID,2,RGB(0,0,0)); /*BLACK*/
  else if(mode==ONPREVIEW) hpen=CreatePen(PS_SOLID,1,RGB(0,0,0)); /*BLACK*/
  ...
  if(mode==ONPRINTER)      rop=SRCAND;     //yocheck
  else if(mode==ONPREVIEW) rop=SRCAND;
  else                     rop=SRCPAINT;

  if(vp.vflag.ev.sectioncode) /*SECTION CODE.*/  //moji
  {
  ...
  for(i=0;i<3;i++)/*FOR KIJUN X,Y,Z*/
  {
	...
	if(i==0) /*X*/
	{
	  while(!flag)
	  {
		...
		if(!strcmp(*(data+0),"XKIJUN")) flag=1;
	  }

	  data=fgetsbrk(fin,&nstr);
	  if(nstr==0) return;
	  if(!strcmp(*(data+0),"NKIJUN")) dim->ndx=strtol(*(data+1),NULL,0);

	  dim->xnodes=(struct dnode *)malloc((3*(dim->ndx)+2)*sizeof(struct dnode));
	  dim->xelems=(struct delem *)malloc(2*(dim->ndx)*sizeof(struct delem));

	  for(j=0;j<(dim->ndx);j++)
	  {
		data=fgetsbrk(fin,&nstr);
		if(nstr==0) return;
		if(!strcmp(*(data+0),"KIJUN"))
		{
		  pnode++;

		  (dim->xelems+pnode)->name=(char *)realloc((dim->xelems+pnode)->name,
												   (strlen(*(data+1))+1)*sizeof(char));
		  strcpy((dim->xelems+pnode)->name,*(data+1));

		  (dim->xnodes+pnode)->loff=pnode;

		  if(!strcmp(*(data+2),"NODE"))
		  {
			(dim->xnodes+pnode)->d[GX]=(go->nodes+(strtol(*(data+3),NULL,0)))->d[GX];
		  }
		  if(!strcmp(*(data+2),"COORD"))
		  {
			(dim->xnodes+pnode)->d[GX]=strtod(*(data+3),NULL);
		  }
		}
	  }
	}
	...
	if(i==1) /*Y*/
	{
	  ...
	  {
		data=fgetsbrk(fin,&nstr);
		if(!strcmp(*(data+0),"YKIJUN")) flag=1;
	  }

	  data=fgetsbrk(fin,&nstr);
	  if(nstr==0) return;
	  if(!strcmp(*(data+0),"NKIJUN")) dim->ndy=strtol(*(data+1),NULL,0);

	  dim->ynodes=(struct dnode *)malloc((3*(dim->ndy)+2)*sizeof(struct dnode));
	  dim->yelems=(struct delem *)malloc(2*(dim->ndy)*sizeof(struct delem));
	  ...
	  for(j=0;j<(dim->ndy);j++)
	  {
		...
		if(!strcmp(*(data+0),"KIJUN"))
		{
		  ...
		  (dim->yelems+pnode)->name=(char *)realloc((dim->yelems+pnode)->name,
												   (strlen(*(data+1))+1)*sizeof(char));
		  strcpy((dim->yelems+pnode)->name,*(data+1));

		  (dim->ynodes+pnode)->loff=pnode;

		  if(!strcmp(*(data+2),"NODE"))
		  {
			(dim->ynodes+pnode)->d[GY]=(go->nodes+(strtol(*(data+3),NULL,0)))->d[GY];
		  }
		  if(!strcmp(*(data+2),"COORD"))
		  {
			(dim->ynodes+pnode)->d[GY]=strtod(*(data+3),NULL);
		  }
		}
	  }
	}
	if(i==2) /*Z*/
	{
	  ...
	  {
		data=fgetsbrk(fin,&nstr);
		if(!strcmp(*(data+0),"ZKIJUN")) flag=1;
	  }

	  data=fgetsbrk(fin,&nstr);
	  if(nstr==0) return;
	  if(!strcmp(*(data+0),"NKIJUN")) dim->ndz=strtol(*(data+1),NULL,0);

	  dim->znodes=(struct dnode *)malloc((3*(dim->ndz)+2)*sizeof(struct dnode));
	  dim->zelems=(struct delem *)malloc(2*(dim->ndz)*sizeof(struct delem));
	  ...
	  for(j=0;j<(dim->ndz);j++)
	  {
		...
		if(!strcmp(*(data+0),"KIJUN"))
		{
		  ...
		  (dim->zelems+pnode)->name=(char *)realloc((dim->zelems+pnode)->name,
												   (strlen(*(data+1))+1)*sizeof(char));
		  strcpy((dim->zelems+pnode)->name,*(data+1));

		  (dim->znodes+pnode)->loff=pnode;

		  if(!strcmp(*(data+2),"NODE"))
		  {
			(dim->znodes+pnode)->d[GZ]=(go->nodes+(strtol(*(data+3),NULL,0)))->d[GZ];
		  }
		  if(!strcmp(*(data+2),"COORD"))
		  {
			(dim->znodes+pnode)->d[GZ]=strtod(*(data+3),NULL);
		  }
		}
	  }
	}
...
//                                  0));            //MIHARA
  ...
  if(!idraw)
  {
    ...
    if(vp.vflag.ev.cangle) /*ELEM CANGLE*/            // TAKATA
    {
      sprintf(str,"%.1f",(ge.cangle)*180/PI);
      TextOut(hdc,Cx,Cy,str,strlen(str));
      Cy+=size.cy;
    }
...
/*DRAW ORGANIZATION ON SCREEN BY AXONOMETRICS,PERSPECTIVE.*/
{
  ...
  double hiju,orate,radius,hsize,csize;
  FILE *ffig; /*mihara for dimention line*/
  ...
  if(vp.vflag.mv.pagetitle)
  {
    if(mode==ONPRINTER || mode==ONPREVIEW)
    {
      setfontformat(hdc,150,50,"ÇlÇr ñæí©",0,0,0);
    }
    ...
    if(mode==ONPRINTER) TextOut(hdc,700,300,str,strlen(str));     //700,300-1200,500
  ...
  if(mode==ONPRINTER || mode==ONPREVIEW)
  {
    setfontformat(hdc,(int)(1.5*gprn.jiheight),
                      (int)(1.5*gprn.jiwidth),"ÇlÇr ñæí©",0,0,0);
  }
  if(mode==ONPRINTER)      SetTextColor(hdc,RGB(0,0,0));
  else if(mode==ONPREVIEW) SetTextColor(hdc,RGB(0,0,0));
  else                     SetTextColor(hdc,RGB(255,255,255));
...
/*mihara for zahyo 20190329///////////////////////////////////////////////////*/
  ...
  {
    imax[1]+=size.cy;
    sprintf(str,"ïîçﬁî‘çÜ");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
  }
  if(vp.vflag.ev.cangle)                                        // TAKATA
  {
    imax[1]+=size.cy;
    sprintf(str,"ÉRÅ[ÉhÉAÉìÉOÉã [ìx]");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
  }
  if(vp.vflag.ev.sectioncode)
  {
    imax[1]+=size.cy;
    sprintf(str,"ífñ î‘çÜ");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
  }
  if(vp.vflag.nv.mcircle)
  {
    imax[1]+=size.cy;
    sprintf(str,"êﬂì_èdó ê}");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
  }
  if(vp.vflag.nv.mvalue)
  {
    imax[1]+=size.cy;
    if(globalunit==1.0)         sprintf(str,"êﬂì_èdó íl [tf]");
    else if(globalunit==SIUNIT) sprintf(str,"êﬂì_èdó íl [kN]");
    else                        sprintf(str,"êﬂì_èdó íl");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
  }
  if(vp.vflag.ev.axis)
  {
    imax[1]+=size.cy;
    sprintf(str,"ïîçﬁç¿ïWé≤");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
  }
  if(vp.vflag.ev.cmqline)
  {
    imax[1]+=size.cy;
    sprintf(str,"ï™äÑê¸ï\é¶");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
  }
  ...
  if(vp.vflag.nv.conffig)
  {
    imax[1]+=size.cy;
    imax[1]+=size.cy;
    sprintf(str,"Å£àÛÇÕéxì_à íuÇï\Ç∑");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
   imax[1]+=size.cy;
    sprintf(str,"(X,Y,Z,É∆x,É∆y,É∆zÇÃÇ§ÇøÅCX,Y,ZÇçSë©ÇµÇƒÇ¢ÇÈ)");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));

//    sprintf(str,"Å°àÛÇÕéxì_à íuÇï\Ç∑");
//    TextOut(hdc,imin[0],imax[1],str,strlen(str));
//    imax[1]+=size.cy;
//    sprintf(str,"(X,Y,Z,É∆x,É∆y,É∆zÇÃÇ§ÇøÅCX,Y,Z,É∆x,É∆y,É∆zÇçSë©ÇµÇƒÇ¢ÇÈ)");
//    TextOut(hdc,imin[0],imax[1],str,strlen(str));

    /*FOR OUTPUT REACTION LONG*/
/*    imax[1]+=size.cy;
    imax[1]+=size.cy;
    sprintf(str,"êîílÇÃñ}ó·");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
    imax[1]+=size.cy;
    sprintf(str,"è„íiÅ@Çyï˚å¸îΩóÕ [kN]");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
    imax[1]+=size.cy;
    sprintf(str,"íÜíiÅ@éxì_èdó Å@ [kN]");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
    imax[1]+=size.cy;
    sprintf(str,"â∫íiÅ@çáåvÅ@Å@Å@ [kN]");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
*/
...
*/
  ...
  if(mode==ONPRINTER || mode==ONPREVIEW)
  {
    setfontformat(hdc,(int)gprn.jiheight,
                      (int)gprn.jiwidth,"ÇlÇr ñæí©",0,0,0);
  }
...
//dimension line 2012.10.04/////////////////////////////////////////////////////
  if(vp.vflag.dv.dimensionline)
  {
	ffig = fgetstofopen(DIRECTORY, "r", ID_SECTIONFILE);
	//if (ffig == NULL)
	//break;

	drawdimensionline(ffig,hdc,vp,mode,go);

	fclose(ffig);
  }
//for Yusuhara 2016.04.22/////////////////////////////////////////////////////
#if 0
  int ii;
  int flag01=0,flag11=0,flag02=0,flag12=0;
  int intersectflag=0;
  int Ox,Oy;                                   /*COORDINATION ARROW*/
  HPEN hpen,hpenrange,ppen;
  HBRUSH hbrush,hbrushrange,pbrush;
  ...
  for(i=0;i<go.nnode;i++)
  {
	for(ii=0;ii<go.nelem;ii++)
	{
	  if((*((go.elems+ii)->nods+0))->code==(go.nodes+i)->code ||
		 (*((go.elems+ii)->nods+1))->code==(go.nodes+i)->code)
	  {
		if((go.elems+ii)->sect->code==201) flag01=1;
		else if((go.elems+ii)->sect->code==211) flag11=1;
		else if((go.elems+ii)->sect->code==202) flag02=1;
		else if((go.elems+ii)->sect->code==212) flag12=1;
	  }
	}
	intersectflag=flag01+flag11+flag02+flag12;

	if(intersectflag==12)
	{
	  hpen        = (HPEN)GetCurrentObject(hdc,OBJ_PEN);
	  hbrush      = (HBRUSH)GetCurrentObject(hdc,OBJ_BRUSH);
	  hbrushrange = CreateSolidBrush(RGB(255,0,0));
	  hpenrange   = CreatePen(PS_SOLID,1,RGB(150,150,150));
	  ppen        = (HPEN)SelectObject(hdc,hpenrange);
	  pbrush      = (HBRUSH)SelectObject(hdc,hbrushrange);

	  if(!nodeontoscreen(*(go.nodes+i),&Ox,&Oy,vp)) return;  /*PROJECTION*/

	  Ellipse(hdc,(Ox-(vp.dparam.hsize)/3*5),(Oy-(vp.dparam.hsize)/3*5),(Ox+(vp.dparam.hsize)/3*5),(Oy+(vp.dparam.hsize)/3*5));  /*FILLED CIRCLE*/
	}
	else if(intersectflag==3)
	{
	  hpen        = (HPEN)GetCurrentObject(hdc,OBJ_PEN);
	  hbrush      = (HBRUSH)GetCurrentObject(hdc,OBJ_BRUSH);
	  hbrushrange = CreateSolidBrush(RGB(0,255,0));
	  hpenrange   = CreatePen(PS_SOLID,1,RGB(150,150,150));
	  ppen        = (HPEN)SelectObject(hdc,hpenrange);
	  pbrush      = (HBRUSH)SelectObject(hdc,hbrushrange);
	...
	else if(intersectflag==4)
	{
	  hpen        = (HPEN)GetCurrentObject(hdc,OBJ_PEN);
	  hbrush      = (HBRUSH)GetCurrentObject(hdc,OBJ_BRUSH);
	  hbrushrange = CreateSolidBrush(RGB(0,0,255));
	  hpenrange   = CreatePen(PS_SOLID,1,RGB(150,150,150));
	  ppen        = (HPEN)SelectObject(hdc,hpenrange);
	  pbrush      = (HBRUSH)SelectObject(hdc,hbrushrange);
...
////////////////////////////////////////////////////////////////////////////////
  ...
  */
  ...
  //if(vp.vflag.nv.code ||
  if(vp.vflag.nv.code || vp.vflag.nv.d[0] || vp.vflag.nv.d[1] || vp.vflag.nv.d[2] ||   //mihara for zahyo for zahyo 20190329
	 vp.vflag.nv.mcircle ||
     vp.vflag.nv.mvalue)
  {
    for(i=0;i<go.nnode;i++) /*NODES*/                             //check MIHARA
    {
      if(insiderange(*(go.nodes+i),vp.range))
      {
        loff=6*((go.nodes+i)->loff);
...
//    sprintf(str,"%ld",(go.nodes+i)->code);                  //check mihara
//    TextOut(hdc,Ox+2,Oy,str,strlen(str));
  }
...
//      if(vp.vflag.nv.code ||  /*CODES.*/
	  if(vp.vflag.nv.code || vp.vflag.nv.d[0] || vp.vflag.nv.d[1] || vp.vflag.nv.d[2] ||  /*CODES.*/   //mihara for zahyo 20190329
		 vp.vflag.nv.conffig) /*CONFFIG.*/
      {
        if(mode==ONPRINTER)      SetTextColor(hdc,RGB(0,0,0));
        else if(mode==ONPREVIEW) SetTextColor(hdc,RGB(0,0,0));
        ...
        drawglobalnode(hdc,vp,*(af.ninit+i),NULL); //Check MIHARA

        if(vp.vflag.nv.conffig) /*DRAW CINFFIG*/                //Conffig Mihara
        {
//          drawglobalconfig(hdc,vp,go,*(af.ninit+i),mode,loff);
        }
...
//if(value!=0.0) sprintf(str,"1/%6.3f",(af.ninit+i)->d[2]/value); /*R*/
...
//FOR CHUM 20120709 Mihara
/*if((af.ninit+i)->d[GZ]==0.000||(af.ninit+i)->d[GZ]==1.685||(af.ninit+i)->d[GZ]==2.985||
(af.ninit+i)->d[GZ]==4.514||(af.ninit+i)->d[GZ]==4.714||(af.ninit+i)->d[GZ]==5.814||(af.ninit+i)->d[GZ]==7.847||(af.ninit+i)->d[GZ]==9.147)*/
		  drawglobaltext(hdc,vp,tn,str);                    /*TEXT.*/
        }
      }
        ...
        for(j=0;j<=5;j++) /*REACTIONS.*/
        {
          if((af.confs+loff+j)->iconf==1)
          {
            ...
            if(vp.vflag.nv.react[j])
            {
              ...
              sprintf(str,"%s %.3f",rlabel[j],value);
...
//if((af.elems+ii)->sect->code!=215&&(af.elems+ii)->sect->code!=216)
...
//    TextOut(hdc,ix1+2,iy1,str,strlen(str));      //mihara
...
//                (0.3*(iy2-iy1)+iy1),str,strlen(str));
  ...
  if(vp.vflag.ev.sectioncode) /*SECTION CODE.*/
  {
	...
	/****SRCANMAX****/
	if(vp.vflag.ev.srcanmax) sprintf(str,"ífñ  %d",elem.sect->ocode);
...
//                (0.3*(iy2-iy1)+iy1+50),str,strlen(str));
  ...
  if(vp.vflag.ev.srcanrate) /*SRCAN RATE.*/
  {
    ...
    rate=0.0;
    for(i=0;i<4;i++)                     //mihara for jogedo
    {
  ...
  else if(mode==ONSRCANMAX) /****SRCANMAX****/
  {
  ...
  if(vp.vflag.ev.deformation && af.ddisp!=NULL
     /*&& ((af.elems+ii)->sect->code!=215||(af.elems+ii)->sect->code!=216)*/)
  {
...
/*DRAW STRESS OF ONE ELEMENT BY AXONOMETRICS,PERSPECTIVE.*/
{
  ...
  GetDlgItemText((wmenu.childs+2)->hwnd,ID_SECTIONFILE,str,120);
  if(!strncmp(str,"shang",5)) strcpy(prj,"shang");
  if(!strncmp(str,"tuti",4))  strcpy(prj,"tuti");
...
if(mode!=ONPREVIEW)
{
  ...
  if(!strncmp(prj,"tuti",4)||!strncmp(prj,"wasestair",9)||!strncmp(prj,"shang",5))
  {
	if(i==0)
	{
	  if(elem.stress[0][i]<=0.0)
	  {
		SetTextColor(hdc,RGB(qred,qgreen,qblue)); /*SHEAR.*/
		drawglobalwire(hdc,vp,af,elem,0,0,255,0,0,255,elem.code,mode);
	  }
	  else
	  {
		SetTextColor(hdc,RGB(mred,mgreen,mblue)); /*SHEAR.*/
		drawglobalwire(hdc,vp,af,elem,255,0,0,255,0,0,elem.code,mode);
	  }
  }

#if 0 //////////////////////////////////////////////////////////////////////////
//strcpy(prj,"tuti"); //mihara
if(!strncmp(prj,"tuti",4)||!strncmp(prj,"wasestair",9))
{
if(i<3)
{
//if(elem.stress[0][i]<=0)
//{
//SetTextColor(hdc,RGB(qred,qgreen,qblue));     /*SHEAR.*/
//}
//else
...
if(elem.stress[0][i]>0.5)
  {
	SetTextColor(hdc,RGB(255,0,50));     /*SHEAR.*/
//	drawglobalwire(hdc,vp,af,*(af.elems+i),170,85,150,
//										   170,85,150,0,mode);
  }
else if(elem.stress[0][i]>0.4)
  {
	SetTextColor(hdc,RGB(255,100,50));     /*SHEAR.*/
//	drawglobalwire(hdc,vp,af,*(af.elems+i),128,128,150,
//										   128,128,150,0,mode);
  }
else if(elem.stress[0][i]>0.3)
  {
	SetTextColor(hdc,RGB(255,150,50));     /*SHEAR.*/
//	drawglobalwire(hdc,vp,af,*(af.elems+i),85,170,150,
//										   85,170,150,0,mode);
  }
else if(elem.stress[0][i]>0.2)
  {
	SetTextColor(hdc,RGB(255,255,50));     /*SHEAR.*/
//	drawglobalwire(hdc,vp,af,*(af.elems+i),43,213,150,
//										   43,213,150,0,mode);
  }
else if(elem.stress[0][i]>0.1)
  {
	SetTextColor(hdc,RGB(150,255,50));     /*SHEAR.*/
//	drawglobalwire(hdc,vp,af,*(af.elems+i),0,255,150,
//										   0,255,150,0,mode);
  }
else if(elem.stress[0][i]>0.0)
  {
	SetTextColor(hdc,RGB(100,255,50));     /*SHEAR.*/
//	drawglobalwire(hdc,vp,af,*(af.elems+i),0,255,150,
//										   0,255,150,0,mode);
  }
else if(elem.stress[0][i]>-0.1)
  {
	SetTextColor(hdc,RGB(50,255,100));     /*SHEAR.*/
//	drawglobalwire(hdc,vp,af,*(af.elems+i),150,213,43,
//										   150,213,43,0,mode);
  }
else if(elem.stress[0][i]>-0.2)
  {
	SetTextColor(hdc,RGB(50,255,150));     /*SHEAR.*/
//	drawglobalwire(hdc,vp,af,*(af.elems+i),150,170,85,
//										   150,170,85,0,mode);
  }
else if(elem.stress[0][i]>-0.3)
  {
	SetTextColor(hdc,RGB(50,255,255));     /*SHEAR.*/
//	drawglobalwire(hdc,vp,af,*(af.elems+i),150,128,128,
//										   150,128,128,0,mode);
  }
else if(elem.stress[0][i]>-0.4)
  {
	SetTextColor(hdc,RGB(50,150,255));     /*SHEAR.*/
//	drawglobalwire(hdc,vp,af,*(af.elems+i),150,85,170,
//										   150,85,170,0,mode);
  }
else if(elem.stress[0][i]>-0.5)
  {
	SetTextColor(hdc,RGB(50,100,255));     /*SHEAR.*/
//	drawglobalwire(hdc,vp,af,*(af.elems+i),150,43,213,
//										   150,43,213,0,mode);
  }
else
  {
	SetTextColor(hdc,RGB(50,0,255));     /*SHEAR.*/
//	drawglobalwire(hdc,vp,af,*(af.elems+i),150,0,255,
//										   150,0,255,0,mode);
  }
}
#endif /////////////////////////////////////////////////////////////////////////
...
//sprintf(str,"%.2f",globalunit*elem.stress[0][i]*100000000);   /*HEAD.*/    //for insect gfcm
...
//sprintf(str,"%.2f",globalunit*elem.stress[1][i]*100000000);   /*HEAD.*/    //for insect gfcm
...
//  SetTextColor(hdc,RGB(0,0,0));                       /*AXIS BLACK.*/
//  hpen=CreatePen(PS_SOLID,1,RGB(0,0,0));         /*AXIS LINE BLACK.*/
  ppen=(HPEN)SelectObject(hdc,hpen);
                    ...
                    int mode)
{
  ...
  if(vp.vflag.mv.pagetitle)
  {
	...
	{
	  setfontformat(hdc,120,40,"ÇlÇr ñæí©",0,0,0);
	  //sprintf(str,"4.3 : íåÅCó¿ÅCï«ÅCè∞ÇÃífñ éZíËåãâ ");
	  //sprintf(str,"4.3 : íåÅCó¿ÅCï«ÇÃífñ éZíËåãâ ");
	  //sprintf(str,"4.3 : íåÅCó¿ÅCÉuÉåÅ[ÉXÅCï«ÅCè∞ÇÃífñ éZíËåãâ ");
	  sprintf(str,"4.3 : íåÅCó¿ÅCÉuÉåÅ[ÉXÇÃífñ éZíËåãâ ");
	  TextOut(hdc,900,500,str,strlen(str));

	  setfontformat(hdc,(int)(1.5*gprn.jiheight),
						(int)(1.5*gprn.jiwidth),"ÇlÇr ñæí©",0,0,0);
	  sprintf(str,"äeífñ éÌï Ç…Ç®Ç¢ÇƒÅAåüíËî‰Ç™ç≈ëÂÇÃïîçﬁÇÕâ∫ê}ÇÃí ÇËÅB");
	  TextOut(hdc,900,700,str,strlen(str));
	  sprintf(str,"Ç±ÇÍÇÁÇ…Ç¬Ç¢ÇƒÇÃÇ›ífñ éZíËÇÃè⁄ç◊Çç⁄ÇπÇƒÇ®Ç≠ÅB");
...
/*mihara for zahyo 20190329///////////////////////////////////////////////////*/
  ...
  {
    imax[1]+=size.cy;
    sprintf(str,"êﬂì_èdó íl");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
  }
  if(vp.vflag.ev.ecircle)
  {
    imax[1]+=size.cy;
    sprintf(str,"ãzé˚ÉGÉlÉãÉMÅ[ê}");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
  }
  if(vp.vflag.ev.evalue)
  {
    imax[1]+=size.cy;
    sprintf(str,"ãzé˚ÉGÉlÉãÉMÅ[íl");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
  }
  ...
  {
    imax[1]+=size.cy;
    sprintf(str,"Çwï˚å¸ïœà  [cm]");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
  }
  if(vp.vflag.nv.disps[1])
  {
    imax[1]+=size.cy;
    sprintf(str,"Çxï˚å¸ïœà  [cm]");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
  }
  if(vp.vflag.nv.disps[2])
  {
    imax[1]+=size.cy;
    sprintf(str,"Çyï˚å¸ïœà  [cm]");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
  }
  /*if(vp.vflag.nv.mcircle)
  {
  ...
  for(i=0;i<8;i++)
  {
    ...
    {
      imax[1]+=size.cy;
      if(globalunit==1.0)         sprintf(str,"é≤óÕ [tf]");
      else if(globalunit==SIUNIT) sprintf(str,"é≤óÕ [kN]");
      else                        sprintf(str,"é≤óÕ");
      TextOut(hdc,imin[0],imax[1],str,strlen(str));
      break;
    }
  }
  for(i=0;i<8;i++)
  {
    ...
    {
      imax[1]+=size.cy;
      if(globalunit==1.0)         sprintf(str,"ÇπÇÒífóÕ [tf]");
      else if(globalunit==SIUNIT) sprintf(str,"ÇπÇÒífóÕ [kN]");
      else                        sprintf(str,"ÇπÇÒífóÕ");
      TextOut(hdc,imin[0],imax[1],str,strlen(str));
      break;
    }
  }
  for(i=0;i<8;i++)
  {
    ...
    {
      imax[1]+=size.cy;
      if(globalunit==1.0)         sprintf(str,"ÇÀÇ∂ÇËÉÇÅ[ÉÅÉìÉg [tfm]");
      else if(globalunit==SIUNIT) sprintf(str,"ÇÀÇ∂ÇËÉÇÅ[ÉÅÉìÉg [kNm]");
      else                        sprintf(str,"ÇÀÇ∂ÇËÉÇÅ[ÉÅÉìÉg");
      TextOut(hdc,imin[0],imax[1],str,strlen(str));
      break;
    }
  }
  for(i=0;i<8;i++)
  {
    ...
    {
      imax[1]+=size.cy;
      if(globalunit==1.0)         sprintf(str,"ã»Ç∞ÉÇÅ[ÉÅÉìÉg [tfm]");
      else if(globalunit==SIUNIT) sprintf(str,"ã»Ç∞ÉÇÅ[ÉÅÉìÉg [kNm]");
      else                        sprintf(str,"ã»Ç∞ÉÇÅ[ÉÅÉìÉg");
      TextOut(hdc,imin[0],imax[1],str,strlen(str));
      break;
    }
  }
...
////////////////////////////////////////////////////////////byMIHARA forReaction

  if(vp.vflag.nv.react[0] ||
     vp.vflag.nv.react[1] ||
     vp.vflag.nv.react[2] ||
     vp.vflag.nv.react[3] ||
     vp.vflag.nv.react[4] ||
  ...
  {
    imax[1]+=size.cy;
    sprintf(str,"äÓëbîΩóÕê}");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));

//    imax[1]+=size.cy;
//    sprintf(str,"îΩóÕÇÃçáåvíl [tf][tfm] Å¶ : áîFx= %.5f áîFy= %.5f áîFz= %.5f",1.0,1.0,1.0);
    /* Rx,Ry,Rz : Under Construction*/
//    TextOut(hdc,imin[0],imax[1],str,strlen(str));
  }
...
//////////////////////////////////////////////////////by MIHARA for Safety Level
  ...
  {
    imax[1]+=size.cy;
    sprintf(str,"à¿ëSó¶ (ífñ åüíËî‰ê})");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));

    if(mode==ONPRINTER)
    {
/*	sprintf(str,"à¿ëSó¶ÇÃñ}ó·");
    TextOut(hdc,3000,imax[1]+size.cy,str,strlen(str));

	sprintf(str,"Å@ : ÅÜ1.0");
    TextOut(hdc,3000,imax[1]+3*size.cy,str,strlen(str));
    sprintf(str,"Å@ : 0.9Å`1.0");
    TextOut(hdc,3000,imax[1]+4*size.cy,str,strlen(str));
    sprintf(str,"Å@ : 0.71428Å`0.9");
    TextOut(hdc,3000,imax[1]+5*size.cy,str,strlen(str));
    sprintf(str,"Å@ : 0.7Å`0.71428");
    TextOut(hdc,3000,imax[1]+6*size.cy,str,strlen(str));
    sprintf(str,"Å@ : 0.6Å`0.7");
    TextOut(hdc,3000,imax[1]+7*size.cy,str,strlen(str));
    sprintf(str,"Å@ : 0.5Å`0.6");
    TextOut(hdc,3000,imax[1]+8*size.cy,str,strlen(str));
    sprintf(str,"Å@ : ÅÉ0.5");
    TextOut(hdc,3000,imax[1]+9*size.cy,str,strlen(str));
...
//	SetTextColor(hdc,RGB(255,255,0));
    ...
    {
    sprintf(str,"à¿ëSó¶ÇÃñ}ó·");
    TextOut(hdc,imin[0]+250,imax[1]+size.cy,str,strlen(str));

    sprintf(str,"Å@ : ÅÜ1.0");
    TextOut(hdc,imin[0]+250,imax[1]+3*size.cy,str,strlen(str));
    sprintf(str,"Å@ : 0.9Å`1.0");
    TextOut(hdc,imin[0]+250,imax[1]+4*size.cy,str,strlen(str));
	sprintf(str,"Å@ : 0.71428Å`0.9");
    TextOut(hdc,imin[0]+250,imax[1]+5*size.cy,str,strlen(str));
    sprintf(str,"Å@ : 0.7Å`0.71428");
    TextOut(hdc,imin[0]+250,imax[1]+6*size.cy,str,strlen(str));
    sprintf(str,"Å@ : 0.6Å`0.7");
    TextOut(hdc,imin[0]+250,imax[1]+7*size.cy,str,strlen(str));
    sprintf(str,"Å@ : 0.5Å`0.6");
    TextOut(hdc,imin[0]+250,imax[1]+8*size.cy,str,strlen(str));
    sprintf(str,"Å@ : ÅÉ0.5");
    TextOut(hdc,imin[0]+250,imax[1]+9*size.cy,str,strlen(str));
...
/*  imax[1]+=size.cy;
  ...
  {
    imax[1]+=size.cy;
    sprintf(str,"Å¶ Ç◊ÇΩäÓëbÇÃèÍçáÇÕëœà≥ÉXÉâÉuÇ…íºê⁄îΩóÕÇì¸óÕÇµÇƒÇ¢ÇÈÇΩÇﬂÅAêﬂì_îΩóÕÇ™ÇŸÇ⁄ÇOÇ…Ç»ÇÈ");
    TextOut(hdc,imin[0],imax[1],str,strlen(str));
  }
*/
////by araki for conffig 2008.12.09/////////////////////////////////////////////
if(vp.vflag.nv.conffig)
  {
...
////////////////////////////////////////////////////////////////////////////////
  ...
  if(mode==ONPRINTER || mode==ONPREVIEW)
  {
	setfontformat(hdc,gprn.jiheight,gprn.jiwidth,"ÇlÇr ñæí©",0,0,0);
  }
...
//#if 0
//else if(!strncmp(prj,"icnew",5))
//{
//	  else if(vp.vflag.ev.srcancolor &&
//			  ((af.elems+i)->srate[0]>1.428571 ||
//			   (af.elems+i)->srate[1]>1.428571 ||
//			   (af.elems+i)->srate[2]>1.428571 ||
//			   (af.elems+i)->srate[3]>1.428571))
//	  {
//
//		drawglobalwire(hdc,vp,af,*(af.elems+i),255,0,150,
//											   255,0,150,code,mode/*,i*/);
//	  }
//}
	  else if(vp.vflag.ev.srcancolor &&
			  ((af.elems+i)->srate[0]>1.0 ||
...
//For Competition
        ...
        ((af.elems+i)->srate[0]>4.0 ||
         (af.elems+i)->srate[1]>4.0 ||
         (af.elems+i)->srate[2]>4.0 ||
         (af.elems+i)->srate[3]>4.0))
{
  drawglobalwire(hdc,vp,af,*(af.elems+i),200,200,255,
										 200,200,255,code,mode/*,i*/);
}
else if(vp.vflag.ev.srcancolor &&
        ((af.elems+i)->srate[0]>0.7 || /*0.9*//*0.8*/
         (af.elems+i)->srate[1]>0.7 ||
         (af.elems+i)->srate[2]>0.7 ||
         (af.elems+i)->srate[3]>0.7))
{
  drawglobalwire(hdc,vp,af,*(af.elems+i),255,150,50,
		...
		((af.elems+i)->srate[0]>0.5 || /*0.71428*/
         (af.elems+i)->srate[1]>0.5 ||
         (af.elems+i)->srate[2]>0.5 ||
         (af.elems+i)->srate[3]>0.5))
{
...
//										 255,255,0,code,mode);
        ...
        ((af.elems+i)->srate[0]>0.4 || /*0.7*//*0.6*/
         ...
         (af.elems+i)->srate[3]>0.4))
{
  drawglobalwire(hdc,vp,af,*(af.elems+i),150,255,0,
        ...
        ((af.elems+i)->srate[0]>0.35 || /*0.6*//*0.5*/
         (af.elems+i)->srate[1]>0.35 ||
         (af.elems+i)->srate[2]>0.35 ||
         (af.elems+i)->srate[3]>0.35))
{
  drawglobalwire(hdc,vp,af,*(af.elems+i),0,255,0,
										 0,255,0,code,mode/*,i*/);
}
else if(vp.vflag.ev.srcancolor &&
        ((af.elems+i)->srate[0]>0.3 || /*0.5*//*0.4*/
         (af.elems+i)->srate[1]>0.3 ||
         (af.elems+i)->srate[2]>0.3 ||
         (af.elems+i)->srate[3]>0.3))
{
  drawglobalwire(hdc,vp,af,*(af.elems+i),0,255,150,
										 0,255,150,code,mode/*,i*/);
}
      ...
      {
        drawglobalwire(hdc,vp,af,*(af.elems+i),0,150,255,
											   0,150,255,code,mode/*,i*/);
      }
#if 1
	  /****SRCANMAX****/
	  ...
	  /****************/
#endif	  //else if(vp.vflag.ev.srcanmax &&
	  //        (af.elems+i)->sect->smax==i)
	  //{
	  //	drawglobalwire(hdc,vp,af,*(af.elems+i),255,0,255,
	  //										   255,0,255,code,mode/*,i*/);
      ...
      else
      {
		drawglobalwire(hdc,vp,af,*(af.elems+i),255,255,255,
                       ...
                       (af.elems+i)->sect->dcolor.b,code,mode/*,i*/);
      }
                       ...
                       struct arclmframe af)
{
  ...
  for(i=0;i<af.nnode;i++)
  {
    drawglobalnode(hdc,vp,*(af.ninit+i),NULL);  //Check by MIHARA
  }
...
//mihara for reverse bond///////////////////////////////////////////////////////
int selectlinenode(struct viewparam vp,struct oelem *oe,POINT point)
/*SELECT NODE OF ORGAN ELEMENT BY CURSOR.*/
/*RETURN:POINTER OF NODE.*/
{
  /*char non[80];*/
  int i;
  int ix1,iy1;                                /*WINDOW COORDINATION*/
  int ix2,iy2;                                /*WINDOW COORDINATION*/
  int ax,ay;                                  /*AVERAGE WINDOW COORDINATION*/
  ...
  for(i=0;i<(oe->nnod);i++)
  {
	...
	if(insiderange(*node,vp.range))
	{
	  if(i==0)      nodeontoscreen(*node,&ix1,&iy1,vp);   /*i*/
	  else if(i==1) nodeontoscreen(*node,&ix2,&iy2,vp);   /*j*/
	}
  }
  ...
  if(ix1!=ix2)
  {
	if(ix1<ix2)
	{
	  if ((point.x<=ax) && (ix1<=point.x)) return 1;
	  else return 2;
	}
	else
	{
	  if ((point.x<=ax) && (ix2<=point.x)) return 2;
	  else return 1;
	}
  }
  else
  {
	if(iy1<iy2)
	{
	  if ((point.y<=ay) && (iy1<=point.y)) return 1;
	  else return 2;
	}
	else
	{
	  if ((point.y<=ay) && (iy2<=point.y)) return 2;
	  else return 1;
	}
  }
...
////////////////////////////
           ...
           !strcmp(*(data+pstr),"SECT"))
        {
		  ...
		  (org->sects+psect)->ocode=(org->sects+psect)->code;  /*miharasect*/
...
//strcpy(prj,"hachihiba23");
if(!strncmp(prj,"hachihiba23",11))
{
if((org->nodes+i)->code==126||(org->nodes+i)->code==159||(org->nodes+i)->code==192||
   (org->nodes+i)->code==224||(org->nodes+i)->code==255||(org->nodes+i)->code==284||
   (org->nodes+i)->code==316||(org->nodes+i)->code==348||(org->nodes+i)->code==379||
   (org->nodes+i)->code==414||(org->nodes+i)->code==449||(org->nodes+i)->code==498||
   (org->nodes+i)->code==562||(org->nodes+i)->code==629||(org->nodes+i)->code==690||
   (org->nodes+i)->code==753||(org->nodes+i)->code==828||(org->nodes+i)->code==925||
   (org->nodes+i)->code==975||(org->nodes+i)->code==1022||(org->nodes+i)->code==1069||
   (org->nodes+i)->code==1107||(org->nodes+i)->code==1150||(org->nodes+i)->code==1214||
   (org->nodes+i)->code==1217||(org->nodes+i)->code==1254||(org->nodes+i)->code==1282||
   (org->nodes+i)->code==1314||(org->nodes+i)->code==1342||(org->nodes+i)->code==1376||
   (org->nodes+i)->code==1408||(org->nodes+i)->code==1445||(org->nodes+i)->code==1474||
   (org->nodes+i)->code==1510)
{((org->confs+6*i+1)->value)=2.0;}
else if((org->nodes+i)->code==138||(org->nodes+i)->code==174||(org->nodes+i)->code==206||
		(org->nodes+i)->code==238||(org->nodes+i)->code==269||(org->nodes+i)->code==297||
		(org->nodes+i)->code==329||(org->nodes+i)->code==363||(org->nodes+i)->code==394||
		(org->nodes+i)->code==430||(org->nodes+i)->code==466||(org->nodes+i)->code==519||
		(org->nodes+i)->code==592||(org->nodes+i)->code==659||(org->nodes+i)->code==722||
		(org->nodes+i)->code==787||(org->nodes+i)->code==660||(org->nodes+i)->code==943||
		(org->nodes+i)->code==997||(org->nodes+i)->code==1043||(org->nodes+i)->code==1088||
		(org->nodes+i)->code==1128||(org->nodes+i)->code==1172||(org->nodes+i)->code==1209||
		(org->nodes+i)->code==1234||(org->nodes+i)->code==1266||(org->nodes+i)->code==1295||
		(org->nodes+i)->code==1327||(org->nodes+i)->code==1356||(org->nodes+i)->code==1390||
		(org->nodes+i)->code==1422||(org->nodes+i)->code==1461||(org->nodes+i)->code==1489||
		(org->nodes+i)->code==1525)
{((org->confs+6*i+1)->value)=-2.0;}
else if((org->nodes+i)->code==970||(org->nodes+i)->code==923||(org->nodes+i)->code==864||
		(org->nodes+i)->code==751||(org->nodes+i)->code==529||(org->nodes+i)->code==106||
		(org->nodes+i)->code==111||(org->nodes+i)->code==1330||(org->nodes+i)->code==116||
		(org->nodes+i)->code==121||(org->nodes+i)->code==334||(org->nodes+i)->code==433||
		(org->nodes+i)->code==488||(org->nodes+i)->code==523||(org->nodes+i)->code==538)
{((org->confs+6*i+0)->value)=1.4;}
else if((org->nodes+i)->code==1209||(org->nodes+i)->code==1202||(org->nodes+i)->code==1251||
		(org->nodes+i)->code==1547||(org->nodes+i)->code==1554||(org->nodes+i)->code==1562||
		(org->nodes+i)->code==943||(org->nodes+i)->code==1569||(org->nodes+i)->code==898||
		(org->nodes+i)->code==690||(org->nodes+i)->code==870||(org->nodes+i)->code==875||
		(org->nodes+i)->code==880||(org->nodes+i)->code==885||(org->nodes+i)->code==890)
{((org->confs+6*i+0)->value)=-1.4;}
}

	fprintf(fout,"VCON  ");                   //MIHARA for Balloon(default:6.3f)
    if((org->confs+6*i+0)->value==0.0) fprintf(fout," 0.0  ");
	else fprintf(fout,"%8.3f",(org->confs+6*i+0)->value);
	if((org->confs+6*i+1)->value==0.0) fprintf(fout," 0.0  ");
	else fprintf(fout,"%8.3f",(org->confs+6*i+1)->value);
	if((org->confs+6*i+2)->value==0.0) fprintf(fout," 0.0  ");
	else fprintf(fout,"%8.3f",(org->confs+6*i+2)->value);
	if((org->confs+6*i+3)->value==0.0) fprintf(fout," 0.0  ");
	else fprintf(fout,"%8.3f",(org->confs+6*i+3)->value);
	if((org->confs+6*i+4)->value==0.0) fprintf(fout," 0.0  ");
	else fprintf(fout,"%8.3f",(org->confs+6*i+4)->value);
	if((org->confs+6*i+5)->value==0.0) fprintf(fout," 0.0");
    else fprintf(fout,"%8.3f",(org->confs+6*i+5)->value);
    fprintf(fout,"\n");
  }
  fprintf(fout,"\n");
...
//      fprintf(fout,"           CANG %.5f\n",(org->elems+i)->cangle);
    ...
    {
//if(/*!strncmp(prj,"akita",5)&&*/((org->elems+i)->sect->code==501)) fprintf(fout,"           TYPE COLUMN\n");
//else	  fprintf(fout,"           TYPE GIRDER\n");
...
int extractarclmfromorgan(struct organ *org,
                          ...
                          struct arclmframe *ax,
                          struct arclmframe *ay)
/*EXTRACT ARCLM FROM ORGAN.*/                                     //Check MIHARA
{
  char str[256],non[80];
  int i,j,k,m,n;
  int count,nsect,nelem,nnode,nreact;
  long int oloff,aloff;
  struct osect *s;
  struct oelem *elems;
  FILE *fout;
  ...
  if(!strncmp(non,"tune",4))
  {
	if(!strncmp(non,"tunesteel",9)) strcpy(prj,"tunesteel");
	else if(!strncmp(non,"tunetimber",10)) strcpy(prj,"tunetimber");
	else strcpy(prj,"tune");
  }
  ...
  if(!strcmp(prj,"tune")) weightdistribution2(NULL,fout,NULL,org);
  else weightdistribution(NULL,fout,NULL,org);
  fclose(fout);

  /*COPY INITIAL SECTION CODE TO OCODE*//*miharasect*/
      ...
      if((org->elems+i)->nnod==2)
      {
        for(j=0;j<nsect;j++)
        {
          if(((org->elems+i)->sect->code)==((az->sects+j)->code))
          {
            ...
            if(!strncmp(prj,"balloonhp",9))
            {
              if(((az->elems+k)->node[0]->d[GZ])>=(12.929-920*0.85/80.71)&&
                 ((az->elems+k)->node[1]->d[GZ])>=(12.929-920*0.85/80.71))
              {
                if(((az->elems+k)->sect->E)==7.0E+01 ||                   /*+X*/
                   ((az->elems+k)->sect->E)==7.0E+00 ||                   /*-X*/
                   ((az->elems+k)->sect->E)==7.0E-01 ||                   /*+Y*/
                   ((az->elems+k)->sect->E)==7.0E-02 )                    /*-Y*/
                {
                  (az->elems+k)->stress[0][1]+=(0.6125*0.6125*0.25)*(1.2-0.168)/1000*((az->elems+k)->node[0]->d[GZ]-(12.929-920*0.85/80.71));
                  (az->elems+k)->stress[1][1]+=(0.6125*0.6125*0.25)*(1.2-0.168)/1000*((az->elems+k)->node[1]->d[GZ]-(12.929-920*0.85/80.71));
                }
                else if(((az->elems+k)->sect->E)==7.0E+02)                /*+Z*/
                {
                  (az->elems+k)->stress[0][2]-=(0.6125*0.6125*0.25)*(1.2-0.168)/1000*((az->elems+k)->node[0]->d[GZ]-(12.929-920*0.85/80.71));
                  (az->elems+k)->stress[1][2]-=(0.6125*0.6125*0.25)*(1.2-0.168)/1000*((az->elems+k)->node[1]->d[GZ]-(12.929-920*0.85/80.71));
                }
              }
            }
...
# if 0
if(((az->elems+k)->sect->code)>=900/*||((az->elems+k)->sect->code)==501*/)
{
sprintf(str,"(az->elems+k)->sect->E=%.3f",(az->elems+k)->sect->E);
MessageBox(NULL,str,"balloon",MB_OK);
}
# endif
            k++;
            break;
...
# if 0
sprintf(str,"nelem=%d,k=%d,az->nelem=%d",nelem,k,az->nelem);
MessageBox(NULL,str,"balloon01",MB_OK);
# endif
...
# if 0
if(((az->elems+j)->sect->code)>=900/*||((az->elems+k)->sect->code)==501*/)
{
sprintf(str,"(az->elems+j)->sect->E=%.3f",(az->elems+j)->sect->E);
MessageBox(NULL,str,"balloon",MB_OK);
}
# endif
...
# if 0
sprintf(str,"nelem=%d,k=%d,az->nelem=%d",nelem,k,az->nelem);
MessageBox(NULL,str,"balloon02",MB_OK);
# endif
...
//sprintf(str,"(org->nelem)=%d",(org->nelem));
//MessageBox(NULL,str,"balloon",MB_OK);
          ...
          *(ay->ninit+k)=*(ay->nodes+k);
          ...
          for(n=0;n<6;n++)
          {
            ...
            else
            {
			  ...
			  (ax->confs+aloff)->value=(org->confs+oloff)->value/*0.0*/;     //mihara for hachihiba
			  (ay->confs+aloff)->value=(org->confs+oloff)->value/*0.0*/;
            }
          ...
          {
            (ax->confs+aloff)->value+=(org->loads+i)->w[WEIGHTEQ];
            *(ax->nmass+k)=(org->loads+i)->mass
                           /9.80665; /*[tf] INTO [100t/gal]*/

# if 0
if(!strncmp(prj,"balloonhp",9))
{
if(((az->elems+j)->sect->code)>=900/*||((az->elems+j)->sect->code)==501*/)
{
sprintf(str,"(az->elems+j)->sect->E=%.3f",(az->elems+j)->sect->E);
MessageBox(NULL,str,"balloon",MB_OK);
}
/*
if(((az->elems+j)->sect->code)>=900)
{
sprintf(str,"(az->elems+j)->sect->E=%.3f",(az->elems+j)->sect->E);
MessageBox(NULL,str,"balloon",MB_OK);
}
*/
//MessageBox(NULL,"Balloon","HORIZONTAL X LOAD",MB_OK);

if(((org->nodes+i)->d[GZ])>=(12.929-920*0.85/80.71))
{
//MessageBox(NULL,"Balloon","d[GZ]",MB_OK);
if(((az->elems+j)->sect->E)==7.0E+01)       /*+X*/
{
//MessageBox(NULL,"Balloon","E==7.0E+01",MB_OK);
(az->confs+aloff)->value+=(0.6125*0.6125*0.25)*(1.2-0.168)*((org->nodes+i)->d[GZ]-(12.929-920*0.85/80.71));
}
if(((az->elems+j)->sect->E)==7.0E+00)       /*-X*/
{
(az->confs+aloff)->value-=(0.6125*0.6125*0.25)*(1.2-0.168)*((org->nodes+i)->d[GZ]-(12.929-920*0.85/80.71));
}
}
...
# endif
          ...
          {
            (ay->confs+aloff)->value+=(org->loads+i)->w[WEIGHTEQ];
...
if(((org->nodes+i)->d[GZ])>=(12.929-920*0.85/80.71))
{
if(((az->elems+j)->sect->E)==7.0E-01)       /*+Y*/
{
(az->confs+aloff)->value+=(0.6125*0.6125*0.25)*(1.2-0.168)*((org->nodes+i)->d[GZ]-(12.929-920*0.85/80.71));
}
if(((az->elems+j)->sect->E)==7.0E-02)       /*-Y*/
{
(az->confs+aloff)->value-=(0.6125*0.6125*0.25)*(1.2-0.168)*((org->nodes+i)->d[GZ]-(12.929-920*0.85/80.71));
}
}
...
if(((org->nodes+i)->d[GZ])>=(12.929-920*0.85/80.71))
{
if(((az->elems+j)->sect->E)==7.0E+02)       /*+Z*/
{
(az->confs+aloff)->value+=(0.6125*0.6125*0.25)*(1.2-0.168)*((org->nodes+i)->d[GZ]-(12.929-920*0.85/80.71));
}
}
...
/*RECTANGLE BAN TO 2 BRACE*/
{
  ...
  struct osect *ps;
  struct osect os={0,0,0/*miharasect*/,NULL,1,
                   1,/*NFIG*/
  ...
  *((obraces+1)->nods+1)=*(oe->bans->nods+3);
  ...
  else
  {
    ...
    if(wrate2>0.5)
    {
    sprintf(str,"WINDOW TOO BIG \n ELEMENT=%d\n äJå˚é¸î‰=%1.5f ",(oe->code),wrate2/1.25);
    MessageBox(NULL,str,"WindowRate",MB_OK);
    }

if(!strncmp(prj,"Odawara",7) && oe->type==WALL &&
   (oe->sect)->code>=811 && (oe->sect)->code<=818)  // HP WALL
{
  double length1=0.0,length2=0.0;
  double height1=0.0,height2=0.0;
  struct line underline;
  struct onode n1,n2,n3,n4,nr3,nr4;

  underline.ends[0].d[0]=(*(oe->bans->nods+0))->d[0];
  underline.ends[0].d[1]=(*(oe->bans->nods+0))->d[1];
  underline.ends[0].d[2]=(*(oe->bans->nods+0))->d[2];
  underline.ends[1].d[0]=(*(oe->bans->nods+1))->d[0];
  underline.ends[1].d[1]=(*(oe->bans->nods+1))->d[1];
  underline.ends[1].d[2]=(*(oe->bans->nods+1))->d[2];

  n1.d[0]=(*(oe->bans->nods+0))->d[0];
  n1.d[1]=(*(oe->bans->nods+0))->d[1];
  n1.d[2]=(*(oe->bans->nods+0))->d[2];
  n2.d[0]=(*(oe->bans->nods+1))->d[0];
  n2.d[1]=(*(oe->bans->nods+1))->d[1];
  n2.d[2]=(*(oe->bans->nods+1))->d[2];
  n3.d[0]=(*(oe->bans->nods+2))->d[0];
  n3.d[1]=(*(oe->bans->nods+2))->d[1];
  n3.d[2]=(*(oe->bans->nods+2))->d[2];
  n4.d[0]=(*(oe->bans->nods+3))->d[0];
  n4.d[1]=(*(oe->bans->nods+3))->d[1];
  n4.d[2]=(*(oe->bans->nods+3))->d[2];

  height1=distancedotline(n3,underline,&nr3);
  height2=distancedotline(n4,underline,&nr4);
  height=(height1+height2)/2;

  length1=distancedotdot(n1,n2);
  length2=distancedotdot(nr3,nr4);
  ...
  /*
  sprintf(str,"t=%.5f L=%.5f H=%.5f Ae=%.5f",thick,length,height,Ae);
  ...
  */
  ...
  if(!find)
  {
	...
	os.ocode=oe->sect->code; /*miharasect*/
...
{
  double eps1=1.0E-12,eps2=1.0E-12;

  if(s1->type!=s2->type) return 0;
  if(s1->ocode!=s2->ocode) return 0; // 200225 furuichi for original section

  if(!strncmp(prj,"kiryu",5)) eps2=1.0E-3;
  else if(!strncmp(prj,"Odawara",7))
  {
    if(s1->type == WALL)
    {
      if((s1->figs+0)->prop->E == 2.1E+6)  eps2=1.0E-1;       // Sect 801
      else if((s1->figs+0)->prop->E == 2.1E+7)  eps2=1.0E-2;  // Sect 802
      else if((s1->figs+0)->prop->E == 2.1E+5)  eps2=1.0E-1;  // Sect 804
      else if((s1->figs+0)->prop->E == 2.1E+4)  eps2=1.0;     // Sect 808
      else if((s1->figs+0)->prop->E == 2.1E+8)  eps2=1.0E-3;  // Sect 811-818
    }
    else eps2=1.0E-3;
  }
  ...
  if((s1->figs+0)->area  < (s2->figs+0)->area -eps2 ||
     (s1->figs+0)->area  > (s2->figs+0)->area +eps2)  return 0;
  if((s1->figs+0)->Ixx   < (s2->figs+0)->Ixx  -eps1 ||
     (s1->figs+0)->Ixx   > (s2->figs+0)->Ixx  +eps1)  return 0;
  if((s1->figs+0)->Iyy   < (s2->figs+0)->Iyy  -eps1 ||
     (s1->figs+0)->Iyy   > (s2->figs+0)->Iyy  +eps1)  return 0;
  if((s1->figs+0)->Jzz   < (s2->figs+0)->Jzz  -eps1 ||
     (s1->figs+0)->Jzz   > (s2->figs+0)->Jzz  +eps1)  return 0;
  if((s1->figs+0)->thick < (s2->figs+0)->thick-eps1 ||
     (s1->figs+0)->thick > (s2->figs+0)->thick+eps1)  return 0;

  if((s1->figs+0)->prop->hiju < (s2->figs+0)->prop->hiju-eps1 ||
     (s1->figs+0)->prop->hiju > (s2->figs+0)->prop->hiju+eps1) return 0;
  if((s1->figs+0)->prop->E    < (s2->figs+0)->prop->E   -eps1 ||
     (s1->figs+0)->prop->E    > (s2->figs+0)->prop->E   +eps1) return 0;
  if((s1->figs+0)->prop->poi  < (s2->figs+0)->prop->poi -eps1 ||
     (s1->figs+0)->prop->poi  > (s2->figs+0)->prop->poi +eps1) return 0;
...
void slabdivision2(HDC hdc,struct viewparam *vp,
                   struct obans gban,struct obans *cmqbans)
/*DIVIDE(ï™äÑ) SLAB BAN INTO CMQ PARTS.*/
...
/*            step++;
            ...
            fprintf(fband,"k=%ld\n",k);
            fprintf(fband,"%ldçsñ⁄Ç%ldçsñ⁄Ç∆ì¸ÇÍë÷Ç¶ÇΩåãâ \n",m,k);
            for(s=0;s<nnode;s++)
            {
              for(t=0;t<=s;t++)
              {
...
//fout=fopen("ludeco.txt","w");                          /*FIGURE FILE*/
//fprintf(fout,"croutludecomposition.\n\n");
...
//fprintf(fout,"value(%ld)=%0.5f\n",j,pivot->value);
//fprintf(fout,"sign(%ld)=%0.5f\n",j,*sign);
...
/*WRITE DATA INTO FILE GLOBAL MATRIX.*/
{
  ...
  if(gdown->m==i)                             /*COMPONENT EXISTENT.*/
  {
	if((data<exp && data>-exp) && i!=j)     /*COMPONENT VANISHED.EXCEPT DIAGONAL.*/
	{
	  pdown->down=gdown->down;
	  free(gdown);
  ...
  else if(data>=exp || data<=-exp)                             /*COMPONENT EMPTY.*/
  {
    gcomp=(struct gcomponent *)malloc(sizeof(struct gcomponent));
    if(gcomp==NULL)
	{
...
int gadd(struct gcomponent *gmtx,           //150219 fukushima for gcomponentadd
         long int i,long int j,double data)
/*ADD DATA INTO FILE GLOBAL MATRIX.*/
{
  if(data==0.0)
  {
    return 0;
  ...
  else                                           /*COMPONENT EMPTY.*/
  {
    ...
    if(gcomp==NULL)
    {
      errormessage("GADD:MEMORY INSUFFICIENT.");
      return 0;
    }
...
///    assemconf201(confs,gvct,dsafety,nnode);        /*GLOBAL VECTOR.*/
    ...
    if(fonl!=NULL && nlap==1) fprintf(fonl,"\"TARGET FORCE {F}\"\n");
    for(i=0;i<nnode;i++)
    {
      if(fonl!=NULL && nlap==1)
      {
...
/*SAVE AS ARCLM INPUTFILE.*/
{
  ...
  for(i=0;i<(af->nsect);i++)
  {
	fprintf(fin,"%5d %.5E %.5f %.10f %.8f %.8f %.8f",     //%5d %.5E %.5f %.4f %.8f %.8f %.8f
            (af->sects+i)->code,
			(af->sects+i)->E,
            (af->sects+i)->poi,
            (af->sects+i)->area,
    ...
    for(j=0;j<6;j++)
    {
      if(!strncmp(prj,"Odawara",7))
      {
        if((af->sects+i)->type==WALL)
        {
          if(j==0)
          {
/*
            if((af->sects+i)->E==2.10000E+06)      fprintf(fin,"     292.0    -292.0");
            else if((af->sects+i)->E==2.10000E+05) fprintf(fin,"     313.5     -97.5");
            else if((af->sects+i)->E==2.10000E+04) fprintf(fin,"     421.0    -132.4");
*/
            if((af->sects+i)->E==2.10000E+08)      fprintf(fin,"     380.3    -380.3");
            else if((af->sects+i)->E==2.10000E+07) fprintf(fin,"     631.4    -631.4");
            else if((af->sects+i)->E==2.10000E+06) fprintf(fin,"     783.7    -243.7");
            else if((af->sects+i)->E==2.10000E+05) fprintf(fin,"     653.7    -653.7");
            else if((af->sects+i)->E==2.10000E+04) fprintf(fin,"     653.7    -653.7");
          }
          else
        ...
        fprintf(fin," %10.6f %10.6f",(af->sects+i)->fmax[j],  /*Keta Default 9.1 Iwanaga*/
                                   (af->sects+i)->fmin[j]);
    }
    ...
    fprintf(fin,"%5d",(af->sects+i)->ocode);/*miharasect*/
  ...
  for(i=0;i<(af->nelem);i++)
  {
    ...
    for(ii=0;ii<2;ii++)                                   /*STRESS.*/
    {
      ...
      if(!strncmp(prj,"balloonhp",9))
      {
        for(jj=0;jj<6;jj++)
        {
          fprintf(fin," %2.9f",(af->elems+i)->stress[ii][jj]);
        }
      }
      else if(!strncmp(prj,"nagaoka",7))
      {
        for(jj=0;jj<6;jj++)
        {
          fprintf(fin," %2.9f",(af->elems+i)->stress[ii][jj]);
      ...
      else
      {
        for(jj=0;jj<6;jj++)
        {
          fprintf(fin," %0.10f",(af->elems+i)->stress[ii][jj]);   /*0.3*/
        }
      }
  ...
  for(i=0;i<(af->nnode);i++) /*CONF VECTOR:CONFINEMENT,VALUE.*/
  {
    ...
    for(j=0;j<6;j++)
    {
      ...
      {
        fprintf(fin," %2.9f",(af->confs+offset)->value);
      }
      else if(!strncmp(prj,"balloonhp",9))
      {
        fprintf(fin," %2.9f",(af->confs+offset)->value);
      }
      else if(!strncmp(prj,"nagaoka",7))
      ...
      else if(!strncmp(prj,"agc",3))
      {
        fprintf(fin," %14.9f",(af->confs+offset)->value);
      }
      else
      {
        fprintf(fin," %14.9f",(af->confs+offset)->value);
...
/*SAVE BAND DECREASED ARCLM.*/
{
  ...
  testm=(long int *)malloc(af->nnode*sizeof(long int));
  testn=(long int *)malloc(af->nnode*sizeof(long int));
  if(testm==NULL || testn==NULL) return 0;
...
void initialreact(FILE *fin,double *dreact,int nreact)            //Check MIHARA
						 ...
						 double cangle)
{
  ...
  {
//MessageBox(NULL,str,"Dcos1",MB_OK);
  ...
  //else if((dl2/dl1)<0.00000001) /*DEFORMED COLUMN*/       //for insect
  else if((dl2/dl1)<0.1) /*DEFORMED COLUMN*///checkMIHARA(BuzaiJiku)(Default0.1)
  {
//MessageBox(NULL,str,"Dcos2",MB_OK);
	ox=dx/dl1;
	oy=dy/dl1;
	oz=dz/dl1;
	ol2=ox*ox+oy*oy;
	...
	*(*(drccos+1)+2)=xz/al1;
	...
	if(oz>=0.0) yy=oz*xx-ox*xz; // 150220 fukushima for arclm201
	else yy=oz*xx+ox*xz; // 150220 fukushima for arclm201
	/*yy=oz*xx-ox*xz;*/
	yz=ox*xy-oy*xx;
...
if(globalfile!=NULL)
{
  fprintf(globalfile,"Matrix Singular.ELEM=%d SECT=%d i=%d j=%d\n",
...
if(fu[j]==0.0)
{
sprintf(s,"Fu[%d].ELEM=%ld",j,elem.code);
MessageBox(NULL,s,"Fu",MB_OK);
}
  ...
  for(n=0;n<=1;n++)
  {
    for(i=0;i<6;i++)
    {
         ...
         elem.iconf[n][i]==-3)
      {
        ...
        {
          sprintf(s,"Matrix Singular.ELEM=%ld",elem.code);
...
//          sprintf(string,"Matrix Singular.SECT=%d",elem->sect->code);
...
/*SELECT MULTI ELEMENTS BY RECT.*/
{
  ...
  for(i=0;i<(org->nelem);i++)
  {
       ...
       vp.vflag.ev.etype[(org->elems+i)->type]==1)
    {
      ...
      {
        if(!insiderange(**((org->elems+i)->nods+j),vp.range) ||
           !nodeinsiderect(**((org->elems+i)->nods+j),il,ir,it,ib,vp))
        {
          in=0; /*OUTSIDE*/
        }
      }
...
/*DELETE MULTI ELEMENTS.*/
{
  ...
  for(i=0;i<nelem2;i++)
  {
    /*for(j=0;j>((elems+i)->nnod);j++) */    //Default
    for(j=0;j<((elems+i)->nnod);j++)         //araki
    {
...
int cmqrightangledtriangle(double w0,
                           ...
                           double L1,double L2,double L3,
                           struct cmqelem *cmq)
/*LOAD:TRIANGLE BETWEEN z=L1 AND z=L1+L2 WITH RIGHT ANGLE(=90Åã).*/
...
/*DISTRIBUTE ELEMENT WEIGHT INTO NODE WEIGHT,CMQ.*/
{
  ...
  else if(!strncmp(non,"monz",4)) strcpy(prj,"monz");
  else if(!strncmp(non,"darr",4)) strcpy(prj,"darr");
  else if(!strncmp(non,"nisia",5)) strcpy(prj,"nisia");
  else if(!strncmp(non,"wasestair",9)) strcpy(prj,"wasestair");
  else if(!strncmp(non,"kohi12",6)) strcpy(prj,"kohi12");
  ...
  else if(!strncmp(non,"nono",4)) strcpy(prj,"nono");
  ...
  else if(!strncmp(non,"Itabashi",8)) strcpy(prj,"Itabashi");
  ...
  else if(!strncmp(non,"Siojiri",7))  strcpy(prj,"Siojiri");
  else if(!strncmp(non,"Sio_yoko",8))  strcpy(prj,"Sio_yoko");
  else if(!strncmp(non,"Toyota",6)) strcpy(prj,"Toyota");
  else if(!strncmp(non,"Yokohama",8)) strcpy(prj,"Yokohama");
  else if(!strncmp(non,"Okinawa",7)) strcpy(prj,"Okinawa");
  else if(!strncmp(non,"todo",4)) strcpy(prj,"todo");
  ...
  else if(!strncmp(non,"venetower",9)) strcpy(prj,"venetower");
  else if(!strncmp(non,"venesmall",9)) strcpy(prj,"venesmall");
  ...
  else if(!strncmp(non,"testtest",8)) strcpy(prj,"testtest");
  else if(!strncmp(non,"atami",5)) strcpy(prj,"atami");
  else if(!strncmp(non,"kankikai",8)) strcpy(prj,"kankikai");
  else if(!strncmp(non,"toyo",4)) strcpy(prj,"toyo");
  else if(!strncmp(non,"ita49fe",7)) strcpy(prj,"ita49fe");
  else if(!strncmp(non,"ita51fe",7)) strcpy(prj,"ita51fe");
  else if(!strncmp(non,"ita",3)) strcpy(prj,"ita");
  else if(!strncmp(non,"asak",4)) strcpy(prj,"asak");
  else if(!strncmp(non,"balloonhp",9)) strcpy(prj,"balloonhp");
  else if(!strncmp(non,"hama",4)) strcpy(prj,"hama");
  else if(!strncmp(non,"tenmo",5)) strcpy(prj,"tenmo");
  else if(!strncmp(non,"ucan",4)) strcpy(prj,"ucan");
  ...
  else if(!strncmp(non,"ebi",3)) strcpy(prj,"ebi");
  else if(!strncmp(non,"tuti",4)) strcpy(prj,"tuti");
  else if(!strncmp(non,"toko",4)) strcpy(prj,"toko");
  else if(!strncmp(non,"motoa",5)) strcpy(prj,"motoa");
  ...
  else if(!strncmp(non,"tunesteel",9)) strcpy(prj,"tunesteel");
  else if(!strncmp(non,"tunetimber",10)) strcpy(prj,"tunetimber");
  else if(!strncmp(non,"tune",4)) strcpy(prj,"tune");
  else if(!strncmp(non,"chum",4)) strcpy(prj,"chum");
  else if(!strncmp(non,"uresi",5)) strcpy(prj,"uresi");
  else if(!strncmp(non,"nagomia",7)) strcpy(prj,"nagomia");
  else if(!strncmp(non,"yuki",4)) strcpy(prj,"yuki");
  ...
  else if(!strncmp(non,"maki",4)) strcpy(prj,"maki");
  else if(!strncmp(non,"tuku",4)) strcpy(prj,"tuku");
  else if(!strncmp(non,"balcony",7)) strcpy(prj,"balcony");
  ...
  else if(!strncmp(non,"hachihiba23",11)) strcpy(prj,"hachihiba23");
  else if(!strncmp(non,"hachirouka",10)) strcpy(prj,"hachirouka");
  else if(!strncmp(non,"shang",5)) strcpy(prj,"shang");
  else if(!strncmp(non,"koenji",6)) strcpy(prj,"koenji");
...
*/
    ...
    if(ge->type==WALL)
    {
      ...
      w[WEIGHTSLAB] =weight+ge->sect->lload[WEIGHTSLAB];
	  w[WEIGHTFRAME]=weight+ge->sect->lload[WEIGHTFRAME];
      w[WEIGHTEQ]   =weight+ge->sect->lload[WEIGHTEQ];            //MIHARA
...
//mihara07.11.04////////////////////////////////////////////////////////////////
  if(fout!=NULL)
  {
   if(globalunit==1.0)
   {
   fprintf(fout,"3.2 : êﬂì_èdó \n\n");
   fprintf(fout,"êﬂì_Ç≤Ç∆Ç…èdó ÇèWåvÇµÇΩåãâ ÇãLÇ∑ÅB\n");
   fprintf(fout,"íåÅCï«ÇÕäKçÇÇÃíÜâõÇ≈è„â∫Ç…ï™îzÇ∑ÇÈÇ‡ÇÃÇ∆Ç∑ÇÈÅB\n\n");
   fprintf(fout," êﬂì_î‘çÜ          êœç⁄â◊èdï ÇÃèdó  [tf]\n\n");
   fprintf(fout,"                 è∞óp     íåó¿óp     ínêkóp\n");
   }
   else
   {
   fprintf(fout,"3.2 : êﬂì_èdó \n\n");
   fprintf(fout,"êﬂì_Ç≤Ç∆Ç…èdó ÇèWåvÇµÇΩåãâ ÇãLÇ∑ÅB\n");
   fprintf(fout,"íåÅCï«ÇÕäKçÇÇÃíÜâõÇ≈è„â∫Ç…ï™îzÇ∑ÇÈÇ‡ÇÃÇ∆Ç∑ÇÈÅB\n\n");
   fprintf(fout," êﬂì_î‘çÜ          êœç⁄â◊èdï ÇÃèdó  [kN]\n\n");
   fprintf(fout,"                 è∞óp     íåó¿óp     ínêkóp\n");
   }
  }
////////////////////////////////////////////////////////////////////////////////
//MessageBox(NULL,"Completed6.","Move Node with Safety",MB_OK);

  total[WEIGHTSLAB] =0.0;
  total[WEIGHTFRAME]=0.0;
  total[WEIGHTEQ]   =0.0;
//MessageBox(NULL,"Completed6-1.","Move Node with Safety",MB_OK);
  for(i=0;i<(org->nelem);i++) /*ADD Q OF CMQ.*/
  {
//if(i==0) MessageBox(NULL,"Completed6-1-1.","Move Node with Safety",MB_OK);
	total[WEIGHTFRAME]+=(org->elems+i)->initial[0][2];
	total[WEIGHTFRAME]+=(org->elems+i)->initial[1][2];
//if(i==0) MessageBox(NULL,"Completed6-1-2.","Move Node with Safety",MB_OK);
...
//if(i==0) MessageBox(NULL,"Completed6-1-3.","Move Node with Safety",MB_OK);
  }
//MessageBox(NULL,"Completed6-2.","Move Node with Safety",MB_OK);
  for(i=0;i<(org->nnode);i++) /*ADD NODE LOADS.*/
  {
	...
	(nload+i)->mass    =(nload+i)->w[WEIGHTEQ];
...
/////////////////////////////////////////////////////////////////////////////////
                                                     /*TAKATA MODIFY 2007/01/25*/
  for(i=0;i<(org->nsect);i++)
  {
	if(!strncmp(prj,"Odawara",7))
    {
      if(((org->sects+i)->code)==701 || ((org->sects+i)->code)==702 || ((org->sects+i)->code)==703)
      {
		baseslab+=*(volumes+i);
      }
    }
  ...
  if(baseslab!=0.0)
  {
    sprintf(non,"\nTOTAL WEIGHT FOR SLAB=%.3f,FRAME=%.3f(REACTION=%.4f,A=%.3f),EQ=%.3f",
            globalunit*total[WEIGHTSLAB],
			globalunit*total[WEIGHTFRAME],
            (globalunit*total[WEIGHTFRAME]/baseslab),
            baseslab,
...
/////////////////////////////////////////////////////////////////////////////////
  ...
  if(fout!=NULL)
  {
	...
	fprintf(fout,"\näeífñ ÇÃïîçﬁëçó ÅiéQçléëóøÅj\n");
	fprintf(fout,"\n ífñ î‘çÜ   í∑Ç≥,ñ êœ[m,m2]\n");

	for(i=0;i<(org->nsect);i++)
	{
	  fprintf(fout,"      %d %9.3f    Åy%sÅz\n",
			  (org->sects+i)->code,*(volumes+i),(org->sects+i)->name);
	}
  }

//Calculating and printing the quantity 20170604 mihara/////////////////////////
  if(fout!=NULL)
  {
	double sumsteel    =0.0;
	double sumconcrete =0.0;
	double sumtimber   =0.0;
	double losssteel   =1.05;
	double lossconcrete=1.1;
	double losstimber  =1.0;

	/*fprintf(fout,"\nVOLUME OF EACH SECTIONS\n");*/
	fprintf(fout,"\nâêÕÉÇÉfÉãêîó ï\ÅiéQçléëóøÅjÅFåvéZèëÇ…ÇÕç⁄ÇπÇ»Ç¢Ç±Ç∆ÅIÅIÅIÅIÅIÅIÅIÅI\n");
	fprintf(fout,"\n ífñ î‘çÜ   í∑Ç≥,ñ êœ[m,m2] ífñ êœ,å˙Ç≥[m2,m] ì¸óÕî‰èd/é¿î‰èd ÉçÉXó¶ èdó ,ëÃêœ[tf,m3]\n");

	for(i=0;i<(org->nsect);i++)
	{
	  if((org->sects+i)->code<700)/*COULMN,GIRDER,BRACE*/
	  {
		if(((org->sects+i)->figs+0)->prop->code<200)/*Steel weight*/
		//î‰èdí≤êÆÇ…ÇÊÇÈí·å∏çló∂
		{
		  fprintf(fout,"      %d %9.3f %14.4f %16.3f %14.2f %10.3f  Steel    Åy%sÅz\n",
			  (org->sects+i)->code,*(volumes+i),
			  ((org->sects+i)->figs+0)->area,(((org->sects+i)->figs+0)->prop->hiju)/7.8,losssteel,
			  ((*(volumes+i))*(((org->sects+i)->figs+0)->area)*((((org->sects+i)->figs+0)->prop->hiju)/7.8)*7.8*losssteel),
			  (org->sects+i)->name);
		  sumsteel+=((*(volumes+i))*(((org->sects+i)->figs+0)->area)*((((org->sects+i)->figs+0)->prop->hiju)/7.8)*7.8*losssteel);
		}
		else if(((org->sects+i)->figs+0)->prop->code<300)/*Concrete volume*/
		//î‰èdí≤êÆÇ…ÇÊÇÈí·å∏çló∂ÅCåyó ÉRÉìÉNÉäÅ[ÉgÇÃèÍçáóvíçà”
		{
		  fprintf(fout,"      %d %9.3f %14.4f %16.3f %14.2f %10.3f  Concrete Åy%sÅz\n",
			  (org->sects+i)->code,*(volumes+i),
			  ((org->sects+i)->figs+0)->area,(((org->sects+i)->figs+0)->prop->hiju)/2.4,lossconcrete,
			  ((*(volumes+i))*(((org->sects+i)->figs+0)->area)*((((org->sects+i)->figs+0)->prop->hiju)/2.4)*lossconcrete),
			  (org->sects+i)->name);
		  sumconcrete+=((*(volumes+i))*(((org->sects+i)->figs+0)->area)*((((org->sects+i)->figs+0)->prop->hiju)/2.4)*lossconcrete);
		}
		else if(((org->sects+i)->figs+0)->prop->code<400)/*Timber volume*/
		//î‰èdí≤êÆÇ…ÇÊÇÈí·å∏ñ¢çló∂
		{
		  fprintf(fout,"      %d %9.3f %14.4f                  %14.2f %10.3f  Timber   Åy%sÅz\n",
			  (org->sects+i)->code,*(volumes+i),
			  ((org->sects+i)->figs+0)->area,losstimber,
			  ((*(volumes+i))*(((org->sects+i)->figs+0)->area)*losstimber),
			  (org->sects+i)->name);
		  sumtimber+=((*(volumes+i))*(((org->sects+i)->figs+0)->area)*losstimber);
		}
	  }
	  else/*SLAB,WALL*/
	  {
		double sumthicksteel    =0.0;
		double sumthickconcrete =0.0;
		double sumthicktimber   =0.0;

		for(j=0;j<(org->sects+i)->nfig;j++)
		{
		  if(((org->sects+i)->figs+j)->prop->code<200)/*Steel*/
		  {
			sumthicksteel += ((org->sects+i)->figs+j)->thick;
		  }
		  else if(((org->sects+i)->figs+j)->prop->code<300)/*Concrete*/
		  {
			sumthickconcrete += ((org->sects+i)->figs+j)->thick;
		  }
		  else if(((org->sects+i)->figs+j)->prop->code<400)/*Timber*/
		  {
			sumthicktimber += ((org->sects+i)->figs+j)->thick;
		  }
		}

		//sectíÜÇ…ìSçúÅCÉRÉìÉNÉäÅ[ÉgÅCñÿÇ™ç¨ç›ÇµÇƒÇ¢ÇÈÇ∆ï°êîçsèëÇ´èoÇ≥ÇÍÇÈÇÃÇ≈íçà”
		if(sumthicksteel!=0.0)
		//î‰èdí≤êÆÇ…ÇÊÇÈí·å∏ñ¢çló∂
		{
		  fprintf(fout,"      %d %9.3f %14.4f                  %14.2f %10.3f  Steel    Åy%sÅz\n",
			  (org->sects+i)->code,*(volumes+i),
			  sumthicksteel,losssteel,
			  ((*(volumes+i))*sumthicksteel*7.8*losssteel),
			  (org->sects+i)->name);
		  sumsteel+=((*(volumes+i))*sumthicksteel*7.8*losssteel);
		}
		if(sumthickconcrete!=0.0)
		//î‰èdí≤êÆÇ…ÇÊÇÈí·å∏ñ¢çló∂
		{
		  fprintf(fout,"      %d %9.3f %14.4f                  %14.2f %10.3f  Concrete Åy%sÅz\n",
			  (org->sects+i)->code,*(volumes+i),
			  sumthickconcrete,lossconcrete,
			  ((*(volumes+i))*sumthickconcrete*lossconcrete),
			  (org->sects+i)->name);
		  sumconcrete+=((*(volumes+i))*sumthickconcrete*lossconcrete);
		}
		if(sumthicktimber!=0.0)
		//î‰èdí≤êÆÇ…ÇÊÇÈí·å∏ñ¢çló∂
		{
		  fprintf(fout,"      %d %9.3f %14.4f                  %14.2f %10.3f  Timber   Åy%sÅz\n",
			  (org->sects+i)->code,*(volumes+i),
			  sumthicktimber,losstimber,
			  ((*(volumes+i))*sumthicktimber*losstimber),
			  (org->sects+i)->name);
		  sumtimber+=((*(volumes+i))*sumthicktimber*losstimber);
		}
		if(sumthicksteel==0.0&&sumthickconcrete==0.0&&sumthicktimber==0.0)
		{
		  fprintf(fout,"      %d %9.3f                                                                     Åy%sÅz\n",
		  (org->sects+i)->code,*(volumes+i),(org->sects+i)->name);
	...
	///*
	fprintf(fout,"\nêîó ÇÃçáåv\n");
	if(!strncmp(prj,"osaka",5))
	{
	  fprintf(fout," íçà”ÅFñ∆êkëwâ∫ÇÃäÓëbñ¢éZì¸\n");
	}
	fprintf(fout," ìSçúó          %9.3f tf (ÉçÉXó¶%3.2fçû)\n",sumsteel,losssteel);
	fprintf(fout," ÉRÉìÉNÉäÅ[Égó  %9.3f m3 (ÉçÉXó¶%3.2fçû)\n",sumconcrete,lossconcrete);
	fprintf(fout," ñÿçﬁó          %9.3f m3 (ÉçÉXó¶%3.2fçû)\n",sumtimber,losstimber);
	//*/
  }
////////////////////////////////////////////////////////////////////////////////
...
//  if(ai.Co==0.0) ai.Co=0.300; /*BASE SHEAR 0.2,0.3,1.0*/     //mihara for vene
...
if(!strncmp(prj,"monz",4))
{
  ai.nfloor=11;
}
if(!strncmp(prj,"darr",4))
{
  ai.nfloor=4;
}
if(!strncmp(prj,"nisia",5))
{
  ai.nfloor=3;
}
if(!strncmp(prj,"hiroi",5))
{
  ai.nfloor=3;
}
if(!strncmp(prj,"kiryu",5))
{
  ai.nfloor=2;
}
if(!strncmp(prj,"Tachikawa",9))
{
  ai.nfloor=6;
}
if(!strncmp(prj,"Container",9))
{
  ai.nfloor=6;
}
if(!strncmp(prj,"Itabashi",8))
{
  ai.nfloor=6;
}
...
if(!strncmp(prj,"Flytower",8))
{
  ai.nfloor=7;
}
if(!strncmp(prj,"Aoyama",6))
{
  ai.nfloor=10;
}
if(!strncmp(prj,"tsurumi",7))
{
  ai.nfloor=6;
}
if(!strncmp(prj,"Siojiri",7))
{
  ai.nfloor=3;
}
if(!strncmp(prj,"Sio_yoko",8))
{
  ai.nfloor=4;
}

if(!strncmp(prj,"Toyota",6))
{
  ai.nfloor=3;
}

if(!strncmp(prj,"Yokohama",8))
{
  ai.nfloor=8;
}

if(!strncmp(prj,"Okinawa",7))
{
  ai.nfloor=9;
  //ai.nfloor=7;
}

if(!strncmp(prj,"todo",4))
{
  ai.nfloor=10;
}

if(!strncmp(prj,"nogi",4))
{
  ai.nfloor=9;
}
...
if(!strncmp(prj,"naga",4))
{
  ai.nfloor=4;
}

if(!strncmp(prj,"kanko56test",11))
{
  ai.nfloor=2;
}

if(!strncmp(prj,"kanko57S",8))
{
  ai.nfloor=2;
}

if(!strncmp(prj,"kanko57",7))
{
  ai.nfloor=2;
}

if(!strncmp(prj,"kanko",5))
{
  ai.nfloor=2;
}

if(!strncmp(prj,"yoga",4))
{
  ai.nfloor=2;
}

if(!strncmp(prj,"testtest",8))
{
  ai.nfloor=3;
}

if((!strncmp(prj,"atami",5))||(!strncmp(prj,"atamijogedo",11)))
{
  ai.nfloor=4;
}

if(!strncmp(prj,"kankikai",8))
{
  ai.nfloor=2;
}
...
if(!strncmp(prj,"venedome",8))
{
  ai.nfloor=2;
}

if(!strncmp(prj,"venetower",9))
{
  ai.nfloor=21;
}

if(!strncmp(prj,"venesmall",9))
{
  ai.nfloor=2;
}

if(!strncmp(prj,"vene",3))
{
  ai.nfloor=2;
}

if(!strncmp(prj,"ita49fe",7))
{
  ai.nfloor=4;
}

if(!strncmp(prj,"ita51fe",7))
{
  ai.nfloor=4;
}

if(!strncmp(prj,"ita",3))
{
  ai.nfloor=4;
}

if(!strncmp(prj,"asak",4))
{
  ai.nfloor=8;
}

if(!strncmp(prj,"hama",4))
{
  ai.nfloor=5;
}

if(!strncmp(prj,"nagaoka",7))
{
  ai.nfloor=7;
}

if(!strncmp(prj,"tenmo",5))
{
  ai.nfloor=14;
}

if(!strncmp(prj,"ucan",4))
{
  ai.nfloor=6;
}

if(!strncmp(prj,"gunma",5))
{
  ai.nfloor=2;
}

if(!strncmp(prj,"himon",5))
{
  ai.nfloor=4;
}

else if(!strncmp(prj,"himo",4))
{
  ai.nfloor=5;
}
...
if(!strncmp(prj,"uresi",5))
{
  ai.nfloor=4;
}

if(!strncmp(prj,"nagomia",7))
{
  ai.nfloor=2;
}

if(!strncmp(prj,"yuki",4))
{
  ai.nfloor=2;
}
...
if(!strncmp(prj,"wasestair",9))
{
  ai.nfloor=2;
}

if(!strncmp(prj,"ginhu",5))
{
  ai.nfloor=14;
}

if(!strncmp(prj,"maki",4))
{
  ai.nfloor=4;
}

if(!strncmp(prj,"tuku",4))
{
  ai.nfloor=3;
}

if(!strncmp(prj,"balcony",7))
{
  ai.nfloor=2;
}

if(!strncmp(prj,"icnew",5))
{
  ai.nfloor=5;
}

if(!strncmp(prj,"koenji",6))
{
  ai.nfloor=3;
}
...
if(!strncmp(prj,"osaka",5))
{
  ...
  ai.Ci    =(double *)malloc(ai.nfloor*sizeof(double));
  ai.Qi    =(double *)malloc(ai.nfloor*sizeof(double));
  ai.Hi    =(double *)malloc(ai.nfloor*sizeof(double));
  ai.facts =(double *)malloc(ai.nfloor*sizeof(double));
  ...
  else if(!strncmp(prj,"kiryu",5))
  {
    *(ai.lbound+ 0)= -5.0;
    *(ai.lbound+ 1)=  0.1;
    *(ai.lbound+ 2)=  4.0;
  }
  else if(!strncmp(prj,"nono",4))
  {
    *(ai.lbound+ 0)= -6.0;
    *(ai.lbound+ 1)=  1.0;
	*(ai.lbound+ 2)=  3.0;
  ...
  else if(!strncmp(prj,"Tachikawa",9))   //Imai Team
  {
    *(ai.lbound+ 0)= -5.0;
    *(ai.lbound+ 1)=  2.0;
    *(ai.lbound+ 2)=  6.0;
  ...
  else if(!strncmp(prj,"Container",9))
  {
    *(ai.lbound+ 0)= -5.0;
    *(ai.lbound+ 1)=  1.0;
    *(ai.lbound+ 2)=  3.0;
  ...
  else if(!strncmp(prj,"Itabashi",8))
  {
    *(ai.lbound+ 0)= -5.0;
    *(ai.lbound+ 1)=  1.0;
    *(ai.lbound+ 2)=  4.0;
  ...
  else if(!strncmp(prj,"Flytower",8))
  {
    *(ai.lbound+ 0)=-10.0;
    *(ai.lbound+ 1)=  0.5;
	*(ai.lbound+ 2)=  5.0;
  ...
  else if(!strncmp(prj,"Sio_yoko",8))
  {
    *(ai.lbound+ 0)= -1.000;
    *(ai.lbound+ 1)=  0.500;
    *(ai.lbound+ 2)=  6.500;
  ...
  else if(!strncmp(prj,"Yokohama",8))
  {
    *(ai.lbound+ 0)= -1.0;
    *(ai.lbound+ 1)=  0.1;
    *(ai.lbound+ 2)=  0.6;
  ...
  else if(!strncmp(prj,"kanko56test",11))
  {
    *(ai.lbound+ 0)= -3.0;
    *(ai.lbound+ 1)=  0.1;
	*(ai.lbound+ 2)= 50.0;
  ...
  else if(!strncmp(prj,"kanko57S",8))
  {
	*(ai.lbound+ 0)= -3.0;
    *(ai.lbound+ 1)= 1.63;
    *(ai.lbound+ 2)= 50.0;
  ...
  else if(!strncmp(prj,"testtest",8))
  {
    *(ai.lbound+ 0)= -1.0;
    *(ai.lbound+ 1)=  2.0;
	*(ai.lbound+ 2)=  5.0;
  ...
  else if((!strncmp(prj,"atami",5))||(!strncmp(prj,"atamijogedo",11)))
  {
    *(ai.lbound+ 0)= -8.0;
    *(ai.lbound+ 1)= -1.0;
    *(ai.lbound+ 2)=  2.0;
  ...
  else if(!strncmp(prj,"kankikai",8))
  {
	*(ai.lbound+ 0)= -3.0;
    *(ai.lbound+ 1)=  0.1;
    *(ai.lbound+ 2)= 50.0;
  ...
  else if(!strncmp(prj,"toyo",4))
  {
	...
	*(ai.lbound+ 3)= 15.2;
    ...
    else if(!strncmp(prj,"venedome",8))
  {
	*(ai.lbound+ 0)= -10.0;
    *(ai.lbound+ 1)=   0.1;
    *(ai.lbound+ 2)=  10.0;
  ...
  else if(!strncmp(prj,"venetower",9))
  {
    *(ai.lbound+ 0)=   -10.0;
    *(ai.lbound+ 1)=     0.1;
    *(ai.lbound+ 2)=    0.71;
  ...
  else if(!strncmp(prj,"venesmall",9))
  {
    *(ai.lbound+ 0)= -10.0;
    *(ai.lbound+ 1)=   0.1;
	*(ai.lbound+ 2)=  10.0;
  ...
  else if(!strncmp(prj,"yamagatei40",11))
  {
	*(ai.lbound+ 0)= -3.0;
	*(ai.lbound+ 1)=  0.1;
	*(ai.lbound+ 2)= 50.0;
  ...
  else if(!strncmp(prj,"yamagatei",9))
  {
	*(ai.lbound+ 0)= -3.0;
	*(ai.lbound+ 1)=  0.1;
	*(ai.lbound+ 2)= 50.0;
  ...
  else if(!strncmp(prj,"wasestair",9))
  {
	*(ai.lbound+ 0)= -10.0;
	*(ai.lbound+ 1)= -1.0;
	*(ai.lbound+ 2)= 100.0;
...
/*FOR KOKUJIMENSHIN MODIFIED BY MIHARA///////////////////////////////////////*/

if(!strncmp(prj,"hakone",6))
{
  (*(ai.Ci+1))=0.1280;       //0.1221   //0.
  (*(ai.Ci+2))=0.1330;       //0.1225
  (*(ai.Ci+3))=0.1370;       //0.1265
  (*(ai.Ci+4))=0.1380;       //0.1298
  (*(ai.Ci+5))=0.1400;       //0.1309
  ...
  for(i=1;i<ai.nfloor;i++)
  {
    ...
    (*(ai.Qi+i))=1.25*(*(ai.Ci+i))*(*(ai.Wi+i));    /*èdóvìxåWêî1.25*/

    if(i>=2)
    {
      (*(ai.Hi+i-1))=(*(ai.Qi+i-1))-(*(ai.Qi+i));

      if(*(ai.wi+i-1)!=0.0)
      {
        (*(ai.facts+i-1))=(*(ai.Hi+i-1))/(*(ai.wi+i-1));
      }
      else (*(ai.facts+i-1))=0.0;
    }
  }
}
...
if(i==1)
{
    (*(ai.Ci+i))=1.000*(*(ai.Ai+i))*(ai.Z)*(ai.Rt)*(ai.Co);       //1.447
}
else if(i==2)
{
    (*(ai.Ci+i))=1.000*(*(ai.Ai+i))*(ai.Z)*(ai.Rt)*(ai.Co);       //1.210
}
else
{
    (*(ai.Ci+i))=(*(ai.Ai+i))*(ai.Z)*(ai.Rt)*(ai.Co);
}

    (*(ai.Qi+i))=(*(ai.Ci+i))*(*(ai.Wi+i));
...
if(i==1)
{
    (*(ai.Ci+i))=1.334*(*(ai.Ai+i))*(ai.Z)*(ai.Rt)*(ai.Co);       //1.334
}
else if(i==2)
{
    (*(ai.Ci+i))=1.000*(*(ai.Ai+i))*(ai.Z)*(ai.Rt)*(ai.Co);       //1.000
}
else
{
    (*(ai.Ci+i))=(*(ai.Ai+i))*(ai.Z)*(ai.Rt)*(ai.Co);
...
else if(!strncmp(prj,"venetower",9))
{
  for(i=1;i<ai.nfloor;i++)
  {
    ...
    (*(ai.Ci+i))=(*(ai.Ai+i))*(ai.Co);
    (*(ai.Qi+i))=1.8*0.7*0.04;
...
else
{
  ...
  (*(ai.Hi+(ai.nfloor-1)))=(*(ai.Qi+(ai.nfloor-1)));
  if(*(ai.wi+(ai.nfloor-1))!=0.0)
  {
    (*(ai.facts+(ai.nfloor-1)))=(*(ai.Hi+(ai.nfloor-1)))
                               /(*(ai.wi+(ai.nfloor-1)));
  }
  else (*(ai.facts+(ai.nfloor-1)))=0.0;
  (*(ai.facts+0))=ai.Cf;
  ...
  if(fout!=NULL)
  {
    ...
    fprintf(fout,"\n3.3 : Aiï™ïzå^ínêkâ◊èd\n\n");
    fprintf(fout,"êÖïΩâ◊èdÇÕåöízäÓèÄñ@é{çsóﬂëÊ88èÇ®ÇÊÇ—åöê›è»çêé¶1793çÜÇ…è]Ç¢ÅAÇ`iï™ïzå^ÇÃínêkóÕÇ∆Ç∑ÇÈÅB\n\n");

//    fprintf(fout,"Floors     n =%d\n",ai.nfloor);
//    fprintf(fout,"Height     H =%.3f\n",ai.hmax);

    fprintf(fout,"äKêî       Å@Å@Å@    n =%d\n",ai.nfloor);
    fprintf(fout,"çÇÇ≥         Å@Å@Å@  H =%.3f\n",ai.hmax);

//    if(ai.hmax!=0.0) fprintf(fout,"Period     T1=%.3fH=%.3f\n",(ai.T1/ai.hmax),ai.T1);
//    else             fprintf(fout,"Period     T1       =%.3f\n",ai.T1);
    if(ai.hmax!=0.0) fprintf(fout,"ÇPéüå≈óLé¸ä˙         T1=%.3fH=%.3f\n",(ai.T1/ai.hmax),ai.T1);
    else             fprintf(fout,"ÇPéüå≈óLé¸ä˙         T1       =%.3f\n",ai.T1);
/*
...
*/
    fprintf(fout,"ínî’é¸ä˙             Tc=%.3f\n",ai.Tc);
    fprintf(fout,"êUìÆì¡ê´åWêî         Rt=%.3f\n",ai.Rt);
    fprintf(fout,"ínàÊåWêî             Z =%.3f\n",ai.Z);
    fprintf(fout,"ïWèÄëwÇπÇÒífóÕåWêî   Co=%.3f\n",ai.Co);
    fprintf(fout,"äÓëbïîï™ÇÃêkìx       Cf=%.3f\n",ai.Cf);

/*by Mihara for Kokuji Menshin////////////////////////////////////////////*/
if(!strncmp(prj,"hakone",6))
{
    fprintf(fout,"èdóvìxåWêî   =1.25\n");
}
/*////////////////////////////////////////////////////////////////////////*/
...
/*////////////////////////////////////////////////////////////////////////*/
  ...
  if(globalunit==1.0)
  {
    fprintf(fout,"\nè∞ópêœç⁄â◊èdÇ…ÇÊÇÈëçèdó      = %.3f [tf]\n",      //mihara
            globalunit*total[WEIGHTSLAB]);
    fprintf(fout,"íåó¿ópêœç⁄â◊èdÇ…ÇÊÇÈëçèdó    = %.3f [tf]\n",
            globalunit*total[WEIGHTFRAME]);
    fprintf(fout,"ínêkópêœç⁄â◊èdÇ…ÇÊÇÈëçèdó    = %.3f [tf]\n",
            globalunit*total[WEIGHTEQ]);
  }
  if(globalunit==SIUNIT)
  {
    fprintf(fout,"\nè∞ópêœç⁄â◊èdÇ…ÇÊÇÈëçèdó      = %.3f [kN]\n",      //mihara
            globalunit*total[WEIGHTSLAB]);
    fprintf(fout,"íåó¿ópêœç⁄â◊èdÇ…ÇÊÇÈëçèdó    = %.3f [kN]\n",
            globalunit*total[WEIGHTFRAME]);
    fprintf(fout,"ínêkópêœç⁄â◊èdÇ…ÇÊÇÈëçèdó    = %.3f [kN]\n",
            globalunit*total[WEIGHTEQ]);
  }

  
  }
  sprintf(ss1,"äeäKïΩãœçÇÇ≥      :");
  sprintf(ss2,"äeäKèdó        wi :");
  sprintf(ss3,"        Wi = É∞wi :");
  sprintf(ss4,"               Ai :           ");

/*by Mihara for Kokuji Menshin////////////////////////////////////////////*/
if(!strncmp(prj,"hakone",6)||!strncmp(prj,"osaka",5))
{
  sprintf(ss5,"çêé¶ñ∆êkópëwÇπÇÒífóÕåWêî Ci : ");
}
/*////////////////////////////////////////////////////////////////////////*/

else
{
  sprintf(ss5,"ëwÇπÇÒífóÕåWêî Ci :           ");
}
  sprintf(ss6,"ëwÇπÇÒífóÕ     Qi :           ");
  sprintf(ss7,"äeäKäOóÕ       Hi :           ");
  sprintf(ss8,"äOóÕåWêî    Hi/wi :");
  ...
  for(i=0;i<ai.nfloor;i++)
  {
    ...
    if(i>0)
    {
      ...
      strcat(ss4,ss);
      sprintf(ss," %10.3f",*(ai.Ci+i));
      strcat(ss5,ss);
      sprintf(ss," %10.3f",globalunit*(*(ai.Qi+i)));
      strcat(ss6,ss);
      sprintf(ss," %10.3f",globalunit*(*(ai.Hi+i)));
      strcat(ss7,ss);
    }
    sprintf(ss," %10.3f",*(ai.facts+i));
    strcat(ss8,ss);
  }
...
/*DISTRIBUTE ELEMENT WEIGHT INTO NODE WEIGHT,CMQ.*/
{
  ...
  /*PROJECT NAME*/
  GetDlgItemText((wmenu.childs+2)->hwnd,ID_SECTIONFILE,non,80);
  if(!strncmp(non,"tune",4)) strcpy(prj,"tune");
  else                            strcpy(prj,"none");
...
//  if(ai.Co==0.0) ai.Co=0.300; /*BASE SHEAR 0.2,0.3,1.0*/     //mihara for vene
  ...
  ai.xbound=(double *)malloc((ai.nx+1)*sizeof(double));
  ai.ybound=(double *)malloc((ai.ny+1)*sizeof(double));
  ai.nfloor=(int *)malloc(ai.nx*ai.ny*sizeof(int));
  ai.hmax  =(double *)malloc(ai.nx*ai.ny*sizeof(double));
  ai.T1    =(double *)malloc(ai.nx*ai.ny*sizeof(double));
  ai.Rt    =(double *)malloc(ai.nx*ai.ny*sizeof(double));
  ai.sumfloor=0;

  for(i=0;i<ai.nx*ai.ny;i++)
  {
	/*GetDlgItemText((wmenu.childs+4)->hwnd,IDVS_PFACT,non,80);*/
	/*ai.T1=strtod(non,NULL);*/
	*(ai.T1+i)=org->ai.T1; /*PERIOD FACTOR*/
	if(*(ai.T1+i)==0.0) *(ai.T1+i)=0.03;
  }
  ...
  if(!strncmp(prj,"tune",4))
  {
	...
	*(ai.ybound+ 0)= -1000.0;
	*(ai.ybound+ 1)=    22.0;     /*22.0*//*-10.0*//*8.0*/
	*(ai.ybound+ 2)=    23.0;
	*(ai.ybound+ 3)=    39.0;
	*(ai.ybound+ 4)=    56.0;
	*(ai.ybound+ 5)=    72.0;
  ...
  for(i=0;i<(ai.nx*ai.ny);i++)
  {
	ai.sumfloor += *(ai.nfloor+i);
  }

//sprintf(non,"sumfloor=%d",ai.sumfloor);    /*Debug*/
//MessageBox(NULL,non,"Distribute",MB_OK);

//for(ii=0;ii<(ai.nx*ai.ny);ii++)/*loop1*/
//{
  /*ai.hmax=0.0;
  ai.fnames=(char **)malloc(ai.nfloor*sizeof(char *));
  ai.nnode =(int *)malloc(ai.nfloor*sizeof(int));
  ...
  ai.fnames=(char **)malloc(ai.sumfloor*sizeof(char *));
  ai.nnode =(int *)malloc(ai.sumfloor*sizeof(int));
  ai.lbound=(double *)malloc((ai.sumfloor+(ai.nx*ai.ny))*sizeof(double));
  ai.levels=(double *)malloc(ai.sumfloor*sizeof(double));
  ai.wi    =(double *)malloc(ai.sumfloor*sizeof(double));
  ai.Wi    =(double *)malloc(ai.sumfloor*sizeof(double));
  ai.Ai    =(double *)malloc(ai.sumfloor*sizeof(double));
  ai.Cf    =(double *)malloc(ai.sumfloor*sizeof(double));
  ai.Ci    =(double *)malloc(ai.sumfloor*sizeof(double));
  ai.Qi    =(double *)malloc(ai.sumfloor*sizeof(double));
  ai.Hi    =(double *)malloc(ai.sumfloor*sizeof(double));
  ai.facts =(double *)malloc(ai.sumfloor*sizeof(double));

  /*ai.hmax=0.0;
  ai.fnames=(char **)malloc(*(ai.nfloor+ii)*sizeof(char *));
  ai.nnode =(int *)malloc(*(ai.nfloor+ii)*sizeof(int));
  ai.lbound=(double *)malloc((*(ai.nfloor+ii)+1)*sizeof(double));
  ai.levels=(double *)malloc(*(ai.nfloor+ii)*sizeof(double));
  ai.wi    =(double *)malloc(*(ai.nfloor+ii)*sizeof(double));
  ai.Wi    =(double *)malloc(*(ai.nfloor+ii)*sizeof(double));
  ai.Ai    =(double *)malloc(*(ai.nfloor+ii)*sizeof(double));
  ai.Ci    =(double *)malloc(*(ai.nfloor+ii)*sizeof(double));
  ai.Qi    =(double *)malloc(*(ai.nfloor+ii)*sizeof(double));
  ai.Hi    =(double *)malloc(*(ai.nfloor+ii)*sizeof(double));
  ai.facts =(double *)malloc(*(ai.nfloor+ii)*sizeof(double));*/

  piv1=0;

  for(ii=0;ii<ai.nx*ai.ny;ii++)
  {
	for(i=0;i<*(ai.nfloor+ii);i++)
	{
	  *(ai.fnames+piv1)=(char *)malloc(80*sizeof(char));
	  sprintf(*(ai.fnames+piv1),"%dFL",i+1); /*FLOOR NAMES*/

//sprintf(non,"floorname=%s",*(ai.fnames+piv1));    /*Debug*/
//MessageBox(NULL,non,"Distribute",MB_OK);
  ...
  if(!strncmp(prj,"tune",4))
  {
	*(ai.lbound+  0)=    -0.5; /*GX:-1000.0Å`1000.0,GY:-1000.0Å`6.0*/
	*(ai.lbound+  1)=     9.0;
	*(ai.lbound+  2)=    13.0;
	*(ai.lbound+  3)=    40.0;
	*(ai.lbound+  4)=    50.0;

	*(ai.lbound+  5)=    -0.5; /*GX:-1000.0Å`1000.0,GY:6.0Å`21.0*/
	*(ai.lbound+  6)=    13.0;
	*(ai.lbound+  7)=    17.0;
	*(ai.lbound+  8)=    20.0;
	*(ai.lbound+  9)=    50.0;

	*(ai.lbound+ 10)=    -0.5; /*GX:-1000.0Å`1000.0,GY:21.0Å`39.0*/
	*(ai.lbound+ 11)=    17.0;
	*(ai.lbound+ 12)=    20.7;
	*(ai.lbound+ 13)=    24.0;
	*(ai.lbound+ 14)=    50.0;

	*(ai.lbound+ 15)=    -0.5; /*GX:-1000.0Å`1000.0,GY:39.0Å`56.0*/
	*(ai.lbound+ 16)=    23.0;
	*(ai.lbound+ 17)=    26.0;
	*(ai.lbound+ 18)=    29.0;
	*(ai.lbound+ 19)=    50.0;

	*(ai.lbound+ 20)=    -0.5; /*GX:-1000.0Å`1000.0,GY:56.0Å`73.0*/
	*(ai.lbound+ 21)=    24.0;
	*(ai.lbound+ 22)=    28.0;
	*(ai.lbound+ 23)=    31.0;
	*(ai.lbound+ 24)=    34.0;
	...
	*(ai.lbound+ 26)=    -0.5; /*GX:-1000.0Å`1000.0,GY:73.0Å`87.0*/
	*(ai.lbound+ 27)=    30.0;
	*(ai.lbound+ 28)=    34.0;
	*(ai.lbound+ 29)=    37.0;
	*(ai.lbound+ 30)=    50.0;

	*(ai.lbound+ 31)=    -0.5; /*GX:-1000.0Å`1000.0,GY:87.0Å`1000.0*/
	*(ai.lbound+ 32)=    23.1;
	*(ai.lbound+ 33)=    27.0;
	*(ai.lbound+ 34)=    30.0;
	*(ai.lbound+ 35)=    50.0;
  ...
  {
	*(ai.lbound+  0)=    -0.5; /*GX:-1000.0Å`10.0,GY:-1000.0Å`10.0*/
	*(ai.lbound+  1)=     0.1;
	*(ai.lbound+  2)=     4.0;
	*(ai.lbound+  3)=     7.0;
	*(ai.lbound+  4)=    50.0;

	*(ai.lbound+  5)=    -0.5; /*GX:10.0Å`20.0,GY:-1000.0Å`10.0*/
	*(ai.lbound+  6)=     0.1;
	*(ai.lbound+  7)=     4.0;
	*(ai.lbound+  8)=     7.0;
	*(ai.lbound+  9)=    50.0;

	*(ai.lbound+ 10)=    -0.5; /*GX:20.0Å`1000.0,GY:-1000.0Å`10.0*/
	*(ai.lbound+ 11)=     0.1;
	*(ai.lbound+ 12)=     4.0;
	*(ai.lbound+ 13)=     7.0;
	*(ai.lbound+ 14)=    50.0;

	*(ai.lbound+ 15)=    -0.5; /*GX:-1000.0Å`10.0,GY:10.0Å`20.0*/
	*(ai.lbound+ 16)=     0.1;
	*(ai.lbound+ 17)=     4.0;
	*(ai.lbound+ 18)=     7.0;
	*(ai.lbound+ 19)=    50.0;

	*(ai.lbound+ 20)=    -0.5; /*GX:10.0Å`20.0,GY:10.0Å`20.0*/
	*(ai.lbound+ 21)=     0.1;
	*(ai.lbound+ 22)=     4.0;
	*(ai.lbound+ 23)=     7.0;
	*(ai.lbound+ 24)=    50.0;

	*(ai.lbound+ 25)=    -0.5; /*GX:20.0Å`1000.0,GY:10.0Å`20.0*/
	*(ai.lbound+ 26)=     0.1;
	*(ai.lbound+ 27)=     4.0;
	*(ai.lbound+ 28)=     7.0;
	*(ai.lbound+ 29)=    50.0;

	*(ai.lbound+ 30)=    -0.5; /*GX:-1000.0Å`10.0,GY:20.0Å`1000.0*/
	*(ai.lbound+ 31)=     0.1;
	*(ai.lbound+ 32)=     4.0;
	*(ai.lbound+ 33)=     7.0;
	*(ai.lbound+ 34)=    50.0;

	*(ai.lbound+ 35)=    -0.5; /*GX:10.0Å`20.0,GY:20.0Å`1000.0*/
	*(ai.lbound+ 36)=     0.1;
	*(ai.lbound+ 37)=     4.0;
	*(ai.lbound+ 38)=     7.0;
	*(ai.lbound+ 39)=    50.0;

	*(ai.lbound+ 40)=    -0.5; /*GX:20.0Å`1000.0,GY:20.0Å`1000.0*/
	*(ai.lbound+ 41)=     0.1;
	*(ai.lbound+ 42)=     4.0;
	*(ai.lbound+ 43)=     7.0;
	*(ai.lbound+ 44)=    50.0;
  ...
  for(i=0;i<(org->nnode);i++)
  {
	...
	for(ii=0;ii<ai.ny;ii++)
	{
	  for(jj=0;jj<ai.nx;jj++)
	  {
		for(j=0;j<*(ai.nfloor+(jj+ai.nx*ii));j++)
		{
		  if((gn->d[GX])>=*(ai.xbound+jj)&&(gn->d[GX])<*(ai.xbound+jj+1)&&
			 (gn->d[GY])>=*(ai.ybound+ii)&&(gn->d[GY])<*(ai.ybound+ii+1)&&
			 (gn->d[GZ])>=*(ai.lbound+(piv1+piv2)) &&(gn->d[GZ])<*(ai.lbound+(piv1+piv2+1)))
		  {

//MessageBox(NULL,"Zone in.","Distribute",MB_OK);    /*Debug*/

			(*(ai.nnode+piv1))++;
			(*(ai.levels+piv1))+=gn->d[GZ];
			(*(ai.wi+piv1))+=(nload+i)->w[WEIGHTEQ];
...
//piv1=0;  /*Debug*/
//for(ii=0;ii<ai.ny;ii++){for(jj=0;jj<ai.nx;jj++){for(i=0;i<*(ai.nfloor+(jj+ai.nx*ii));i++){
//sprintf(non,"nnode(%d)=%d\nlevel(%d)=%.3f\nweight(%d)=%.3f\npivot=%d",
//		i,*(ai.nnode+piv1),i,*(ai.levels+piv1),i,*(ai.wi+piv1),piv1);
//MessageBox(NULL,non,"Distribute",MB_OK);
//piv1++;}}}
  ...
  for(ii=0;ii<ai.nx*ai.ny;ii++)
  {
	...
	*(ai.hmax+ii)=0.0;
	...
	for(i=0;i<*(ai.nfloor+ii);i++)
	{
	  if(*(ai.nnode+piv1)>0)
	  {
		(*(ai.levels+piv1))/=(double)(*(ai.nnode+piv1)); /*MEAN LEVEL*/
	  }
	  if((*(ai.hmax+ii))<(*(ai.levels+piv1))) *(ai.hmax+ii)=*(ai.levels+piv1);

	  for(j=0;j<(*(ai.nfloor+ii)-i);j++)
	  {
		(*(ai.Wi+piv1))+=(*(ai.wi+(piv1+j)));
	  }
	  piv1++;
	}

//sprintf(non,"hmax(%d)=%.3f\nT1(%d)=%.3f",ii,*(ai.hmax+ii),ii,*(ai.T1+ii));    /*Debug*/
//MessageBox(NULL,non,"Distribute",MB_OK);

	for(i=0;i<*(ai.nfloor+ii);i++)
	{
	  if(i==0) *(ai.hmax+ii)-=(*(ai.levels+piv2)); /*HEIGHT*/
	  piv2++;
	}
	*(ai.T1+ii)*=*(ai.hmax+ii); /*PERIOD*/

//sprintf(non,"hmax(%d)=%.3f\nT1(%d)=%.3f",ii,*(ai.hmax+ii),ii,*(ai.T1+ii));    /*Debug*/
//MessageBox(NULL,non,"Distribute",MB_OK);

	if(*(ai.T1+ii)<ai.Tc) *(ai.Rt+ii)=1.0;
	else if(*(ai.T1+ii)<2.0*(ai.Tc)) *(ai.Rt+ii)=1.0-0.2*(*(ai.T1+ii)/ai.Tc-1.0)
														*(*(ai.T1+ii)/ai.Tc-1.0);
	else *(ai.Rt+ii)=1.6*(ai.Tc)/(*(ai.T1+ii));

	/**(ai.Cf+ii)=0.5*(ai.Z)*(*(ai.Rt+ii))*(ai.Co);*/ /*FOUNDATION SHEAR*/
	*(ai.Cf+ii)=1.0*(ai.Z)*(*(ai.Rt+ii))*(ai.Co); /*FOUNDATION SHEAR*/

	for(i=0;i<*(ai.nfloor+ii);i++)
	{
	  if(i>=1)
	  {
		if(i==1) W1=*(ai.Wi+piv3);
		if(W1!=0.0) alpha=(*(ai.Wi+piv3))/W1;
		else alpha=0.0;
		...
		if(alpha!=0.0)
		{
		  (*(ai.Ai+piv3))=1.0
						  +(1.0/sqrt(alpha)-alpha)
						  *2.0*(*(ai.T1+ii))/(1.0+3.0*(*(ai.T1+ii)));
		}
		else (*(ai.Ai+piv3))=1.0;

		(*(ai.Ci+piv3))=(*(ai.Ai+piv3))*(ai.Z)*(*(ai.Rt+ii))*(ai.Co);
		(*(ai.Qi+piv3))=(*(ai.Ci+piv3))*(*(ai.Wi+piv3));

		if(i>=2)
		{
		  (*(ai.Hi+piv3-1))=(*(ai.Qi+piv3-1))-(*(ai.Qi+piv3));

		  if(*(ai.wi+piv3-1)!=0.0)
		  {
			(*(ai.facts+piv3-1))=(*(ai.Hi+piv3-1))/(*(ai.wi+piv3-1));
		  }
		  else (*(ai.facts+piv3-1))=0.0;
		}

		if(i==(*(ai.nfloor+ii)-1))
		{
//MessageBox(NULL,"Zone in.","Distribute",MB_OK);    /*Debug*/
		  (*(ai.Hi+piv3))=(*(ai.Qi+piv3));
		  if(*(ai.wi+piv3)!=0.0)
		  {
			(*(ai.facts+piv3))=(*(ai.Hi+piv3))/(*(ai.wi+piv3));
		  }
		  else (*(ai.facts+piv3))=0.0;
        }
	  }
	  else
	  {
		(*(ai.facts+piv3))=(*(ai.Cf+ii));
	  }
	  piv3++;
	}
  }
	  ...
	  for(jj=0;jj<ai.nx;jj++)
	  {
		for(j=0;j<*(ai.nfloor+(jj+ai.nx*ii));j++)
		{
		  if((gn->d[GX])>=*(ai.xbound+jj)&&(gn->d[GX])<*(ai.xbound+jj+1)&&
			 ...
			 (gn->d[GZ])>=*(ai.lbound+(piv1+piv2))&&(gn->d[GZ])<*(ai.lbound+(piv1+piv2+1)))
		  {
			((nload+i)->w[WEIGHTEQ])*=(*(ai.facts+piv1));
			//if(ii==2) ((nload+i)->w[WEIGHTEQ])*=-(*(ai.facts+piv1));
			//else      ((nload+i)->w[WEIGHTEQ])*=(*(ai.facts+piv1));
  ...
  if(fout!=NULL)
  {
	...
	if(globalunit==1.0)
	{
	  fprintf(fout,"\nè∞ópêœç⁄â◊èdÇ…ÇÊÇÈëçèdó      = %15.3f [tf]\n",
			  globalunit*total[WEIGHTSLAB]);
	  fprintf(fout,"íåó¿ópêœç⁄â◊èdÇ…ÇÊÇÈëçèdó    = %15.3f [tf]\n",
			  globalunit*total[WEIGHTFRAME]);
	  fprintf(fout,"ínêkópêœç⁄â◊èdÇ…ÇÊÇÈëçèdó    = %15.3f [tf]\n\n",
			  globalunit*total[WEIGHTEQ]);
	}
	if(globalunit==SIUNIT)
	{
	  fprintf(fout,"\nè∞ópêœç⁄â◊èdÇ…ÇÊÇÈëçèdó      = %15.3f [kN]\n",
			  globalunit*total[WEIGHTSLAB]);
	  fprintf(fout,"íåó¿ópêœç⁄â◊èdÇ…ÇÊÇÈëçèdó    = %15.3f [kN]\n",
			  globalunit*total[WEIGHTFRAME]);
	  fprintf(fout,"ínêkópêœç⁄â◊èdÇ…ÇÊÇÈëçèdó    = %15.3f [kN]\n\n",
			  globalunit*total[WEIGHTEQ]);
	}
	...
	for(ii=0;ii<ai.ny;ii++)
	{
	  for(jj=0;jj<ai.nx;jj++)
	  {
		fprintf(fout,"ÅúÉ]Å[Éì %d\n",(jj+ai.nx*ii+1));
		fprintf(fout,"  îÕàÕ                 %10.3f ÅÖ X ÅÉ%10.3f\n",*(ai.xbound+jj),*(ai.xbound+jj+1));
		fprintf(fout,"                       %10.3f ÅÖ Y ÅÉ%10.3f\n\n",*(ai.ybound+ii),*(ai.ybound+ii+1));
		fprintf(fout,"  äKêî       Å@Å@Å@    n =%d\n",*(ai.nfloor+(jj+ai.nx*ii)));
		fprintf(fout,"  çÇÇ≥         Å@Å@Å@  H =%.3f\n",*(ai.hmax+(jj+ai.nx*ii)));

		if(*(ai.hmax+(jj+ai.nx*ii))!=0.0)
		  fprintf(fout,"  ÇPéüå≈óLé¸ä˙         T1=%.3fH=%.3f\n",
				  (*(ai.T1+(jj+ai.nx*ii))/(*(ai.hmax+(jj+ai.nx*ii)))),*(ai.T1+(jj+ai.nx*ii)));
		else
		  fprintf(fout,"  ÇPéüå≈óLé¸ä˙         T1=%.3f\n",*(ai.T1+(jj+ai.nx*ii)));
		...
		for(i=0;i<*(ai.nfloor+(jj+ai.nx*ii));i++)
		{
		  sprintf(ss," %10.3f",*(ai.levels+piv1));
		  strcat(ss1,ss);
		  sprintf(ss," %10.3f",globalunit*(*(ai.wi+piv1)));
		  strcat(ss2,ss);
		  sprintf(ss," %10.3f",globalunit*(*(ai.Wi+piv1)));
		  strcat(ss3,ss);
		  if(i>0)
		  {
			sprintf(ss," %10.3f",*(ai.Ai+piv1));
			strcat(ss4,ss);
			sprintf(ss," %10.3f",*(ai.Ci+piv1));
			strcat(ss5,ss);
			sprintf(ss," %10.3f",globalunit*(*(ai.Qi+piv1)));
			strcat(ss6,ss);
			sprintf(ss," %10.3f",globalunit*(*(ai.Hi+piv1)));
			strcat(ss7,ss);
		  }
		  sprintf(ss," %10.3f",*(ai.facts+piv1));
		  strcat(ss8,ss);
