...
void assemshell(struct oshell* shells, struct memoryshell* mshell, int nshell, long int* constraintmain,
				struct gcomponent* mmtx, struct gcomponent* gmtx,
				double* iform, double* lastddisp, double* ddisp, double* finternal, double* fexternal)
{
	struct oshell shell;
	int i,j,ii,jj;
	int nnod;
	...
	for (i = 1; i <= nshell; i++)
	{
		...
		estrain = assemshellestrain(shell, B, edisp, lastedisp);

		returnmapilyushin(shell, C, estress);
