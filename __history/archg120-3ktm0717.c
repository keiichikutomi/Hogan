...
/*#define DIRECTORY "c:\\d\\cdocs\\outou_toshima\\"*/ /*DATA DIRECTORY default:"c:\\cdocs\\data\\"*/
...
struct onode{long int code,loff;     /*CODE NUMBER,OFFSET POSITION.*/   /*code=節点番号, loff=通し番号*/
			 double d[3];};          /*節点座標 d[0],d[1],d[2] =(x,y,z)*/
...
void LDLmode(struct gcomponent *gmtx,struct oconf *confs,double *lastpivot,double **mode,double *norm,long int msize);
...
int arclm201_arclength(struct arclmframe *af,int idinput);  /*by Mihara for Non-Linear Shape Analysis 100510*/
...
void assemconf(struct oconf *confs,double *gvct,double dsafety,
			   int nnode);
void asseminpressureconf(struct oconf *confs,double *gvct,
			   int nnode);
...
void LDLmode(struct gcomponent *gmtx,struct oconf *confs,double *lastpivot,double **mode,double *norm,long int msize)
{
  ...
  for(i=msize;i<=1;i++)
  {
	if(*(lastpivot+i-1)*((gmtx+i-1)->value)<0)
	{
	  LDLnorm=0.0;
	  for(j=msize;j>=1;j--)                                 /*BACKWARD.*/
	  {
...
int arclm201_arclength(struct arclmframe *af,int idinput)
{
  DWORD memory0,memory1,memory2;

  FILE *fin,*fout,*fonl,*ffig,*ffig2;                       /*FILE 8 BYTES*/
  double *ddisp,*dreact,*iform,*lastddisp;
  struct memoryelem *melem;
  char dir[]=DIRECTORY;                            /*DATA DIRECTORY*/
  ...
  double gg,kk;
  double *G;    /*Arc-Length weight parameter*/
  double *gvct,*lastgvct,*cmqvct,*pvct,*dpvct,*lastdpvct,*tarf,*truef,*lasttarf,*ubf;
  /*GLOBAL VECTOR*/
  ...
  double *norm;/*norm of LDLmode before normalized*/
  double lambda;
  double upue,upup;
  double func[2],tforce1,tforce2;
  ...
  ffig2=fopen("hognon.fig2","w");                       /*FIGURE FILE*/
  ...
  pvct=(double *)malloc(msize*sizeof(double));           /*FORCE BASE VECTOR.*/
  dpvct=(double *)malloc(msize*sizeof(double));          /*INCREMENT FORCE VECTOR.*/
  lastdpvct=(double *)malloc(msize*sizeof(double));      /*INCREMENT FORCE VECTOR.*/
  ...
  ubf=(double *)malloc(msize*sizeof(double));            /*UNBALANCED FORCE VECTOR.*/
  G=(double *)malloc((msize+1)*sizeof(double));          /*ARC-LENGTH WEIGHT.*/
  lastpivot=(double *)malloc(msize*sizeof(double));       /*PIVOT SIGN OF TANGENTIAL STIFFNESS.*/

  if(gemtx==NULL || gmtx==NULL || gvct==NULL || cmqvct==NULL || pvct==NULL || dpvct==NULL || tarf==NULL || truef==NULL || ubf==NULL || G==NULL) return 0;
  for(i=0;i<=msize-1;i++)
  {
	(gemtx+i)->down=NULL;        /*GEOMETRIC MATRIX INITIALIZATION.*/
	(gmtx+i)->down=NULL;
  ...
  *(G+msize)=1;
  ...
  while(nlap<=laps)      /***KUTOMI FOR ARC-LENGTH INCREMENTAL***/
  {
    ...
    for(i=1;i<=nelem;i++)               /*ASSEMBLAGE GLOBAL MATRIX.*/
	{
      ...
      {
        modifycmq201(melem,&elem);
        assemcmq201(elem,tmatrix,confs,cmqvct); /*ASSEMBLAGE CMQ AS LOADS.*/
		for(ii=0;ii<2;ii++)                          /*ELEM STRESS RESET.*/
        {
          for(jj=0;jj<6;jj++)
          {
	...
	if(iteration==1)
	{
      if(fonl!=NULL)
	  {
		...
		/*truef:前のiterationで増分変位から求めた節点力*/
	  }

	  asseminpressureconf(confs,pvct,nnode);/*pvct:増分荷重dΛPの基準となる荷重ベクトルP 同lapでは常に更新せずに保持する*/
	  ...
	  for(ii=0;ii<msize;ii++)
	  {
		...
		*(ubf+i)=*(tarf+i)-*(truef+i);   /*unblf:非平衡力　UNBALANCED FORCE.*/
		residual+=*(ubf+i)**(ubf+i);   /*residual:残差　非平衡力のノルムの二乗*/
	  }
	  ...
	  for(ii=0;ii<msize;ii++)
	  {
		if((confs+ii)->iconf!=1)
		{
			arcsum+=*(G+ii)*(*(gvct+ii))*(*(gvct+ii));/*変位スケーリングU_p*G*U_p*/
		}
	  }
	  arcsum+=*(G+msize);/*荷重スケーリング可算U_p*G*U_p+γ*/
	  lambda=arclength/sqrt(arcsum);/*荷重倍率dΛ=Δ/√(U_p*G*U_p+γ)*/
	  ...
	  else
	  {
		...
		for(ii=0;ii<msize;ii++)
		{
		  arcsign+=(*(ddisp+ii)-*(lastddisp+ii))**(gvct+ii);
		  *(lastddisp+ii)=*(ddisp+ii);
		  *(lasttarf+ii)=*(tarf+ii);
		}
	  ...
	  for(ii=0;ii<msize;ii++)
	  {
		if((confs+ii)->iconf!=1)
		{
		  *(gvct+ii)*=lambda*arcsign;/*gvct:増分変位dΛU_p*/
		  *(lastgvct+ii)=*(gvct+ii);
	...
	if(iteration!=1)
	{
	  ...
	  if(fout!=NULL) fprintf(fonl,"RESIDUAL=%12.9f\n",residual);
	  modifygivend(gmtx,ubf,confs,nnode); /*0:LOAD 1:DISPLACEMENT.*/
	  ...
	  if(fonl!=NULL)
	  {
		...
		for(i=0;i<nnode;i++)
		{
		  fprintf(fonl,"NODE:%5ld %12.9f %12.9f %12.9f %12.9f %12.9f %12.9f\n",(nodes+i)->code,*(ubf+(6*i+0)),*(ubf+(6*i+1)),*(ubf+(6*i+2)),*(ubf+(6*i+3)),*(ubf+(6*i+4)),*(ubf+(6*i+5)));
		}
	  }
	  ...
	  if(lastsign<sign && iteration==2 && nlap!=1)/*最初の修正子計算時に前のlapからのpivotの符号変化が検出された場合、予測子を1/2して修正子再計算　二分法で座屈点精算*/
	  {
		bucklingflag=1;
		LL=0.0;
		/*LR=2.0;*/
	  ...
	  if(bucklingflag==1)
	  {
		if(LR-LL>biseceps)
		{
			...
			for(ii=0;ii<=msize;ii++)
			{
			  if((confs+ii)->iconf!=1)
			  {
				*(gvct+ii)=*(lastgvct+ii)*arcratio;
				*(dpvct+ii)=*(lastdpvct+ii)*arcratio;
				*(tarf+ii)=*(lasttarf+ii)+*(dpvct+ii);
		...
		{
			mode=(double **)malloc(sign*sizeof(double *));   /*GLOBAL VECTORS*/
			norm=(double *)malloc(sign*sizeof(double));
			for(i=0;i<sign;i++)
			{
				*(mode+i)=(double *)malloc(msize*sizeof(double));
			}
			LDLmode(gmtx,confs,lastpivot,mode,norm,msize);
		}
	  ...
	  else/*修正子計算時に前のlapからのpivotの符号変化が検出されない場合、次のiterationへ行き計算続行*/
	  {
		...
		for(ii=0;ii<=msize;ii++)
		{
		  if((confs+ii)->iconf!=1)
		  {
			upue+=*(G+ii)*(*(gvct+ii))*(*(ubf+ii));/*U_p*G*δU_e*/
			upup+=*(G+ii)*(*(gvct+ii))*(*(gvct+ii));/*U_p*G*U_p*/
		...
		for(ii=0;ii<=msize;ii++)
		{
		  if((confs+ii)->iconf!=1)
		  {
			*(gvct+ii)=*(gvct+ii)*lambda+*(ubf+ii);/*gvct:増分変位δU_e+δΛU_p*/
	...
	for(i=1;i<=nelem;i++)                   /*STRESS OUTPUT,UPDATE.*/
	{
	  if(bucklingflag==1)                   /*予測子計算前の内力に戻す*/
	  {
		for(ii=0;ii<2;ii++)
		{
		  for(jj=0;jj<6;jj++)
		  {
			(melem+i-1)->stress[ii][jj]=(melem+i-1)->laststress[ii][jj];
		  }
		}
	  }
	  if(iteration==1)
	  {
		for(ii=0;ii<2;ii++)
		{
		  for(jj=0;jj<6;jj++)
		  {
			(melem+i-1)->laststress[ii][jj]=(melem+i-1)->stress[ii][jj];
		  }
		}
	  }
	  inputelem(elems,melem,i-1,&elem);
	  ...
	  estress=elemstressnl(&elem,gvct,melem);                /*estress:増分変位gvctによって増えた内力の総計{df}.*/
	  /*elem,melem->stress:部材端力CMQは更新済み{f+df}.*/
	  estress2=(double *)malloc(12*sizeof(double));  /*ACCUMULATION STRESS*/
	  for(ii=0;ii<=1;ii++)                                /*{f+df}.*/
	  {
		for(jj=0;jj<6;jj++)
		{
		  *(estress2+(6*ii+jj))=0.0;
		  *(estress2+(6*ii+jj))=elem.stress[ii][jj];/*estress2:増分変位gvctによって増えた部材端力CMQ{f+df}.*/
		}
	  }
    ...
    if(fonl!=NULL)
    {
	  ...
	  for(ii=0;ii<nnode;ii++)
	  {
		...
		if((nodes+ii)->code==fnode && ffig!=NULL)
		{
		  fprintf(ffig,"LAP: %3d / %3d ITERATION: %2d NODE %3d {Fz}= %18.12f {U}= %16.12f {RESISUAL}= %16.12f {DETERMINANT}= %16.12f {SIGN}= %5.0f {BUCKLINGFLAG}= %1d {arcratio}= %16.12f\n",
				  nlap,laps,iteration,(nodes+ii)->code,*(truef+(6*ii+2)),*(ddisp+6*ii+2),residual,determinant,sign,bucklingflag,arcratio);
		}
	  }
	  /*fprintf(fonl,"\"REACTION\"\n");*/
	  /*outputreaction(gmtx,gvct,nodes,confs,dreact,fonl,nnode);*/
	...
	/***KUTOMI FOR ARC-LENGTH INCREMENTAL***/
	if( (residual<tolerance || iteration==maxiteration)&& iteration!=1 && bucklingflag!=1)
	{
	  nlap++;
	  iteration=0;
	}
