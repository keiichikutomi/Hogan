				...
				double* iform, double* ddisp, double* finternal, double* fexternal)
{
	...
	double* gforminit, * eforminit;                      /*DEFORMED COORDINATION OF ELEMENT*/
	double* lastgform, * lasteform;                      /*INITIAL COORDINATION OF ELEMENT*/
	double* gform, * eform;                      /*INITIAL COORDINATION OF ELEMENT*/
	double* lastedisp, * edisp;

	double** drccos,** lastdrccos,** drccosinit;                           /*MATRIX*/
	double** lastT,** lastTt;
	double** T,** Tt;
	...
	for (i = 1; i <= nshell; i++)
	{
		...
		estrain = assemshellestrain(shell, B, edisp);
		estress = assemshellestress(shell, C, estrain);/*UPDATE STRESS & CONSISTENT STIFFNESS, HARDENING PARAMS BY RETURN-MAPPING*/
		einternal = assemshelleinternal(shell, B, estress);
