	...
	while (nlap <= laps && ENDFLAG == 0)
	{
		...
		*/
		...
		if(iteration==1)
		{
			for(i = 0; i < nshell; i++)
			{
				outputmemoryshell(shells,mshell,i);/*Initialshellでは*/
            }
		}
		residual = 0.0;
		/*
...
void assemshellEx(struct oshell* shells, struct memoryshell* mshell, int nshell, long int* constraintmain,
					 std::vector<Triplet>& Ktriplets, std::vector<Triplet>& Mtriplets,
					 Vector& iform, Vector& ddisp, Vector& finternal, Vector& fexternal)
{
	struct oshell shell;
	int i,j,ii,jj;
	int nnod;
	...
	for (i = 1; i <= nshell; i++)
	{
		...
		assemtripletshell(Ktriplets, Me, &shell, constraintmain); /*ASSEMBLAGE TANGENTIAL STIFFNESS MATRIX.*/

		Me = transformationIII(Me, T, 6*nnod);/*[Ke]=[Tt][Pt][Ht][K][H][P][T]*/
		assemtripletshell(Mtriplets, Me, &shell, constraintmain);
