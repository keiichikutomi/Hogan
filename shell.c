...
double*** assemshellshapef(struct oshell shell, double** drccos, double A);
double** assemshellemtx(struct oshell shell,double** C, double ***B, double A);
...
void concistentCilyushin(struct oshell shell, double** C, double** W, double* lambda);/*UPDATE STIFFNESS MATRIX*/
...
double* returnmapilyushin(struct oshell shell, double** C, double* estress)
{
   ...
   if( *(f+0)>0 || *(f+1)>0 )/*IF YIELD SURFACE ACTIVE.*/
   {
	   ...
	   /*RETURN-MAPPING PROCEDURE USING NEWTON-RAPTHON METHOD FOR ILYUSHIN'S YIELD CONDITION.*/
		...
		concistentCilyushin(shell, C, W, lambda);
...
void concistentCshellilyushin(struct oshell shell, double** C, double** W, double* estress, double* lambda)
{
  int i,j;
  double fn,fm;
  double e[3],a[6];
