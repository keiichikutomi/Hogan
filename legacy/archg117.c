...
#define DIRECTORY "c:\\cdocs\\data\\"    /*DIRECTORY*/
...
void getsectionopaque(HWND hdwnd,double *rate);
void setsectionopaque(HWND hdwnd,double rate);
...
/*GET OPAQUE RATE FROM DIALOG.*/
{
  ...
  GetDlgItemText(hdwnd,IDVS_OPAQUE,data,80);
  *rate=strtod(data,NULL);
  return;
}/*getsectionopaque*/
...
/*SET OPAQUE RATE INTO DIALOG.*/
{
  ...
  sprintf(str,"%.3f",rate);
  SetDlgItemText(hdwnd,IDVS_OPAQUE,str);
  return;
}/*setsectionopaque*/
...
/*DRAW BAN ON SCREEN BY AXONOMETRICS,PERSPECTIVE.*/
{
  ...
  if(rop==SRCAND) hpen=CreatePen(PS_SOLID,1,RGB(orate*r,
                                                orate*g,
                                                orate*b));
...
/*DRAW ELEMENT ON SCREEN BY AXONOMETRICS,PERSPECTIVE.*/
{
  ...
  if(mode==ONPRINTER)      hpen=CreatePen(PS_SOLID,2,RGB(0,0,0));
  else if(mode==ONPREVIEW) hpen=CreatePen(PS_SOLID,1,RGB(0,0,0));
...
/*DRAW ELEMENT,PLASTIC HINGE BY AXONOMETRICS,PERSPECTIVE.*/
{
  ...
  MoveToEx(hdc,ix1,iy1,NULL);
  LineTo(hdc,ix2,iy2);
  if(vp.vflag.ev.deformation && af.fdisp!=NULL)
  {
  ...
  if(vp.vflag.ev.hinge && af.fdisp!=NULL)          /*PLASTIC HINGE.*/
  {
    if(mode==ONPRINTER)
    {
      hpen=CreatePen(PS_SOLID,1,RGB(0,0,0)); /*BLACK*/
  ...
  for(i=0;i<6;i++) /*0:Nz 1:Qx 2:Qy 3:Mz 4:Mx 5:My*/
  {
    ...
    for(j=1;j<=7;j++) /*ELEMENT TYPE.*/
    {
         ...
         ev.stress[j][i])
      {
        ...
        sprintf(str,"%.1f",globalunit*elem.stress[0][i]);   /*HEAD.*/
        ...
        sprintf(str,"%.1f",globalunit*elem.stress[1][i]);   /*TAIL.*/
                    ...
                    int mode)
{
  ...
  for(i=0;i<af.nelem;i++)
  {
       ...
       vp.vflag.ev.etype[(af.elems+i)->sect->type]==1)
    {
      ...
      else if(globalstatus==ARCLMRATE &&
        ...
        if((org->nodes+i)->code==(az->elems+j)->node[m]->code)
        {
          ...
          *(ay->ninit+k)=*(ay->nodes+k);
          ...
          if((az->confs+aloff)->iconf==0)
          {
            ...
            *(az->nmass+k)=(org->loads+i)->w[WEIGHTFRAME]
                           /9.8; /*[tf] INTO [t]*/
          }
          else *(az->nmass+k)=0.0;
          ...
          if((ax->confs+aloff)->iconf==0)
          {
            ...
            *(ax->nmass+k)=(org->loads+i)->w[WEIGHTEQ]
                           /9.8; /*[tf] INTO [t]*/
          }
          else *(ax->nmass+k)=0.0;
          ...
          {
            (ay->confs+aloff)->value
            +=(org->loads+i)->w[WEIGHTEQ];

            *(ay->nmass+k)=(org->loads+i)->w[WEIGHTEQ]
                           /9.8; /*[tf] INTO [t]*/
          }
          else *(ay->nmass+k)=0.0;
  ...
  *sign=1.0;
  ...
  for(j=1;j<=msize;j++)                             /*DECOMPOSITION.*/
  {
    if((confs+j-1)->iconf==0) /*FREE*/
    {
      ...
      if((pivot->value)==0.0){*sign=0.0; return 0;}      /*INSTABLE.*/
      /*det*=pivot->value;*/
...
int arclm101(struct arclmframe *af,int idinput)
{
  ...
  double determinant,sign,data,safety,dsafety;
  double func[2];
  clock_t t0,t1,t2;
  ...
  for(nlap=1;nlap<=laps;nlap++)
  {
    ...
    for(i=1;i<=nelem;i++)                   /*STRESS OUTPUT,UPDATE.*/
    {
      ...
      estress=elemstress(&elem,gvct,felem,fout,func);
        ...
        for(j=1;j<=12;j++)
        {
          if(iconf[j-1]!=1)
          {
            if((a[0][0]*a[1][1]-a[0][1]*a[1][0])==0.0)
            {
              ...
              MessageBox(NULL,"Matrix Singular.","Assempmtx",MB_OK);
  ...
  for(n=0;n<=1;n++)
  {
    for(i=0;i<6;i++)
    {
      if(elem.iconf[n][i]==1)
      {
        ...
        {
          MessageBox(NULL,"Matrix Singular.","Modifyhinge",MB_OK);
        }
...
/*ELEMENT STRESS INCREMENTAL.*/
{
  ...
  updatestress(felem,fout,edisp,estress,estiff,elem,
               func);                                    /*{f}+{df}*/
    ...
    for(i=0;i<=11;i++)
    {
      if(iconf[i]!=1)
      {
        ...
        if(detinverse==0.0)
        {
          ...
          MessageBox(NULL,"Matrix Singular.","Updatestress",MB_OK);
        }
        ...
        if(elem->iconf[i][ii]==0)
        {
             ...
             elem->sect->Jzz==0.0) /*FOR BRACE*/
          {
            /*elem->sect->area=0.0;*/    /*WRONG.ALL ELEM WITH THIS*/
                                         /*SECT WILL BE SET 0.     */
            elem->iconf[i][0]=1;     /*-1:PLASTIC 0:ELASTIC 1:HINGE*/
          }
          else
          {
...
/*
sprintf(str,"\0");
sprintf(s,"POLYCURVES=%d\n",dppc.npcurve);
strcat(str,s);
for(i=0;i<dppc.npcurve;i++)
{
  sprintf(s,"POLYCURVE %d CURVES=%d\n",i+1,(dppc.pcurves+i)->ncurve);
  strcat(str,s);

  for(j=0;j<(dppc.pcurves+i)->ncurve;j++)
  {
    sprintf(s,"CURVE %d (%.3f,%.3f)(%.3f,%.3f)\n",
            j+1,((dppc.pcurves+i)->curves+j)->dots[0]->d[GX],
                ((dppc.pcurves+i)->curves+j)->dots[0]->d[GY],
                ((dppc.pcurves+i)->curves+j)->dots[1]->d[GX],
                ((dppc.pcurves+i)->curves+j)->dots[1]->d[GY]);
    strcat(str,s);
  }
}
...
for(i=0;i<nbound;i++)
{
  sprintf(s,"BOUND %d Y=%.3f=%.3E\n",i+1,*(bounds+i),*(bounds+i));
  strcat(str,s);
}
MessageBox(NULL,str,"Ultimate Bending",MB_OK);
*/
...
/*
sprintf(str,"\0");
sprintf(s,"SORTED BOUNDS=%d\n",nbound);
strcat(str,s);
for(i=0;i<nbound;i++)
{
  sprintf(s,"BOUND %d Y=%.3f\n",i+1,*(bounds+i));
  strcat(str,s);
}
MessageBox(NULL,str,"Ultimate Bending",MB_OK);
*/
...
sprintf(str,"\0");
sprintf(s,"N,M BOUNDS=%d\n",nbound);
strcat(str,s);
for(i=0;i<nbound;i++)
{
  sprintf(s,"BOUND %d Y=%.3f N=%.3f M=%.3f\n",
          i+1,*(bounds+i),*(Nb+i),*(Mb+i));
  strcat(str,s);
}
MessageBox(NULL,str,"Ultimate Bending",MB_OK);
...
sprintf(s,"Nu=%.3f\n",Nu);
strcat(str,s);
sprintf(s,"UPPER BOUND %d Y=%.3f N=%.3f M=%.3f\n",
        ibound-1,*(bounds+ibound-1),*(Nb+ibound-1),*(Mb+ibound-1));
strcat(str,s);
sprintf(s,"LOWER BOUND %d Y=%.3f N=%.3f M=%.3f\n",
        ibound,*(bounds+ibound),*(Nb+ibound),*(Mb+ibound));
strcat(str,s);
MessageBox(NULL,str,"Ultimate Bending",MB_OK);
  ...
  for(ii=0;ii<dppc.npcurve;ii++)
  {
    ...
    for(i=0;i<(dppc.pcurves+ii)->ncurve;i++)
    {
      ...
      y2=cv->dots[1]->d[GY];
/*
sprintf(str,"CURVE %d (%.3f %.3f) (%.3f %.3f) RANGE=%.3f,%.3f\n",
        i+1,x1,y1,x2,y2,b1,b2);
MessageBox(NULL,str,"Ultimate Bending",MB_OK);
*/
      if(cv->type==CTYPE_LINE)
      {
...
/******/
sprintf(str,"AU Angle(%.3f %.3f) {C0}=%.3f+%.3f+%.3f+%.3f\n",
        a1,a2,
        (op->fuc)*(-(x2-x1)*cy),
        (op->fuc)*((x1-cx)*(y1-cy)),
        (op->fuc)*(-(x2-cx)*(y2-cy)),
        (op->fuc)*(-/*signx**/r*r*(sin(2.0*a1)-sin(2.0*a2)
                               +2.0*a1-2.0*a2)/4.0)
       );
MessageBox(NULL,str,"Ultimate Bending",MB_OK);
/******/
...
/******/
sprintf(str,"AL Angle(%.3f %.3f) {C0}=%.3f+%.3f+%.3f+%.3f\n",
        a1,a2,
        (op->fut)*(-(x2-x1)*cy),
        (op->fut)*((x1-cx)*(y1-cy)),
        (op->fut)*(-(x2-cx)*(y2-cy)),
        (op->fut)*(-/*signx**/r*r*(sin(2.0*a1)-sin(2.0*a2)
                               +2.0*a1-2.0*a2)/4.0)
       );
MessageBox(NULL,str,"Ultimate Bending",MB_OK);
/******/
...
/******/
sprintf(str,"HU-U Angle(%.3f %.3f) {C0}=%.3f+%.3f+%.3f+%.3f\n",
        a1,a2,
        (op->fuc)*((x1-cx)*cy),
        (op->fuc)*((x1-cx)*(y1-cy)),
        (op->fuc)*(-(signy)*/*signx**/r*r*(sin(2.0*a1)+2.0*a1)/4.0),
        (op->fuc)*(/*signx**/PI*(-1.250*r*cy
                             +signy*0.157*cy*cy
                             +0.407/r*cy*cy*cy)/4.0)
       );
MessageBox(NULL,str,"Ultimate Bending",MB_OK);
...
/******/
sprintf(str,"HU-L Angle(%.3f %.3f) {C0}=%.3f+%.3f+%.3f+%.3f\n",
        a1,a2,
        (op->fut)*(-(x2-cx)*cy),
        (op->fut)*(-(x2-cx)*(y2-cy)),
        (op->fut)*(/*signx**/r*r*(sin(2.0*a2)+2.0*a2)/4.0),
        (op->fut)*(-/*signx**/PI*(-1.250*r*cy
                             +signy*0.157*cy*cy
                             +0.407/r*cy*cy*cy)/4.0)
       );
MessageBox(NULL,str,"Ultimate Bending",MB_OK);
...
MessageBox(NULL,str,"Ultimate Bending",MB_OK);
        ...
        else if(y2>=y1) /*CROSSING AND TAIL UPPER*/
        {
          ...
          C3+=(op->fuc)*(signx*PI*0.407/r/4.0);
/******/
sprintf(str,"TU-U Angle(%.3f %.3f) {C0}=%.3f+%.3f+%.3f+%.3f\n",
        a1,a2,
        (op->fuc)*(-(x2-cx)*cy),
        (op->fuc)*(-(x2-cx)*(y2-cy)),
        (op->fuc)*((signy)*/*signx**/r*r*(sin(2.0*a2)+2.0*a2)/4.0),
        (op->fuc)*(-/*signx**/PI*(-1.250*r*cy
                             +signy*0.157*cy*cy
                             +0.407/r*cy*cy*cy)/4.0)
       );
...
MessageBox(NULL,str,"Ultimate Bending",MB_OK);
          ...
          C3+=(op->fut)*(-signx*PI*0.407/r/4.0);
/******/
sprintf(str,"TU-L Angle(%.3f %.3f) {C0}=%.3f+%.3f+%.3f+%.3f\n",
        a1,a2,
        (op->fut)*((x1-cx)*cy),
        (op->fut)*((x1-cx)*(y1-cy)),
        (op->fut)*(-/*signx**/r*r*(sin(2.0*a1)+2.0*a1)/4.0),
        (op->fut)*(/*signx**/PI*(-1.250*r*cy
                             +signy*0.157*cy*cy
                             +0.407/r*cy*cy*cy)/4.0)
       );
...
strcat(str,s);
sprintf(s,"(%.3f) + (%.3f)y + (%.3f)y2 + (%.3f)y3 = 0\n",
        C0,C1,C2,C3);
strcat(str,s);
...
for(i=0;i<nanswer;i++)
{
  sprintf(s,"ANSWER %d=%.3f=%.3E\n",i+1,answer[i],answer[i]);
  strcat(str,s);
}
sprintf(s,"UPPER=%.3f=%.3E LOWER=%.3f=%.3E\n",b1,b1,b2,b2);
strcat(str,s);
MessageBox(NULL,str,"Ultimate Bending",MB_OK);
...
sprintf(str,"\0");
sprintf(s,"ANSWER Yn=%.3f\n",Yn);
strcat(str,s);
for(i=0;i<nanswer;i++)
{
  sprintf(s,"ANSWER %d=%.3f\n",i+1,answer[i]);
  strcat(str,s);
}
MessageBox(NULL,str,"Ultimate Bending",MB_OK);
...
{
  char non[500],ss[256];
  char ss1[256],ss2[256],ss3[256],ss4[256];
  char ss5[256],ss6[256],ss7[256],ss8[256];
...
*/
  ...
  sprintf(non,"TOTAL WEIGHT FOR SLAB=%.3f,FRAME=%.3f,EQ=%.3f",
  ...
  /*Ai LOADS*/
  GetDlgItemText((wmenu.childs+4)->hwnd,IDVS_PFACT,non,80);
  ai.T1=strtod(non,NULL); /*PERIOD FACTOR*/
  if(ai.T1==0.0) ai.T1=0.03;

  GetDlgItemText((wmenu.childs+4)->hwnd,IDVS_BASESHEAR,non,80);
  ai.Co=strtod(non,NULL); /*PERIOD FACTOR*/
  if(ai.Co==0.0) ai.Co=0.300; /*BASE SHEAR 0.2,0.3,1.0*/
  ai.Cf=0.5*ai.Co; /*FOUNDATION SHEAR*/
