...
int bclng003(struct arclmframe *af); /*BUCKLING CONDENSATION (elements' group)*/
...
/*AFTER "ARCLM001".*/
{
  ...
  fout=fgetstofopen("\0","w",ID_OUTPUTFILE);          /*OUTPUT FILE*/
  feig=fopen("eig.txt","w");                  /*tsutsumi for .rat*/
  globalfile=fout;
...
fprintf(fout,"\nELEM %d OFFSET=%d\n",(af->elems+i)->code,i+1);
//fprintf(feig,"\nELEM: %4d SECT: 101",i+1);      /*tsutsumi for .rat*/
fprintf(feig,"\nELEM: %4d",i+1);
...
sprintf(string,"ELEM OFFSET=%d",i);
errormessage(string);
...
fprintf(fout,"\n");
	...
	//deigabgeneral(kcmtx,gcmtx,CF,12,12,12,eps,EE,VV);  // tsutsumi
...
for(ii=0;ii<12;ii++)
{
  if(!CF[ii])
  {
	  ...
	  fprintf(feig,"%18.8f",EE[ii]);              /*tsutsumi for .rat*/
  }
}
fprintf(globalfile,"\n");

fprintf(globalfile,"EIGEN VALUE {E}\n");
for(ii=0;ii<12;ii++)                             //ujioka
{
  if(!CF[ii])
  {
	  fprintf(globalfile," %18.8f\n",1/EE[ii]);
...
int bclng003(struct arclmframe *af)
/*ELASTIC BUCKLING FOR ARCLM FRAME.*/
/*AFTER "ARCLM001".*/
{
  DWORD memory0,memory1;
...
bisecstart:
	  ...
	  forwardbackward(gmtx,evec,confs,N,gcomp1);      //error
