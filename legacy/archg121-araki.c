...
extern int nmultinode,nmultielem;                 //araki for multinode
...
int exchangelines(struct gcomponent *gmtx1,
                  ...
                  char flag);
int decreasebandorgan(long int *moff,long int *noff,
                      struct organ *org);                   //080912 araki
...
/*** save as arclm araki for wind loads ************************/
int saveasarclmwind(char *fname,struct arclmframe *af,int hoko);
/***************************************************************/
int savebanddecreasedarclm(struct arclmframe *af);
int savebanddecreasedarclm2(char *fname,
                            char *fnamex,
                            char *fnamey,
                            struct arclmframe *af,
                            struct arclmframe *afx,
                            struct arclmframe *afy);   //araki
...
/*    sprintf(str,"%.3f",gn.d[GX]);
    ...
    TextOut(hdc,Ox+2,Oy+50,str,strlen(str));
    sprintf(str,"%.3f",gn.d[GZ]);
    TextOut(hdc,Ox+2,Oy+100,str,strlen(str));*/      //mihara
...
void drawprintrange(HDC hdc,struct viewparam vp)
{
          ...
          ppen        = SelectObject(hdc,hpenrange);
          pbrush      = SelectObject(hdc,hbrushrange);
...
/*DRAW CONFFIG GLOBAL ON SCREEN BY AXONOMETRICS,PERSPECTIVE.*/
{
     ...
     (go.confs+(loff+5))->iconf==1) /*Tx,Ty,Tz FIXED*/
  {
    ...
    {
      ppen=SelectObject(hdc,hpenrigid);
      pbrush=SelectObject(hdc,hbrushrigid);
    }
...
/*DRAW CONFFIG GLOBAL ON SCREEN BY AXONOMETRICS,PERSPECTIVE.*/
{
  ...
  if((go.confs+loff+3)->iconf==1 ||                                             //araki for keisansho
     (go.confs+loff+4)->iconf==1 ||
     (go.confs+loff+5)->iconf==1) /*Tx,Ty,Tz FIXED*/
  {
    if(mode==ONPRINTER||mode==ONPREVIEW)
...
MessageBox(NULL,str,"Icon",MB_OK);*/
  ...
  pbrushC = SelectObject(hdcC,hbrushC);
...
/*MessageBox(NULL,"Painted.","Icon",MB_OK);*/
  ...
  pbitM = SelectObject(hdcM,(pc->ici.hbmMask));
  hbrushM = (HBRUSH)CreateSolidBrush(RGB(255,255,255));
  pbrushM = SelectObject(hdcM,hbrushM);
  PatBlt(hdcM,0,0,ix,iy,PATCOPY);

  pbitI = SelectObject(hdcI,(pc->ici.hbmColor));
  hbrushI = (HBRUSH)CreateSolidBrush(RGB(0,0,0));
  pbrushI = SelectObject(hdcI,hbrushI);
                 ...
                 ICONINFO *ici,DWORD rop)
{
  ...
  hdcC = CreateCompatibleDC(hdc);
  pbit = SelectObject(hdcC,(ici->hbmColor));
...
/*DRAW ORGANIZATION ON SCREEN BY AXONOMETRICS,PERSPECTIVE.*/
{
  ...
  /*** 09.01.16 araki for kaigyou *******/
  /*int kaigyou=0;
  char *strkaigyou="a",*cp;
  int ch;*/
  /**************************************/
...
//      kaigyou++;
       /* kaigyou[i]=0*/
 //   }
 //   imin[1]-=kaigyou*size.cy;
 /*   for(i=0;i<strlen(str);i++){
      if(i==ch){
        imin[1]+=size.cy;
        TextOut(hdc,imin[0],imin[1],str,strlen(str));
      }
      else TextOut(hdc,imin[0],imin[1],str,strlen(str));
    } */
    /**************************************************/
  }
...
/*  char rlabel[6][10]={"F    ","L    ","F+L  ","Mx","My","Mz"};*/   /*REACTION.*/ /*araki for keisansho*/
/*  char rlabel[6][10]={"F+L  ","X    ","F+L+X","F+L-X","My","Mz"};*/   /*REACTION.*/ /*araki for keisansho*/
/*  char rlabel[6][10]={"F+L  ","Y    ","F+L+Y","F+L-Y","My","Mz"};*/   /*REACTION.*/ /*araki for keisansho*/
    ...
    if(insiderange(*(af.ninit+i),vp.range))
    {
      ...
      if(af.dreact!=NULL)
      {
        ...
        for(j=0;j<=5;j++) /*REACTIONS.*/
        {
          ...
          {
            value=*(af.dreact+nreact)*globalunit;             //araki for reaction
            nreact++;

            if(vp.vflag.nv.react[j])
            {
              sprintf(str,"%s %.5f",rlabel[j],value);
             /* sprintf(str,"%s %6.3lf",rlabel[j],value);*/    //araki for keisansho
...
/*DRAW ELEMENT,PLASTIC HINGE BY AXONOMETRICS,PERSPECTIVE.*/
{
  ...
  if(vp.vflag.ev.sectioncode) /*SECTION CODE.*/
  {
    ...
    TextOut(hdc,(0.65*(ix2-ix1)+ix1),                     //araki
                (0.65*(iy2-iy1)+iy1),str,strlen(str));    //araki
  }
  if(vp.vflag.ev.srcanrate) /*SRCAN RATE.*/
  {
    ...
    //vp.dparam.hsize=10; //hsize araki
    sprintf(str,"%.3f",rate);
  ...
  /*** 2010.12.30 araki for srcanmax *****************/
  else if(mode==ONSRCANMAX)
  {
    hpen=CreatePen(PS_SOLID,18,RGB(ered,egreen,eblue));
  }
  /***************************************************/
  ...
  if(vp.vflag.ev.deformation && af.ddisp!=NULL)
  {
    ...
    else
    {
      if(mode==ONPRINTER)
      {
        /*** 09.01.16 araki for color of deformed line ************************/
        //if(!vp.vflag.ev.srcancolor)
        //  hpen=CreatePen(PS_SOLID,2,RGB(0,150,255));    /*BLUE*/
        //else
        //  hpen=CreatePen(PS_SOLID,1,RGB(0,0,0));        /*BLACK*/
        /**********************************************************************/
          hpen=CreatePen(PS_SOLID,2,RGB(ered,egreen,eblue));                //Default

      }
      else /*hpen=CreatePen(PS_SOLID,1,RGB(ered,egreen,eblue));*/          //Default
           hpen=CreatePen(PS_SOLID,1,RGB(0,0,255));    /*BLUE*/            //09.01.13 araki
  ...
  */
  ...
  /*** 10.12.30. araki for srcanmax ***/
  /*if(vp.vflag.ev.srcanmax && mode==ONPRINTER)
  {
    hpen=CreatePen(PS_SOLID,3,RGB(0,0,0));*/    /*BLACK*/
...
/*DRAW STRESS OF ONE ELEMENT BY AXONOMETRICS,PERSPECTIVE.*/
{
  ...
  /*** araki for Nz ***/
  GetDlgItemText((wmenu.childs+2)->hwnd,ID_SECTIONFILE,str,80);
...
if(mode!=ONPREVIEW)
{
  for(i=0;i<6;i++) /*0:Nz 1:Qx 2:Qy 3:Mz 4:Mx 5:My*/
  {
    ...
    /*    if(!strncmp(prj,"hakushima",9))
        ...
        sprintf(str,"%.3f",globalunit*elem.stress[0][i]);   /*HEAD.*/      //araki for Nz
        tn=setcoord(((0.25/**(6-i)*/)*elength),0.0,0.0);
        /*drawtextalignedonlocaldot(hdc,vp,tdrccos,ninit[0],tn,str,
                                    TEXT_CENTER,TEXT_TOP);*/
        drawtextonlocaldot(hdc,vp,tdrccos,ninit[0],tn,str);                //araki for Nz
        ...
        sprintf(str,"%.3f",globalunit*elem.stress[1][i]);   /*TAIL.*/       //araki for Nz
        tn=setcoord(((1-0.25/**(i+1)*/)*elength),0.0,0.0);
        /*drawtextalignedonlocaldot(hdc,vp,tdrccos,ninit[0],tn,str,
                                  TEXT_CENTER,TEXT_TOP);*/
        /*** araki for Nz ***/
        if(!strncmp(prj,"hakushima",9) && i==0){}
       // if(!strncmp(prj,"hakushima",9)){}
        else{
        /********************/
                    ...
                    int mode)
{
  ...
  {
    imin[1]-=size.cy;
    //////////////////////////081202 araki for keisansho///////
    /*if(vp.vflag.mv.pagetitle && vp.vflag.ev.srcanrate)
    {
    sprintf(str,"äeïîçﬁÇ…Ç¬Ç¢Çƒ,í∑ä˙,X,Yï˚å¸íZä˙â◊èdéûÇ…Ç®ÇØÇÈé≤óÕ,ÇπÇÒífóÕ,ã»Ç∞ÉÇÅ[ÉÅÉìÉgÇÃåüíËî‰ÇÃÇ§Çø,ç≈Ç‡ëÂÇ´Ç¢ílÇï\é¶ÇµÇƒÇ¢ÇÈ.");
    TextOut(hdc,imin[0],imin[1]-600,str,strlen(str));
    }*/
    ...
    sprintf(str,"äeïîçﬁÇ…Ç¬Ç¢Çƒ,í∑ä˙,X,Yï˚å¸íZä˙â◊èdéûÇ…Ç®ÇØÇÈé≤óÕ,ÇπÇÒífóÕ,ã»Ç∞ÉÇÅ[ÉÅÉìÉgÇÃåüíËî‰ÇÃÇ§Çø,ç≈Ç‡ëÂÇ´Ç¢ílÇï\é¶ÇµÇƒÇ¢ÇÈ.");
    TextOut(hdc,imin[0],imin[1]-600,str,strlen(str));
    sprintf(str,"ê}ÇÕåüíËî‰ÇêFï\é¶ÇµÇΩÇ‡ÇÃÇ≈,ê¬ÇÕó]óTÇ™ëÂÇ´Ç≠,óŒÅ`â©Å`ÉIÉåÉìÉWÇ∆Ç»ÇÈÇ…ÇµÇΩÇ™Ç¡Çƒó]óTÇ™è≠Ç»Ç≠Ç»ÇÈÇ±Ç∆Çé¶Ç∑.");
    TextOut(hdc,imin[0],imin[1]-500,str,strlen(str));
    sprintf(str,"ê‘Ç™Ç†ÇÈÇ∆ë´ÇËÇ»Ç¢Ç±Ç∆Çï\Ç∑Ç™,Ç±ÇÃê}Ç…ÇÕë∂ç›ÇµÇ»Ç¢ÇÃÇ≈,ê¨óßÇµÇƒÇ¢ÇÈÇ±Ç∆Ç™ï™Ç©ÇÈ.");
    TextOut(hdc,imin[0],imin[1]-400,str,strlen(str));
    }*/

    /*sprintf(str,"ïîï™ìIÇ…åüíËî‰Ç™1.0Çí¥Ç¶ÇƒÇ¢ÇÈê‘Ç¢ïîçﬁÇ™Ç†ÇÈÇ™ÅCäJå˚à íuÇÃí≤êÆìôÇ≈ç°å„åüì¢ÇêiÇﬂÇÈÇ‡ÇÃÇ∆Ç∑ÇÈ."); */

    /*if(vp.vflag.mv.pagetitle)
    {
    sprintf(str,"ífñ éZíËÇÕëSïîçﬁÇ…Ç¬Ç¢ÇƒçsÇ¡ÇƒÇ¢ÇÈÅDäeífñ éÌï Ç…Ç®Ç¢ÇƒÅCâûóÕÇ™ç≈ëÂÇÃïîçﬁÇÕâ∫ê}ÇÃÇ∆Ç®ÇËÅD");
    TextOut(hdc,imin[0],imin[1]-600,str,strlen(str));
    sprintf(str,"Ç±ÇÍÇÁÇ…Ç¬Ç¢ÇƒÇÃÇ›ífñ éZíËÇÃè⁄ç◊Çç⁄ÇπÇƒÇ®Ç≠ÅD");
    TextOut(hdc,imin[0],imin[1]-500,str,strlen(str));
    }*/
    //////////////////////////////////////////////////////////
    sprintf(str,"%s",(wdraw.childs+1)->title);
  ...
  for(i=0;i<8;i++)
  {
    if(vp.vflag.ev.stress[i][3])
    {
      ...
      /*** araki for obj *******************************************/
      GetDlgItemText((wmenu.childs+2)->hwnd,ID_SECTIONFILE,prj,80);
      if(!strncmp(prj,"obj",3)) sprintf(str,"ç¿ã¸í∑Ç≥åWêî");
      /*************************************************************/
...
//////////////////////////////////////////////////////by MIHARA for Safety Level
  ...
  if(vp.vflag.ev.srcancolor)
  {
    ...
    /*** by araki *********************************************/
    sprintf(str,"à¿ëSó¶ÇÃñ}ó·");
    TextOut(hdc,imin[0]+2000,imax[1]+size.cy,str,strlen(str));

    sprintf(str,"Å@ : ÅÜ1.0");
    TextOut(hdc,imin[0]+2000,imax[1]+3*size.cy,str,strlen(str));
    sprintf(str,"Å@ : 0.9Å`1.0");
    TextOut(hdc,imin[0]+2000,imax[1]+4*size.cy,str,strlen(str));
    sprintf(str,"Å@ : 0.71428Å`0.9");
    TextOut(hdc,imin[0]+2000,imax[1]+5*size.cy,str,strlen(str));
    sprintf(str,"Å@ : 0.7Å`0.71428");
    TextOut(hdc,imin[0]+2000,imax[1]+6*size.cy,str,strlen(str));
    sprintf(str,"Å@ : 0.6Å`0.7");
    TextOut(hdc,imin[0]+2000,imax[1]+7*size.cy,str,strlen(str));
    sprintf(str,"Å@ : 0.5Å`0.6");
    TextOut(hdc,imin[0]+2000,imax[1]+8*size.cy,str,strlen(str));
    sprintf(str,"Å@ : ÅÉ0.5");
    TextOut(hdc,imin[0]+2000,imax[1]+9*size.cy,str,strlen(str));
    ...
    sprintf(str,"Å°");
    TextOut(hdc,imin[0]+2000,imax[1]+4*size.cy,str,strlen(str));
    SetTextColor(hdc,RGB(255,240,10));                             //araki for yellow Default 255,255,0
    sprintf(str,"Å°");
    TextOut(hdc,imin[0]+2000,imax[1]+5*size.cy,str,strlen(str));
    SetTextColor(hdc,RGB(150,255,0));
    sprintf(str,"Å°");
...
////by araki for conffing 2008.12.09////////////////////////////////
if(vp.vflag.nv.conffig)
  {
       ...
       vp.vflag.ev.etype[(af.elems+i)->sect->type]==1)
    {
              ...
              (af.elems+i)->sect->code==code)
      {
        drawglobalwire(hdc,vp,af,*(af.elems+i),255,0,255,
              ...
              ((af.elems+i)->srate[0]>1.0 || /*Shiojiri 2.0*/
               ...
               (af.elems+i)->srate[3]>1.0))
      {
        drawglobalwire(hdc,vp,af,*(af.elems+i),255,0,150,
              ...
              ((af.elems+i)->srate[0]>0.8 || /*0.9*//*0.8*//*Shiojiri 1.0*/
               ...
               (af.elems+i)->srate[3]>0.8))
      {
        drawglobalwire(hdc,vp,af,*(af.elems+i),255,150,50,
              ...
              ((af.elems+i)->srate[0]>0.6 || /*0.71428*//*Shiojiri 0.4*/
               ...
               (af.elems+i)->srate[3]>0.6))
      {
        drawglobalwire(hdc,vp,af,*(af.elems+i),255,255,0,
              ...
              ((af.elems+i)->srate[0]>0.4 || /*0.7*//*0.6*//*Shiojiri 0.35*/
               ...
               (af.elems+i)->srate[3]>0.4))
      {
        drawglobalwire(hdc,vp,af,*(af.elems+i),150,255,0,
              ...
              ((af.elems+i)->srate[0]>0.3 || /*0.6*//*0.5*//*Shiojiri 0.3*/
               ...
               (af.elems+i)->srate[3]>0.3))
      {
        drawglobalwire(hdc,vp,af,*(af.elems+i),0,255,0,
              ...
              ((af.elems+i)->srate[0]>0.2 || /*0.5*//*0.4*//*Shiojiri 0.2*/
...
//#if 0
              ...
              ((af.elems+i)->srate[0]>0.9 || /*0.9*//*0.8*//*Shiojiri 1.0*/
               ...
               (af.elems+i)->srate[3]>0.9))
      {
        drawglobalwire(hdc,vp,af,*(af.elems+i),255,150,50,
              ...
              ((af.elems+i)->srate[0]>0.71428 || /*0.71428*//*Shiojiri 0.4*/
               (af.elems+i)->srate[1]>0.71428 ||
               (af.elems+i)->srate[2]>0.71428 ||
               (af.elems+i)->srate[3]>0.71428))
      {
        drawglobalwire(hdc,vp,af,*(af.elems+i),255,240,10,
                                               255,240,10,code,mode);    //araki for yellow Default 255,255,0
      }
      else if(vp.vflag.ev.srcancolor &&
              ((af.elems+i)->srate[0]>0.7 || /*0.7*//*0.6*//*Shiojiri 0.35*/
               ...
               (af.elems+i)->srate[3]>0.7))
      {
        drawglobalwire(hdc,vp,af,*(af.elems+i),150,255,0,
              ...
              ((af.elems+i)->srate[0]>0.6 || /*0.6*//*0.5*//*Shiojiri 0.3*/
               (af.elems+i)->srate[1]>0.6 ||
               (af.elems+i)->srate[2]>0.6 ||
               (af.elems+i)->srate[3]>0.6))
      {
        drawglobalwire(hdc,vp,af,*(af.elems+i),0,255,0,
              ...
              ((af.elems+i)->srate[0]>0.5 || /*0.5*//*0.4*//*Shiojiri 0.2*/
...
//#endif
              ...
              (af.elems+i)->sect->smax==i)
      {
        ...
        /*** 10.06.08. araki for srcanmax ***/
        if(mode==ONPRINTER)
        {
          /*--- 11.04.18. araki for srcanmax ---*/
        if((af.elems+i)->sect->type==WALL || (af.elems+i)->sect->type==SLAB/* || (af.elems+i)->sect->code==102*/)
        {
          hsize=vp.dparam.hsize;
          vp.dparam.hsize*=5.0;
          drawglobalwire(hdc,vp,af,*(af.elems+i),0,0,0,
...
/*SAVE ORGAN INTO OUTPUT FILE.*/
{
  ...
  for(i=0;i<nsect;i++)
  {
    ...
    else
    {
      ...
      /*  fprintf(fout,"NZMAX    1.000 ");
        fprintf(fout,"NZMIN   -1.000\n");
        fprintf(fout,"         ");
        fprintf(fout,"QXMAX    1.000 ");
        fprintf(fout,"QXMIN   -1.000\n");
        fprintf(fout,"         ");
        fprintf(fout,"QYMAX    1.000 ");
        fprintf(fout,"QYMIN   -1.000\n");
        fprintf(fout,"         ");
        fprintf(fout,"MZMAX    1.000 ");
        fprintf(fout,"MZMIN   -1.000\n");
        fprintf(fout,"         ");
        fprintf(fout,"MXMAX    1.000 ");
        fprintf(fout,"MXMIN   -1.000\n");
        fprintf(fout,"         ");
        fprintf(fout,"MYMAX    1.000 ");
        fprintf(fout,"MYMIN   -1.000\n");  */               //araki
      }
    }
  ...
  for(i=0;i<nnode;i++)
  {
    ...
    /*fprintf(fout,"NODE %4ld  ",(org->nodes+i)->code+3000);*/      //araki for comb
    ...
    else fprintf(fout,"%6.3f",(org->confs+6*i+0)->value);
    if((org->confs+6*i+1)->value==0.0) fprintf(fout," 0.0  ");
    else fprintf(fout,"%6.3f",(org->confs+6*i+1)->value);
    if((org->confs+6*i+2)->value==0.0) fprintf(fout," 0.0  ");
    else fprintf(fout,"%6.3f",(org->confs+6*i+2)->value);
    if((org->confs+6*i+3)->value==0.0) fprintf(fout," 0.0  ");
    else fprintf(fout,"%6.3f",(org->confs+6*i+3)->value);
    if((org->confs+6*i+4)->value==0.0) fprintf(fout," 0.0  ");
    else fprintf(fout,"%6.3f",(org->confs+6*i+4)->value);
    if((org->confs+6*i+5)->value==0.0) fprintf(fout," 0.0");
    else fprintf(fout,"%6.3f",(org->confs+6*i+5)->value);
    fprintf(fout,"\n");
  }
  fprintf(fout,"\n");
  ...
  for(i=0;i<nelem;i++)
  {
    ...
    /*fprintf(fout,"ELEM %5ld ",(org->elems+i)->code+9000);*/         //araki for comb
    fprintf(fout,"ESECT %3ld ",(org->elems+i)->sect->code);

    if(((org->elems+i)->nnod)==2)
    {
      /*if((org->elems+i)->sect->code==504)
      {
        (org->elems+i)->cangle=0;
      } */                                                   //araki for hakushima
    ...
    /*** 11.09.05 araki for Change i, j (for hakushima)*********/
/*    if(((org->elems+i)->nnod)==2 && (org->elems+i)->type==COLUMN)
    {
      struct onode *inode,*jnode,cnode;
      inode=*((org->elems+i)->nods+0);
      jnode=*((org->elems+i)->nods+1);

      cnode.code=(inode->code);
      if((inode->d[GZ])>(jnode->d[GZ]))
      {
        if((inode->d[GZ])==0.1)
        {
          (*((org->elems+i)->nods+0))->code=(*((org->elems+i)->nods+1))->code;
          (*((org->elems+i)->nods+1))->code=cnode.code;
        }
      }
    }          */
    /********************************************/
    ...
    for(j=0;j<(org->elems+i)->nnod;j++)
    {
      ...
      /*fprintf(fout," %d",(*((org->elems+i)->nods+j))->code+3000);*/       //araki for comb
    }
    fprintf(fout," BONDS");
    ...
    if(((org->elems+i)->nban)>=1)
    {
      ...
      for(j=0;j<(org->elems+i)->nban;j++)
      {
        ...
        for(k=0;k<((org->elems+i)->bans+j)->nnod;k++)
        {
          ...
          /*fprintf(fout," %ld",
                  (*(((org->elems+i)->bans+j)->nods+k))->code+3000);*/   //araki for comb
        }
...
//      fprintf(fout,"           CANG %.5f\n",(org->elems+i)->cangle);
    ...
    {
      /*if((org->elems+i)->sect->code>400 && (org->elems+i)->sect->code<499)
        fprintf(fout,"           TYPE COLUMN\n");
      else    */
    ...
    {
      /*if((org->elems+i)->sect->code==711)
        fprintf(fout,"           TYPE WALL\n");
      else   */                                            //araki for elemtype
...
/*EXTRACT ARCLM FROM ORGAN.*/
{
  ...
  if(!strncmp(non,"hakushima",9))     strcpy(prj,"hakushima");
  /***************************/
...
/*COMPARE FIRST FIGURE*/
{
  ...
  /*double eps1=1.0E-2,eps2=1.0E-2;*/ /*ChangedbyMIHARA,DEFAULT=1.0E-12*/

  if(!strncmp(prj,"kiryu",5)) eps2=1.0E-4;

  if(!strncmp(prj,"koshi",5)) eps2=1.0E-4;

/*  if(!strncmp(prj,"turu",4))  eps2=1.0E+1;  */
  ...
  /*** araki for tsurukawa ***/
/*  if(!strncmp(prj,"turu",4))
  {
    if(s1->type==SLAB)
    {
      if(((s1->figs+0)->prop->E)*((s1->figs+0)->area)  < ((s2->figs+0)->prop->E)*((s2->figs+0)->area) -eps2 ||
         ((s1->figs+0)->prop->E)*((s1->figs+0)->area)  < ((s2->figs+0)->prop->E)*((s2->figs+0)->area) +eps2)  return 0;
    }
    else
    {
      if((s1->figs+0)->area  < (s2->figs+0)->area -eps1 ||
         (s1->figs+0)->area  > (s2->figs+0)->area +eps1)  return 0;
    }
  } */
  /******************************/
...
/*        for(k=n+1;k<m-1;k++) *//*SEARCH CHANGABLE LINE.*/
        for(k=n+1;k<=m-1;k++) /*SEARCH CHANGABLE LINE.  //araki kÇÕn+1Ç©ÇÁ
/*        for(k=0;k<=m-1;k++)*/ /*SEARCH CHANGABLE LINE.*/  //araki kÇÕ0Ç©ÇÁ
        {
          flag=0;
/*          if(k==n) k++; */  //araki kÇÕ0Ç©ÇÁ
          for(l=0;l<nnode;l++) /*0-n,m-N*/
          {
            if(l<=n) mn=*(ii+k)-(k-l);      //araki kÇÕn+1Ç©ÇÁ
///////////////////////////////////////////////////araki kÇÕ0Ç©ÇÁ//
/*            if(l<=n)
            {
              if(k<l) mn=*(ii+l)-(l-k);
...
///////////////////////////////////////////////////araki//
            if(l==n)
            {
              if(k<=n) l=i+k-1;      //kÇ™n+1Ç©ÇÁÇ»ÇÁÇ±ÇÃçsïsóv
              else     l=i+k;
            }
//////////////////////////////////////////////////////////
          }
...
///////////////////////////////////////////////////080912 araki/////
int decreasebandorgan(long int *moff, /*MOVED POSITION OF NODE*/
                      long int *noff, /*CHANGED NODE OFFSETS*/
                      struct organ *org)
/*DECREASE BAND WIDTH OF ORGAN.*/
{
  char *cmtx,c; /*CONNECTION MATRIX*/
  char flag,str[512];
  /*char s[80];*/
  long int h,i,j,k,l,m,n,s,t,mn,hk,hm,st,nnode,nelem,off,band;
  long int *ii; /*POSITION OF DIAGONAL*/
  long int mm,nn;
  long int step; 
  long int exchangedband;
  ...
  for(i=0;i<nelem;i++)
  {
    if((orgaf->elems+i)->type==WALL || (orgaf->elems+i)->type==SLAB)
    {
      if((orgaf->elems+i)->nnod==4) count++;
    }
  }
  ...
  for(i=0;i<nelem;i++)
  {
    if((orgaf->elems+i)->type==WALL || (orgaf->elems+i)->type==SLAB)
    {
      if((orgaf->elems+i)->nnod==4)
      {
        recttobrace((orgaf->elems+i),orgaf,(nelem+2*j),2,1.0);
        j++;
      }
    }
  }
...
*/
  ...
  for(i=0;i<nelem;i++) /*ARRANGE CONNECTIONS.*/
  {
    m=(*((orgaf->elems+i)->nods+0))->loff;
    n=(*((orgaf->elems+i)->nods+1))->loff;
...
int arclm101(struct arclmframe *af,int idinput)
{
  ...
  /*** 09.08.28 araki for Tsurukawa *******************************************/
  FILE *fotp;                             /*OUTPUT LAST STEP IN OHX, OHY STYLE*/
  int j,m,n,mm,nn[2],offset;
  double gstiff,dreaction,data[6];
  ...
  else
  {
    ...
    for(i=0;i<nelem;i++)
    {
      for(n=1;n<=2;n++)
      {
        nn[n-1]=(elems+i)->node[n-1]->code;
        if(n==1) sprintf(string,"%5d %4d %4d",
                         (elems+i)->code,((elems+i)->sect)->code,nn[n-1]);
        if(n==2) sprintf(string,"           %4d",nn[n-1]);
        for(j=0;j<6;j++)
        {
          sprintf(s," %9.8f",*(stresslong+12*i+j+6*(n-1)));
          strcat(string,s);
        }
        if(fotp!=NULL) fprintf(fotp,"%s\n",string);
      }
    }
    if(fotp!=NULL) fprintf(fotp,"\n\n");
    */
    ...
    if(fotp!=NULL)
    {
      fprintf(fotp,"** DISPLACEMENT OF NODE\n\n");
      fprintf(fotp,"  NO          U          V          W");
      fprintf(fotp,"         KSI         ETA       OMEGA\n\n");
    }

    for(i=0;i<nnode;i++)
    {
      for(j=0;j<=5;j++) data[j]=*(displong+6*i+j);
      sprintf(string,
    ...
    */
    ...
    for(i=1;i<=6*nnode;i++)
    {
      if((af->confs+i-1)->iconf==1)
      {
        sprintf(string,"%4ld %10ld %14.6f     1",
                (af->nodes+(i-1)/6)->code,(i-1)%6+1,*(reactlong+j));
        if(fotp!=NULL) fprintf(fotp,"%s\n",string);
        j++;
      }
    }
  ...
  for(nlap=1;nlap<=laps;nlap++)
  {
    ...
    if(nlap==laps)
    {
      ...
      {
        fprintf(fotp,"\n\n");
        fprintf(fotp,"** FORCES OF MEMBER\n\n");
        fprintf(fotp,"  NO   KT NODE         N        Q1        Q2");
        fprintf(fotp,"        MT        M1        M2\n\n");
      }
    }
    /**************************************************************************/
    for(i=1;i<=nelem;i++)                   /*STRESS OUTPUT,UPDATE.*/
    {
      ...
      /*** 09.08.28 araki for Tsurukawa ***************************************/
      if(nlap==laps) outputstress02(elem,estress,stresslong,i-1,fotp,func);
      /************************************************************************/
      free(estress);
    }
    ...
    /*if(nlap==laps){
      if(fotp!=NULL){
        fprintf(fotp,"** REACTION\n\n");
        fprintf(fotp,"  NO  DIRECTION              R    NC\n\n");
      }
      outputreaction02(gmtx,gvct,nodes,confs,dreact,reactlong,fotp,nnode);
    }*/
    /**************************************************************************/
    ...
    if(nlap==laps)
    {
      ...
      fprintf(fotp,"\n\n");
      outputdisp02(ddisp,displong,fotp,nnode,nodes);  /*INCREMENTAL DISPLACEMENT.*/
      if(fotp!=NULL) fprintf(fotp,"\n\n");
...
/*SAVE AS ARCLM INPUTFILE.*/
{
  ...
  /*** 090716 araki for brace *************************************************/
  GetDlgItemText((wmenu.childs+2)->hwnd,ID_SECTIONFILE,str,80);
  if(!strncmp(str,"azabu",5))     strcpy(prj,"azabu");
  if(!strncmp(str,"kato",4))      strcpy(prj,"kato");
  if(!strncmp(str,"obj",3))       strcpy(prj,"obj");
  if(!strncmp(str,"gate",4))      strcpy(prj,"gate");
  if(!strncmp(str,"boma",4))      strcpy(prj,"boma");
  if(!strncmp(str,"resttoplight",12))      strcpy(prj,"resttoplight");
  if(!strncmp(str,"hakushima",9))      strcpy(prj,"hakushima");
  /****************************************************************************/
  ...
  for(i=0;i<(af->nsect);i++)
  {
  /*** 090716 araki for azabu *************************************************/
    if(!strncmp(prj,"azabu",5)){
      if(strstr(fname,"inl")){
        if(((af->sects+i)->code==205)||((af->sects+i)->code==206)||((af->sects+i)->code==506)){
          (af->sects+i)->area=0.00000001;
          (af->sects+i)->Ixx =0.000000000001;
          (af->sects+i)->Iyy =0.000000000001;
          (af->sects+i)->Jzz =0.000000000001;
        }
      }
    }

    if(!strncmp(prj,"kato",4)){
      /*if(strstr(fname,"inl")){*/
        if(((af->sects+i)->code==206)){
          (af->sects+i)->area=0.00000001;
          (af->sects+i)->Ixx =0.000000000001;
          (af->sects+i)->Iyy =0.000000000001;
          (af->sects+i)->Jzz =0.000000000001;
      ...
      /*}*/
      if(strstr(fname,"ihx")||strstr(fname,"ihy")){
        if(((af->sects+i)->code==204)||((af->sects+i)->code==205)||((af->sects+i)->code==206)){
          (af->sects+i)->area=0.00000001;
          (af->sects+i)->Ixx =0.000000000001;
          (af->sects+i)->Iyy =0.000000000001;
          (af->sects+i)->Jzz =0.000000000001;
    ...
    if(!strncmp(prj,"obj",3)){
      if(strstr(fname,"inl")){
        if(((af->sects+i)->code==601)||((af->sects+i)->code==602)||((af->sects+i)->code==603)||((af->sects+i)->code==604)){
          (af->sects+i)->area=0.00000001;
          (af->sects+i)->Ixx =0.000000000001;
          (af->sects+i)->Iyy =0.000000000001;
          (af->sects+i)->Jzz =0.000000000001;
      ...
      if(strstr(fname,"ihx")){
        if(((af->sects+i)->code==603)||((af->sects+i)->code==604)){
          (af->sects+i)->area=0.00000001;
          (af->sects+i)->Ixx =0.000000000001;
          (af->sects+i)->Iyy =0.000000000001;
          (af->sects+i)->Jzz =0.000000000001;
      ...
      if(strstr(fname,"ihy")){
        if(((af->sects+i)->code==601)||((af->sects+i)->code==602)){
          (af->sects+i)->area=0.00000001;
          (af->sects+i)->Ixx =0.000000000001;
          (af->sects+i)->Iyy =0.000000000001;
          (af->sects+i)->Jzz =0.000000000001;
    ...
    if(!strncmp(prj,"boma",4)){
      if(strstr(fname,"inl")){
        if(((af->sects+i)->code==601)||((af->sects+i)->code==602)||((af->sects+i)->code==603)){
          (af->sects+i)->area=0.00000001;
          (af->sects+i)->Ixx =0.000000000001;
          (af->sects+i)->Iyy =0.000000000001;
          (af->sects+i)->Jzz =0.000000000001;
    ...
    if(!strncmp(prj,"resttoplight",12)){
      if(strstr(fname,"inl")){
        if(((af->sects+i)->code==203)||((af->sects+i)->code==204)){
          (af->sects+i)->area=0.00000001;
          (af->sects+i)->Ixx =0.000000000001;
          (af->sects+i)->Iyy =0.000000000001;
          (af->sects+i)->Jzz =0.000000000001;
  ...
  /****************************************************************************/
    fprintf(fin,"%5d %.7E %.5f %.8f %.12f %.12f %.12f",
            (af->sects+i)->code,
            (af->sects+i)->E,
            (af->sects+i)->poi,
            (af->sects+i)->area,
  ...
  for(i=0;i<(af->nnode);i++) /*CONF VECTOR:CONFINEMENT,VALUE.*/
  {
    ...
    for(j=0;j<6;j++)
    {
      ...
      /*** 2010.12.08 araki for VCON ***/
      /*if(!strncmp(prj,"misato",6)){
        if(strstr(fname,"ihx") && j==0){
          if((af->nodes+i)->code==1317 || (af->nodes+i)->code==1318)
            (af->confs+offset)->value+=0.118;
          if((af->nodes+i)->code==1322 || (af->nodes+i)->code==1324 || (af->nodes+i)->code==1333 || (af->nodes+i)->code==1334)
            (af->confs+offset)->value+=0.184;
        }
        if(strstr(fname,"ihy") && j==1){
          if((af->nodes+i)->code==1317 || (af->nodes+i)->code==1318)
            (af->confs+offset)->value+=0.240;
          if((af->nodes+i)->code==1322 || (af->nodes+i)->code==1324 || (af->nodes+i)->code==1333 || (af->nodes+i)->code==1334)
            (af->confs+offset)->value+=0.373;
        }
      }*/
      /**********************************/

      fprintf(fin," %8.8f",(af->confs+offset)->value);
    }
    fprintf(fin,"\n");
  }
  /*LONGREACTION:UNDER CONSTRUCTION.*/
...
/*** save as arclm araki for wind loads **************************/ //UNDER CONSTRUCTION
int saveasarclmwind(char *fname,struct arclmframe *af,int hoko)
/*SAVE AS ARCLM INPUTFILE FOR WIND LOADS AGAINST FLAT ROOF.*/
/*HOKO 0:X POSITIVE*/
/*HOKO 1:X NEGATIVE*/
/*HOKO 2:Y POSITIVE*/
/*HOKO 3:Y NEGATIVE*/
{
  ...
  double value=0;
  double windpressure=5.65;  /*INPUT WINDPRESSURE[kgf/m2]*/
  double alpha=0.2;          /*INPUT ALPHA*/
  ...
  for(i=0;i<(af->nnode);i++)
  {
    if(xmin>(af->nodes+i)->d[GX]) xmin=(af->nodes+i)->d[GX];
    if(xmax<(af->nodes+i)->d[GX]) xmax=(af->nodes+i)->d[GX];
    if(ymin>(af->nodes+i)->d[GY]) ymin=(af->nodes+i)->d[GY];
    if(ymax<(af->nodes+i)->d[GY]) ymax=(af->nodes+i)->d[GY];
    if(zmin>(af->nodes+i)->d[GZ]) zmin=(af->nodes+i)->d[GZ];
    if(zmax<(af->nodes+i)->d[GZ]) zmax=(af->nodes+i)->d[GZ];
  }
  ...
  fprintf(fin,"%7.3f %7.3f %7.3f\n",xmax,ymax,zmax);

  fprintf(fin,"%5d %5d %5d\n",af->nnode,af->nelem,af->nsect);

  for(i=0;i<(af->nsect);i++)
  {
    fprintf(fin,"%5d %.5E %.5f %.8f %.12f %.12f %.12f",
            (af->sects+i)->code,
            (af->sects+i)->E,
            (af->sects+i)->poi,
            (af->sects+i)->area,
  ...
  for(i=0;i<(af->nnode);i++) /*CONF VECTOR:CONFINEMENT,WIND LOADS.*/
  {
    fprintf(fin,"%5d",(af->nodes+i)->code);
    ...
    for(j=0;j<6;j++)
    {
      if(j==0)
      {
        if(hoko==0)      value=( 0.8*pow(((af->nodes+i)->d[GZ])/zmax,2*alpha)+0.4)*windpressure/1000*2.2/3*1;
        else if(hoko==1) value=(-0.8*pow(((af->nodes+i)->d[GZ])/zmax,2*alpha)-0.4)*windpressure/1000*2.2/3*1;
        else if(hoko==2) value=(-0.7+0.7)*windpressure/1000*2.2/3*1;
        else if(hoko==3) value=(-0.7+0.7)*windpressure/1000*2.2/3*1;
        fprintf(fin," %8.8f",value);
      }
      if(j==1)
      {
        if(hoko==0)      value=(-0.7+0.7)*windpressure/1000*2.2/3*1;
        else if(hoko==1) value=(-0.7+0.7)*windpressure/1000*2.2/3*1;
        else if(hoko==2) value=( 0.8*pow(((af->nodes+i)->d[GZ])/zmax,2*alpha)+0.4)*windpressure/1000*2.2/3*1;
        else if(hoko==3) value=(-0.8*pow(((af->nodes+i)->d[GZ])/zmax,2*alpha)-0.4)*windpressure/1000*2.2/3*1;
        fprintf(fin," %8.8f",value);
      }
      if(j>1)
      {
...
/*            (af->nodes+off)->d[GX],
            ...
            (af->nodes+i)->d[GX],
            (af->nodes+i)->d[GY],
            (af->nodes+i)->d[GZ]);         //araki
  }
  for(i=0;i<(af->nelem);i++)
  {
    offhead=*(noff+((af->elems+i)->node[0])->loff);
    offtail=*(noff+((af->elems+i)->node[1])->loff);         //araki
    fprintf(fin,"%5d  %5d  %5d %5d  %.5f",
            (af->elems+i)->code,
            (af->elems+i)->sect->code,
/*            (af->elems+i)->node[0]->code,
            (af->elems+i)->node[1]->code,  */
            (af->nodes+offhead)->code,
            (af->nodes+offtail)->code,        //araki
            (af->elems+i)->cangle);
  ...
  for(i=0;i<(af->nnode);i++) /*CONF VECTOR:CONFINEMENT,VALUE.*/
  {
    ...
    for(j=0;j<6;j++)
    {
      offset=6*off+j;
/*      fprintf(fin," %1d",(af->confs+offset)->iconf);   */
      fprintf(fin," %1d",(af->confs+6*i+j)->iconf);          //araki
    }
    for(j=0;j<6;j++)
    {
      offset=6*off+j;
/*      fprintf(fin," %8.8f",(af->confs+offset)->value);   */
      fprintf(fin," %8.8f",(af->confs+6*i+j)->value);       //araki
    }
    fprintf(fin,"\n");
  }
  /*LONGREACTION:UNDER CONSTRUCTION.*/
...
///////////////////////////////////////////////////////////////////080811 araki/////
int savebanddecreasedarclm2(char *fname,char *fnamex,char *fnamey,
                            struct arclmframe *af,
                            struct arclmframe *afx,
                            struct arclmframe *afy)
/*SAVE BAND DECREASED ARCLM 2.*/
/*OUTPUT REVISED BY ARAKI*/
{
  FILE *fin,*finx,*finy;
  char str[256],dandf[256];
...
/*      fprintf(fin," %8.8f",(af->confs+offset)->value);   */
  ...
  /*OUTPUT IHX.*/
  fprintf(finx,"%5d %5d %5d\n",af->nnode,af->nelem,af->nsect);

  for(i=0;i<(af->nsect);i++)
  {
    fprintf(finx,"%5d %.5E %.5f %.4f %.8f %.8f %.8f",
            (af->sects+i)->code,
            (af->sects+i)->E,
            (af->sects+i)->poi,
            (af->sects+i)->area,
    ...
    for(j=0;j<6;j++)
    {
      fprintf(finx," %9.1f %9.1f",(af->sects+i)->fmax[j],
                                  (af->sects+i)->fmin[j]);
    }
    if((af->sects+i)->type!=TYPENULL)
    {
      fprintf(finx," %5d",(af->sects+i)->type);
    }
  ...
  for(i=0;i<(af->nnode);i++)
  {
    ...
    fprintf(finx,"%5d %7.3f %7.3f %7.3f\n",
            (af->nodes+off)->code,
/*            (af->nodes+off)->d[GX],
            (af->nodes+off)->d[GY],
            (af->nodes+off)->d[GZ]);     */
...
/*            (af->elems+i)->node[0]->code,
    ...
    for(ii=0;ii<2;ii++)                  /*BOUNDARY.0:RIGID 1:HINGE*/
    {
      fprintf(finx," %1d %1d %1d",
              (af->elems+i)->iconf[ii][3],
              (af->elems+i)->iconf[ii][4],
              (af->elems+i)->iconf[ii][5]);
    }
    for(ii=0;ii<2;ii++)                                   /*STRESS FOR IHX.*/
    {
      for(jj=0;jj<6;jj++)
      {
        fprintf(finx," %.3f",(afx->elems+i)->stress[ii][jj]);
      }
    }
    fprintf(finx,"\n");
  }
  ...
  for(i=0;i<(af->nnode);i++) /*CONF VECTOR:CONFINEMENT,VALUE.*/
  {
    ...
    fprintf(finx,"%5d",(af->nodes+off)->code);

    for(j=0;j<6;j++)
    {
      offset=6*off+j;
/*      fprintf(finx," %1d",(af->confs+offset)->iconf);   */
      fprintf(finx," %1d",(af->confs+6*i+j)->iconf);    //araki
    }
    for(j=0;j<6;j++)                                       /*LOADS FOR IHX.*/
    {
      offset=6*off+j;
/*      fprintf(finx," %8.8f",(afx->confs+offset)->value); */
      fprintf(finx," %8.8f",(afx->confs+6*i+j)->value);     //araki
    }
    fprintf(finx,"\n");
  }
  /*LONGREACTION:UNDER CONSTRUCTION.*/
  ...
  /*OUTPUT IHY.*/
  fprintf(finy,"%5d %5d %5d\n",af->nnode,af->nelem,af->nsect);

  for(i=0;i<(af->nsect);i++)
  {
    fprintf(finy,"%5d %.5E %.5f %.4f %.8f %.8f %.8f",
            (af->sects+i)->code,
            (af->sects+i)->E,
            (af->sects+i)->poi,
            (af->sects+i)->area,
    ...
    for(j=0;j<6;j++)
    {
      fprintf(finy," %9.1f %9.1f",(af->sects+i)->fmax[j],
                                  (af->sects+i)->fmin[j]);
    }
    if((af->sects+i)->type!=TYPENULL)
    {
      fprintf(finy," %5d",(af->sects+i)->type);
    }
  ...
  for(i=0;i<(af->nnode);i++)
  {
    ...
    fprintf(finy,"%5d %7.3f %7.3f %7.3f\n",
            (af->nodes+off)->code,
/*            (af->nodes+off)->d[GX],
            (af->nodes+off)->d[GY],
            (af->nodes+off)->d[GZ]);     */
...
/*            (af->elems+i)->node[0]->code,
    ...
    for(ii=0;ii<2;ii++)                  /*BOUNDARY.0:RIGID 1:HINGE*/
    {
      fprintf(finy," %1d %1d %1d",
              (af->elems+i)->iconf[ii][3],
              (af->elems+i)->iconf[ii][4],
              (af->elems+i)->iconf[ii][5]);
    }
    for(ii=0;ii<2;ii++)                                   /*STRESS FOR IHY.*/
    {
      for(jj=0;jj<6;jj++)
      {
        fprintf(finy," %.3f",(afy->elems+i)->stress[ii][jj]);
      }
    }
    fprintf(finy,"\n");
  }
  ...
  for(i=0;i<(af->nnode);i++) /*CONF VECTOR:CONFINEMENT,VALUE.*/
  {
    ...
    fprintf(finy,"%5d",(af->nodes+off)->code);

    for(j=0;j<6;j++)
    {
      offset=6*off+j;
/*      fprintf(finy," %1d",(af->confs+offset)->iconf);  */
      fprintf(finy," %1d",(af->confs+6*i+j)->iconf);      //araki
    }
    for(j=0;j<6;j++)                                       /*LOADS FOR IHY.*/
    {
      offset=6*off+j;
/*      fprintf(finy," %8.8f",(afy->confs+offset)->value); */
      fprintf(finy," %8.8f",(afy->confs+6*i+j)->value);   //araki
    }
    fprintf(finy,"\n");
  }
  /*LONGREACTION:UNDER CONSTRUCTION.*/
  ...
  sprintf(str,"Saved As %s, %s, %s",fname,fnamex,fnamey);
  MessageBox(NULL,str,"Save",MB_OK);

  return 1;
}/*savebanddecreasedarclm2*/
//////////////////////////////////////////////////////////////////////////////
                         ...
                         double cangle)
{
  ...
  else if((dl2/dl1)<0.1) /*DEFORMED COLUMN*/    //MIHARA (DEFAULT0.1)
  {
    ox=dx/dl1;
    oy=dy/dl1;
    oz=dz/dl1;
...
/*REACTIONS OUTPUT IN OHX,OHY STYLE.*/
{
  ...
  for(i=1;i<=6*nnode;i++)
  {
    ...
    if(iconf==1)
    {
      reaction=*(dreact+nreact)-*(reactlong+nreact);     /*É∞{dR}={R}-{RL}*/
...
/*DRAW SELECTING RECT BEFORE DEFINITION.*/
{
  ...
  if(wp.hdcB != NULL && wp.hdcC != NULL)
  {
    ...
    ppen = SelectObject(wp.hdcC,hpen);
...
/*DRAW ADDING CURVE.*/
{
  ...
  if(wp.hdcB != NULL && wp.hdcC != NULL)
  {
    ...
    ppen=SelectObject(wp.hdcC,hpen1);
    pbrsh = SelectObject(wp.hdcC,hbrsh1);
...
/*DISTRIBUTE ELEMENT WEIGHT INTO NODE WEIGHT,CMQ.*/
{
  ...
  else if(!strncmp(non,"Mori",4)) strcpy(prj,"Mori");
  ...
  else if(!strncmp(non,"Matu",4)) strcpy(prj,"Matu");
  else if(!strncmp(non,"tuki",4)) strcpy(prj,"tuki");
  else if(!strncmp(non,"Shiojirisato",12)) strcpy(prj,"Shiojirisato");
  ...
  else if(!strncmp(non,"kase",4)) strcpy(prj,"kase");
  else if(!strncmp(non,"gcdp",4)) strcpy(prj,"gcdp");
  else if(!strncmp(non,"saito",5)) strcpy(prj,"saito");
  else if(!strncmp(non,"ten",3))  strcpy(prj,"ten");
  else if(!strncmp(non,"turu",4)) strcpy(prj,"turu");
  else if(!strncmp(non,"obj",3)) strcpy(prj,"obj");
  else if(!strncmp(non,"taiwan",6)) strcpy(prj,"taiwan");
  else if(!strncmp(non,"azabu",5)) strcpy(prj,"azabu");
  ...
  else if(!strncmp(non,"yamagakou",9)) strcpy(prj,"yamagakou");
  else if(!strncmp(non,"yamagahyou",10)) strcpy(prj,"yamagahyou");
  else if(!strncmp(non,"yamagakanri",11)) strcpy(prj,"yamagakanri");
  else if(!strncmp(non,"yamagahall",10)) strcpy(prj,"yamagahall");
  else if(!strncmp(non,"kyouai",6)) strcpy(prj,"kyouai");
  else if(!strncmp(non,"maebashi",8)) strcpy(prj,"maebashi");
  else if(!strncmp(non,"beijin",6)) strcpy(prj,"beijin");
  ...
  else if(!strncmp(non,"kaga",4)) strcpy(prj,"kaga");
  else if(!strncmp(non,"shibuya",7)) strcpy(prj,"shibuya");
  else if(!strncmp(non,"nishinomiya",11)) strcpy(prj,"nishinomiya");
  else if(!strncmp(non,"ureshi",5)) strcpy(prj,"ureshi");
...
/////////////////////////////////////////////////////////////////////////////////
  ...
  if(baseslab!=0.0)
  {
    sprintf(non,"\nTOTAL WEIGHT FOR SLAB=%.3f,FRAME=%.3f(REACTION=%.4f),EQ=%.3f",
...
if(!strcmp(prj,"vene")) {}       //for venezia
if(!strcmp(prj,"obj")) {}        //for obj
/*if(!strcmp(prj,"kato")) {}*/        //for kato
/*if(!strcmp(prj,"mita")) {}*/        //for mita
else if(ai.Co==0.0) ai.Co=0.300; /*BASE SHEAR 0.2,0.3,1.0*/
...
if(!strncmp(prj,"Matu",4))
{
  ai.nfloor=6;
}

if(!strncmp(prj,"tuki",4))
{
  ai.nfloor=9;
}

if(!strncmp(prj,"Shiojirisato",12))
{
  ai.nfloor=7;
}
...
if(!strncmp(prj,"yoko",4))
{
  ai.nfloor=5;
}

if(!strncmp(prj,"kase",4))
{
  ai.nfloor=4;
}

if(!strncmp(prj,"gcdp",4))
{
  ai.nfloor=4;
}

if(!strncmp(prj,"saito",5))
{
  ai.nfloor=4;
}

if(!strncmp(prj,"ten",3))
{
  ai.nfloor=5;
}
...
if(!strncmp(prj,"lacie",5))
{
  ai.nfloor=3;
}

if(!strncmp(prj,"yamagakou",9)||
   !strncmp(prj,"yamagahyou",10)||
   !strncmp(prj,"yamagakanri",11))
{
  ai.nfloor=3;
}

if(!strncmp(prj,"yamagahall",10))
{
  ai.nfloor=2;
}

if(!strncmp(prj,"kyouai",6))
{
  ai.nfloor=5;
}

if(!strncmp(prj,"maebashi",8))
{
  ai.nfloor=2;
}

if(!strncmp(prj,"beijin",6))
{
  ai.nfloor=2;
}

if(!strncmp(prj,"aaxc",4))
{
  ai.nfloor=8;
}

if(!strncmp(prj,"ordos",5))
{
  ai.nfloor=8;
}
...
if(!strncmp(prj,"kaga",4))
{
  ai.nfloor=3;
}

if(!strncmp(prj,"shibuya",7))
{
  ai.nfloor=9;
}

if(!strncmp(prj,"nishinomiya",11))
{
  ai.nfloor=2;
}
if(!strncmp(prj,"ureshi",5))
{
  ai.nfloor=3;
}
  ...
  else if(!strncmp(prj,"Shiojirisato",12))
  {
    *(ai.lbound+ 0)= -5.0;
    *(ai.lbound+ 1)= -0.1;
    *(ai.lbound+ 2)=  0.1;
  ...
  else if(!strncmp(prj,"yamagakou",9)||
          !strncmp(prj,"yamagahyou",10)||
          !strncmp(prj,"yamagakanri",11))
  {
    *(ai.lbound+ 0)=-50.0;
  ...
  else if(!strncmp(prj,"yamagahall",10))
  {
    *(ai.lbound+ 0)=-50.0;
    *(ai.lbound+ 1)=  0.1;
    *(ai.lbound+ 2)=100.0;
  ...
  else if(!strncmp(prj,"maebashi",8))
  {
    *(ai.lbound+ 0)=-50.0;
    *(ai.lbound+ 1)=  0.1;
    *(ai.lbound+ 2)=100.0;
  ...
  else if(!strncmp(prj,"hakushima",9))
  {
    *(ai.lbound+ 0)=-50.0;
    *(ai.lbound+ 1)=  0.1;
    *(ai.lbound+ 2)=100.0;
  ...
  else if(!strncmp(prj,"nishinomiya",11))
  {
    *(ai.lbound+ 0)=-50.0;
    *(ai.lbound+ 1)=  0.1;
    *(ai.lbound+ 2)=100.0;
  ...
  /*ai.hmax-=(*(ai.levels+0));*/ /*HEIGHT*/   /* araki for turu */
