/*白島応答用, LOOXよりコピー, ARCHG120-3.C との同期未完了*/
...
struct oconf{signed char iconf;
             double value,kconf;/*ELASTIC CONF*/}; /*ONLY FOR ANALYSIS.*/
...
void modifykconf(struct gcomponent *gmtx,
                 struct oconf *confs,int nnode); /*ELATIC CONF*/
                    ...
                    int mode)
{
  ...
  if(mode==ONPRINTER || mode==ONPREVIEW)
  {
    setfontformat(hdc,gprn.jiheight,gprn.jiwidth,"ＭＳ 明朝",0,0,0);
  }
...
/*MessageBox(NULL,str,"Input",MB_OK);*/
        ...
        if(nstr-pstr>=7 && (org->nnode)-pnode>0 &&
           !strcmp(*(data+pstr),"KCON")) /*ELASTIC CONF*/
        {
          pstr++;
          for(i=0;i<=5;i++)
          {
            (org->confs+6*((org->nodes+pnode)->loff)+i)->kconf
            =strtod(*(data+pstr+i),NULL);
          }
          pstr+=6;
          ident++;
        ...
        if((org->nodes+i)->code==(az->elems+j)->node[m]->code)
        {
          ...
          *(ay->ninit+k)=*(ay->nodes+k);
          ...
          for(n=0;n<6;n++)
          {
            ...
            (az->confs+aloff)->kconf=(org->confs+oloff)->kconf; /*ELASTIC CONF*/
            (ax->confs+aloff)->kconf=(org->confs+oloff)->kconf;
            (ay->confs+aloff)->kconf=(org->confs+oloff)->kconf;
...
/*TRANSLATE ARCLM INPUTFILE TEXT INTO MEMORY.*/
{
  ...
  for(i=1;i<=(af->nnode);i++) /*CONF VECTOR:CONFINEMENT,VALUE.*/
  {
    ...
    for(j=1;j<=6;j++)
    {
      ...
      if(n>13) /*ELASTIC CONF*/
      {
        (af->confs+offset)->kconf=strtod(*(data+j+12),NULL);
      }
      else
      {
        (af->confs+offset)->kconf=0.0;
      }

      if((af->confs+offset)->iconf==1 ||
         (af->confs+offset)->kconf!=0.0) (af->nreact)++; /*ELASTIC CONF*/
    }
...
/*SAVE AS ARCLM INPUTFILE.*/
{
  ...
  for(i=0;i<(af->nnode);i++) /*CONF VECTOR:CONFINEMENT,VALUE.*/
  {
    ...
    fprintf(fin," "); /*ELASTIC CONF*/

    for(j=0;j<6;j++)
    {
      offset=6*i+j;
      fprintf(fin," %8.5f",(af->confs+offset)->kconf); /*ELASTIC CONF*/
    }
    fprintf(fin,"\n");
  }
  /*LONGREACTION:UNDER CONSTRUCTION.*/
                         ...
                         double cangle)
{
  ...
  else if((dl2/dl1)<0.0001) /*DEFORMED COLUMN*/
  {
    ox=dx/dl1;
    oy=dy/dl1;
    oz=dz/dl1;
...
void modifykconf(struct gcomponent *gmtx,
                 struct oconf *confs,int nnode)
/*MODIFY GLOBAL MATRIX BY ELASTIC CONFS.*/
{
  signed char iconf;
  long int ii,jj;
  double gstiff,kconf;
  ...
  {
    iconf=(confs+ii-1)->iconf;
    kconf=(confs+ii-1)->kconf;

    if(kconf!=0.0)
    {
      gread(gmtx,ii,ii,&gstiff);
      gstiff+=kconf; /*ELASTIC CONF*/
      gwrite(gmtx,ii,ii,gstiff);
