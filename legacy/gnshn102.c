...
#define FLOORS 5 /*INCLUDING GL,RFL*/
...
/*GRAVITY LOADED ANALYSIS IS ALREADY DONE BY ARCLM.*/
{
  ...
  {
    fprintf(ftxt,"   LAP");
    for(i=0;i<FLOORS;i++) fprintf(ftxt,"         Dx%2d         Dy%2d",
                                  i+1,i+1);
    for(i=0;i<FLOORS-1;i++) fprintf(ftxt,"         Qx%2d         Qy%2d",
                                    i+1,i+1);
  ...
  /*fxacc=fopen("HcnhEW.txt","r");*/ /*Toshima*/
  /*fxacc=NULL;*/ /*X*/
  ...
  /*afact=50.0/ 36.99;*/   /*HACHINOHE LEVEL 2*/
...
for(i=0;i<nelem;i++)
{
  if((elems+i)->code==1041 ||
     ...
     (elems+i)->code==1258)
  {
    if(ferr!=NULL) fprintf(ferr,"ELEM %d N= %14.5f\n",(elems+i)->code,(elems+i)->stress[0][0]);
  }
}
for(i=0;i<nelem;i++)
{
  if((elems+i)->code==1041 ||
     ...
     (elems+i)->code==1258)
  {
    if(ferr!=NULL) fprintf(ferr,"MELEM %d N= %14.5f\n",
                           (af->melem+i)->code,
                           (af->melem+i)->stress[0][0]);
  }
}
#endif
...
if(ferr!=NULL) fprintf(ferr,"     N:ELEM1041");
if(ferr!=NULL) fprintf(ferr,"     N:ELEM1042");
if(ferr!=NULL) fprintf(ferr,"     N:ELEM1043");
if(ferr!=NULL) fprintf(ferr,"     N:ELEM1245");
if(ferr!=NULL) fprintf(ferr,"     N:ELEM1252");
if(ferr!=NULL) fprintf(ferr,"     N:ELEM1254");
if(ferr!=NULL) fprintf(ferr,"     N:ELEM1256");
if(ferr!=NULL) fprintf(ferr,"     N:ELEM1258");
if(ferr!=NULL) fprintf(ferr,"     dX:NODE123     dY:NODE123");
if(ferr!=NULL) fprintf(ferr,"     dX:NODE135     dY:NODE135\n");
*/
...
w1=2.0*PI/T1;
  ...
  for(nlap=1;nlap<=laps;nlap++)
  {
	...
	errormessage(string);
	if(fout!=NULL) fprintf(fout,"%s\n",string);
	if(ferr!=NULL) fprintf(ferr," %5d %8.3f",nlap,nlap*ddt);
...
/*if(elem.code==1501)
{
  if(fout!=NULL) fprintf(fout,"ELEM%d [ke0]\n",elem.code);
  for(ii=0;ii<12;ii++)
  {
    for(jj=0;jj<12;jj++)
    {
...
/*if(elem.code==1501)
{
  if(fout!=NULL) fprintf(fout,"ELEM%d [ke1]\n",elem.code);
  for(ii=0;ii<12;ii++)
  {
    for(jj=0;jj<12;jj++)
    {
...
/*if(elem.code==1501)
{
  if(fout!=NULL) fprintf(fout,"ELEM%d [ke2]\n",elem.code);
  for(ii=0;ii<12;ii++)
  {
    for(jj=0;jj<12;jj++)
    {
...
/*if(elem.code==1501)
{
  if(fout!=NULL) fprintf(fout,"ELEM%d [ke3]\n",elem.code);
  for(ii=0;ii<12;ii++)
  {
    for(jj=0;jj<12;jj++)
    {
...
/*if(elem.code==1501)
{
  if(fout!=NULL) fprintf(fout,"ELEM%d [e]\n",elem.code);
  for(ii=0;ii<12;ii++)
  {
    for(jj=0;jj<12;jj++)
    {
      if(fout!=NULL) fprintf(fout," %12.5E",*(*(e+ii)+jj));
    }
    if(fout!=NULL) fprintf(fout,"\n");
  }
  if(fout!=NULL) fprintf(fout,"\n");
...
/*if(elem.code==1501)
{
  ...
  for(ii=0;ii<12;ii++)
  {
    for(jj=0;jj<12;jj++)
    {
      if(fout!=NULL) fprintf(fout," %12.5E",*(*(t+ii)+jj));
    }
    if(fout!=NULL) fprintf(fout,"\n");
  }
  if(fout!=NULL) fprintf(fout,"\n");
...
/*if(elem.code==1501)
{
  if(fout!=NULL) fprintf(fout,"ELEM%d [ke4]\n",elem.code);
  for(ii=0;ii<12;ii++)
  {
    for(jj=0;jj<12;jj++)
    {
...
/*if(elem.code==1501)
{
  if(fout!=NULL) fprintf(fout,"ELEM%d [ke5]\n",elem.code);
  for(ii=0;ii<12;ii++)
  {
    for(jj=0;jj<12;jj++)
    {
  ...
  if(fout!=NULL) fprintf(fout,"\n");
    ...
    for(i=0;i<3;i++)
    {
      ...
      /*tdis[i]+=ddt*(pdis[i]+tvel[i])/2.0;*/ /*ERROR*//*GROUND DISPLACEMENT.*/
...
#if 0    /*‰ðÍØ‚è‘Ö‚¦*/  /*0:‰ž“š‰ðÍ,1:ŒÅ—L’l‰ðÍ*/

MessageBox(NULL,"Calculate Eigen Period.","GNSHN",MB_OK);
...
#if 1 /*FOR PROPORTION TO OCCASIONAL STIFFNESS I.*/
if(dtype==2)
{
    if(h1==0.0) cmtx=NULL;
    else
...
#if 1 /*FOR PRORORTION TO OCCASIONAL STIFFNESS II.*/
if(dtype==3)
{
    for(i=1;i<=msize;i++)                         /*INITIALIZATION.*/
    {
...
if(fout!=NULL) fprintf(fout,"T1=%.5f w1=%.5f h1=%.5f\n",T1,w1,h1);
/*
if(cmtx!=NULL)
{
  if(fout!=NULL) fprintf(fout,"[C]\n");
  for(ii=451;ii<=462;ii++)
  {
    for(jj=451;jj<=462;jj++)
    {
      gread(cmtx,ii,jj,&data);
      if(fout!=NULL) fprintf(fout," %12.5E",data);
    }
...
/*if(elem.code==1012 || elem.code==1501)*/
/*if((elems+i)->code==1719 ||
   (elems+i)->code==1721 ||
   (elems+i)->code==1736 ||
   (elems+i)->code==1738 ||
   (elems+i)->code==1724 ||
...
/*if((elems+i)->code==1041 ||
   ...
   (elems+i)->code==1256 ||
   (elems+i)->code==1258)*/
/*(elem.code==1012 || elem.code==1501 ||
   elem.code==1304 || elem.code==1421 ||
   elem.code==1321 || elem.code==1331 ||
   elem.code==1333 || elem.code==1338 ||
   elem.code==1074 || elem.code==1076 ||
   elem.code==1057 || elem.code==1059 ||
   elem.code==1061 || elem.code==1063)*/
if(0) /*FOR TOSHIMA*/
{
      outputstress(elem,estress,fout,func);

  fprintf(ferr," %9d %12.5f %12.5f %12.5f %12.5f %12.5f",
          elem.code,elem.stress[0][0],elem.stress[0][2],elem.stress[1][2],elem.stress[0][4],elem.stress[1][4]);
...
/*if((nodes+ii)->code==257 ||
   (nodes+ii)->code==258)*/
/*if((nodes+ii)->code==230 ||
   (nodes+ii)->code==247)*/
/*if((nodes+ii)->code==123 ||
   (nodes+ii)->code==135)*/
/*if((nodes+ii)->code==257 ||
   (nodes+ii)->code==258 ||
   (nodes+ii)->code==377 ||
   (nodes+ii)->code==601 ||
   (nodes+ii)->code==613)*/
/*if((nodes+ii)->code==167 ||
   (nodes+ii)->code==111 ||
   (nodes+ii)->code==112 ||
   (nodes+ii)->code==121 ||
   (nodes+ii)->code==132)*/ /*FOR TOSHIMA*/
if((nodes+ii)->code==109 ||
   (nodes+ii)->code==110 ||
   (nodes+ii)->code==126 ||
   (nodes+ii)->code==135) /*FOR TOSHIMA*/
{
  ...
  fprintf(ferr," NODE %9d %12.5f %12.5f %12.5f %12.8f %12.8f %12.8f",
          (nodes+ii)->code,
          *(ddisp+6*ii+0)-(mnode+ii)->d[0],
          *(ddisp+6*ii+1)-(mnode+ii)->d[1],
          *(ddisp+6*ii+2)-(mnode+ii)->d[2],
          *(u+6*ii+0),*(u+6*ii+1),
          *(ud+6*ii+0),*(ud+6*ii+1),
          *(udd+6*ii+0),*(udd+6*ii+1));

      /*if(ferr!=NULL) fprintf(ferr," %14.5f",*(ddisp+6*ii+2)-8.6);*/
/*if((nodes+ii)->code==123 && ferr!=NULL)
{
  fprintf(ferr," %14.5f %14.5f",
          *(ddisp+6*ii+0)+0.799,*(ddisp+6*ii+1)+1.300);
}*/
/*if((nodes+ii)->code==135 && ferr!=NULL)
{
  fprintf(ferr," %14.5f %14.5f",
          *(ddisp+6*ii+0)+2.035,*(ddisp+6*ii+1)-0.402);
}*/

/*if((nodes+ii)->code==247) fprintf(ferr,"\n");*/
/*if((nodes+ii)->code==135) fprintf(ferr,"\n");*/
                 ...
                 struct arclmframe *af,double *ddisp,double dacc[])
{
  ...
  /*double flevel[FLOORS+1]={ -0.1,  0.1,   5.0, 10.0,
                              16.0, 17.5, 100.0};*//*HAKONE*/
  double flevel[FLOORS+1]={ -7.0, -0.5, 2.0, 8.0, 12.0, 20.0};
...
{
  if(fout!=NULL)
  fprintf(fout,"ELEM %d:nz=%9.5f[tf] qx=%9.5f[tf] qy=%9.5f[tf] mz=%9.5f[tfm] mx=%9.5f[tfm] my=%9.5f[tfm]\n",
          elem->code,*(estress+0),*(estress+1),*(estress+2),*(estress+3),*(estress+4),*(estress+5));
}
*/
    for(ii=1;ii<=(FLOORS-1);ii++)
    {
...
{
  if(fout!=NULL)
  fprintf(fout,"ELEM %d:Fx=%9.5f[tf] Fy=%9.5f[tf] Fz=%9.5f[tf] Mx=%9.5f[tfm] My=%9.5f[tfm] Mz=%9.5f[tfm]\n",
          elem->code,*(gstress+0),*(gstress+1),*(gstress+2),*(gstress+3),*(gstress+4),*(gstress+5));
}
*/
/*fprintf(stderr,"ELEM %d:Fx=%.5f[tf] Fy=%.5f[tf] Fz=%.5f[tf]",
        elem.ielem,*(gstress+0),*(gstress+1),*(gstress+2));*/
/*gets(non);*/
  ...
  for(ii=1;ii<=(FLOORS-1);ii++)
  {
    ...
    if(ftxt!=NULL)
    {
      ...
      fprintf(ftxt," %12.5E %12.5E",
              sumshear[ii-1][0],sumshear[ii-1][1]);
    }
  }
