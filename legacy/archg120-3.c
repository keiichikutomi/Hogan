...
struct onode{long int code,loff;     /*CODE NUMBER,OFFSET POSITION.*/   //note:code=節点番号, loff=通し番号
			 double d[3];};          //note:節点座標 d[0],d[1],d[2] =(x,y,z)
...
double **mallocdoublematrixxyz(int msize);
...
int extractarclmfromorganquiet(struct organ *org,               //ujioka
                          struct arclmframe *az,
                          struct arclmframe *ax,
						  struct arclmframe *ay);
...
//void gfree2(struct gcomponent *gmtx,long int nnode);       //ujioka
...
void weightdistribution(HDC hdc,FILE *fout,struct viewparam *vp,
						struct organ *org);
void weightdistributionquiet(HDC hdc,FILE *fout,struct viewparam *vp,
						struct organ *org);
  ...
  for(i=0;i<6;i++) /*0:Nz 1:Qx 2:Qy 3:Mz 4:Mx 5:My*/
  {
    ...
    for(j=1;j<=7;j++) /*ELEMENT TYPE.*/
    {
         ...
         ev.stress[j][i])
      {
		...
		// sprintf(str,"%.2f",globalunit*elem.stress[0][i]);   /*HEAD.*/ // default
		sprintf(str,"%.0f",globalunit*elem.stress[0][i]);   /*HEAD.*/
		...
		// sprintf(str,"%.2f",globalunit*elem.stress[1][i]);   /*TAIL.*/ // default
		   sprintf(str,"%.0f",globalunit*elem.stress[1][i]);   /*TAIL.*/
		tn=setcoord(((1-0.25/**(i+1)*/)*elength),0.0,0.0);
		/*drawtextalignedonlocaldot(hdc,vp,tdrccos,ninit[0],tn,str,
								  TEXT_CENTER,TEXT_TOP);*/
        // drawtextonlocaldot(hdc,vp,tdrccos,ninit[0],tn,str); // 200403 furuichi
...
/*drawglobalwire(hdc,vp,af,*(af.elems+i),255,150,50,
               ...
               (af.elems+i)->srate[3]>0.71428))
      {
        ...
        else
        {
          drawglobalwire(hdc,vp,af,*(af.elems+i),255,255,0,
              ...
              ((af.elems+i)->srate[0]>0.5 || /*0.7*//*0.6*//*0.5*/
               ...
               (af.elems+i)->srate[3]>0.5))
      {
        drawglobalwire(hdc,vp,af,*(af.elems+i),150,255,0,
              ...
              ((af.elems+i)->srate[0]>0.3 || /*0.6*//*0.5*//*0.3*/
               ...
               (af.elems+i)->srate[3]>0.3))
      {
        drawglobalwire(hdc,vp,af,*(af.elems+i),0,255,0,
              ...
              ((af.elems+i)->srate[0]>0.2 || /*0.5*//*0.4*//*0.2*/
...
int extractarclmfromorganquiet(struct organ *org,
                          struct arclmframe *az,
						  struct arclmframe *ax,
						  struct arclmframe *ay)
/*EXTRACT ARCLM FROM ORGAN.*/
...
*/
  ...
  /*MASS, AI EARTHQUAKE, CMQ*/
  fout=fopen("hogtxt.wgt","w");
  weightdistributionquiet(NULL,fout,NULL,org);
  fclose(fout);
...
/*STATIC INCREMENTAL ANALYSIS.*/
{
  ...
  FILE *fin,*fout,*ffig,*ferr;                       /*FILE 8 BYTES*/
  double *ddisp,*dreact;
  struct memoryelem *melem;
...
/*if(nlap*dsafety<0.1)
{
     ...
     elem.sect->code==615)
  {
	...
	for(ii=0;ii<nnode;ii++)
	{
	  ...
	  if(ffig!=NULL && (nodes+ii)->code==103)
